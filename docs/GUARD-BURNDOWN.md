...
|---|---|---|---|---|
| **`src/test/*.test.ts*`** | 567 | **567 — COMPLETE** | 0 | **0** |
| **`src/test/edge/` (money)** | 72 | **72 — COMPLETE** | 0 | **0** |
| **colocated beside components** | 378 | 377 | 0 | **1** |
| **Playwright `e2e/`** | 69 | **60 — COMPLETE** | 9 | **0** |
| **total** | **1086** | **1076 (99%)** | **9** | **1** |

`npm run vacuity` prints the same three numbers on every run:

```
registration: 1076/1086 guards register a mutation (9 exempt with a reason, 0 grandfathered)
```
