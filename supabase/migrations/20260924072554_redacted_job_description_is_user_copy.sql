-- AL-012: purge_user_data wrote the internal marker '[removed at account deletion]'
-- into a retained job's description, and every surface that renders a job
-- (applied/posted cards, job dialog, stat tiles) printed it raw to the helper
-- and applicants who still see that job. The retained description is now the
-- sentence those people should read. Body is the live pg_get_functiondef of
-- 2026-09-24 with only that literal changed (both the SET and its idempotency
-- predicate). Rows already carrying the old marker are rewritten (0 on prod
-- at authoring time).
...
UPDATE public.jobs
   SET description = 'This job''s details were removed when the poster closed their account.'
 WHERE description = '[removed at account deletion]';
