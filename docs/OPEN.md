...
- [ ] **Q211 "Pay selected" bulk payout moves no money (found by Q76, 2026-09-23).** AdminPayoutBatches.triggerBulkPayout still calls `stripe-payouts`, which only reads the CALLER's own Connect balance and ignores helper_id; assertTransferHappened turns that into a loud error per batch, so nothing is paid and nothing false is logged, but the button cannot work. The single-batch path already uses get_payout_batch_job_ids + release-payout; the bulk path should reuse it (or the button should go). Guard to add with the fix: a test that the bulk handler invokes release-payout.
- [ ] **Q212 fan_out_broadcast_to_notifications looks dead (found by Q76, 2026-09-23; source read only, not verified live).** No caller in src/, supabase/functions or any migration: sweep_pending_broadcast_fan_outs inlines the same fan-out, and EXECUTE is revoked from authenticated (20260518150000) while the body requires an admin auth.uid(), so no role can usefully call it. Verify on prod (pg_proc.proacl, pg_stat_user_functions calls) and drop it in a migration if unused; then remove its SQL_EXEMPT entry in src/test/adminActionsAreAudited.test.ts (the two-way check will demand it).
...
- [~] **Q76 FIXED IN CODE 2026-09-23 (branch cloud/q76-admin-audit-trail), live check pending. Guard: src/test/adminActionsAreAudited.test.ts** derives the admin-action inventory from source (SQL functions gating on has_role(..,'admin') that write, read from the NEWEST migration with DROP honoured; edge units that call has_role admin, per `action ===` branch; every write on src/components/admin/** + src/pages/Admin*) and fails when one has no admin_audit_log write, with exact two-way exemption lists. Shown red on origin/main d4053d896: 23 offenders (resolve_stalled_job_flag; release-payout and execute-dispute-split admin paths; 20 client writes: broadcasts create/cancel/delete, admin notes add/edit/delete, support ticket status, unban, remove job, 8 marketing writes). Fixes: migration 20260923162243 (resolve_stalled_job_flag writes its own row, client write removed); _shared/adminAuditLog.ts writeAdminAudit (.select("id") + Slack on a lost row) in release-payout (admin-initiated only) and execute-dispute-split; create-payment admin_refund_general's audit insert no longer drops its error; logAdminAction now .select("id") and reports a zero-row insert.
  LEAD MUST VERIFY LIVE on a seed target after db-deploy: (1) `pg_get_functiondef('public.resolve_stalled_job_flag(uuid)'::regprocedure)` contains admin_audit_log and `proacl` has no PUBLIC/anon; resolve a seed escalated nudge as admin, expect one row action='resolve_stalled_job_flag'. (2) Admin-release a seed job's payout (release-payout) and expect action='release_payout' with the transfer id. (3) Execute a seed dispute split and expect action='execute_dispute_split'. (4) Admin general refund on a seed job: row 'job_admin_refund'. (5) From the admin UI on seed records: unban, remove job, broadcast create/cancel/delete, note add/edit/delete, support status, one marketing draft create+delete: one row each. Needs an lh-money-escrow REVIEW-ONLY pass (release-payout, execute-dispute-split, create-payment edits). Was: Every admin action (ban/suspend,
