-- RE-APPLY of 20260915034822_dispute_settlement_claim_and_race_locks.sql.
-- That migration's schema_migrations entry was recorded on prod (poisoned, via a
-- repair/failed deploy at ea2260767) WITHOUT its objects being created, so
-- `db push --include-all` skipped it and rpc_add_dispute_evidence /
-- rpc_supersede_dispute_decision / the settlement-claim objects were missing on
-- prod while the edge+client code was live. The original is fully replay-safe
-- (CREATE OR REPLACE, IF NOT EXISTS, DROP+CREATE trigger; PGlite 3x), so this
-- re-applies its body verbatim under a fresh timestamp to create the objects.
