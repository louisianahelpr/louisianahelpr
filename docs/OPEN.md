...
- [ ] 24h messaging lockout + embedded double-card: REBUILT on branch `lockout-embedded` (migration 20260914201350, supersedes 20260831053124 and the 90d23935d draft). Not landed; prod proof pending (coordinator).
...
- [ ] prod-audit post-job teardown leaves a cancelled job every run: the final Post mints a Stripe checkout session, the jobs DELETE policy (20260903055308) refuses a poster once a session exists, and `scripts/e2e/prod-lifecycle-sweeper.mjs` matches only "[E2E DO NOT ACCEPT]", not the bracket-free `E2E-PRODAUDIT` title. Needs the sweeper (or an equivalent) to cover that marker so the nightly prod-audit does not accumulate rows.
