...
- [ ] **Q211 Does the hourly canary (Q61) send email every hour? (2026-09-23)** Each run's apply and message notify poster-e2e / helper-e2e, and send-notification-email only suppresses a seed subject to a NON-seed recipient, so both @mailinator accounts may get ~48 Resend sends a day unless their email_* preferences are off. Not measured (no prod access on the branch): read `email_send_log` for the two accounts after the first live runs; if it grew, turn their email prefs off (test-owned rows) and count Resend volume under Q63. Guard once decided: a canary-run assertion that no email_send_log row landed for the run window.
...
- [ ] **Q61 Hourly canary on the core loop.** Full journeys run nightly, so a
  ...
  failure.
  STATUS 2026-09-23: BUILT on branch cloud/q61-hourly-canary, NOT yet run live
  (no shared-account passwords off CI). e2e/canary/core-loop.spec.ts (project
  `canary`) walks the loop on this commit's local build + prod as poster-e2e /
  helper-e2e: applies to the Q100 funded fixture, poster messages the
  applicant, re-mints a Stripe TEST checkout on ONE persistent unpaid is_seed
  canary job (never paid), then deletes its application, message and
  notifications in `finally` and re-reads them (red if any row is left).
  .github/workflows/core-loop-canary.yml: cron `47 * * * *`, own group, job
  timeouts 3/40/5, stands down while a scheduled/dispatched shared-account
  suite runs (scripts/canary/shared-accounts-busy.mjs), red -> `nightly-red:
  core-loop-canary` + ledger item of that title + Slack; green closes both.
  Guard: src/test/coreLoopCanary.test.ts (4 @mutate lines, all killed). Lead:
  dispatch it, confirm green, set REQUEST_BUDGET (300, an estimate) from the
  printed count, then tick. Email fan-out volume: Q211.
