...
- **Queue (this file):** 163 done, 35 partly done (fixed, protection pending), 110 open. Source of truth for work.
...
**Queue: 308 items — 163 done, 35 partly done (fixed, protection pending), 110 open.**
...
- [x] **Q214 BulkDismissBar (My Posts selection mode) floats at a hand-typed `calc(env(safe-area-inset-bottom) + 80px)` (found by Q191, 2026-09-23).** Every other bottom clearance reads the shared `--bottom-nav-h` / `safe-nav` token (96px dock default, 0 with no dock) and `--safe-area-bottom`, not env() directly. Not measured whether 80px clears the dock on every device; measure, then move it onto the token.
  Fixed 09-23: BulkDismissBar uses bottom-safe-nav; bottom 80→112px, clears dock by ~48px at 375; guard floatingBarsUseDockToken (red on original); shots ~/.lh-shots/quick-c/q214-*
...
- [x] **Q247 /pets at desktop shows two "Add a Pet" buttons when the user has no pets. (Q165 carry-over, 2026-09-23.)** Evidence: src/pages/PetProfiles.tsx:172-180 (title-row button, always rendered at lg) + :283-291 (empty-state button). From: docs/archive/VISUAL-2026-08-24.md.
  Fixed 09-23 (lane quick-c): title-row Add a Pet hidden while empty; 1440 buttons 2→1; guard emptyStateCtaNotDuplicatedInHeader (red on original); shots ~/.lh-shots/quick-c/q247-*
...
- [x] **Q293 A scheduled prod-load run can be cancelled with nothing reported (found by the Q70 review, 2026-09-23).** GitHub keeps one PENDING run per concurrency group; a third run entering prod-load cancels the waiting one at workflow level, so no job (not even notify) runs and no nightly-red issue is filed. For the monthly privacy journey that is a missed month. schedule-heartbeat.yml derives each workflow's cadence from its cron (weekly here, because of the first-7-days gate), so it cannot tell that the one DUE week was cancelled. Fix: have the privacy journey's green notify record the month it ran (e.g. the nightly-red issue sync, or a dated marker), and alarm when the current month has none after day 8. Guard: a test on that check.
  Fixed 09-23: privacy-journey records green month on a standing marker issue; schedule-heartbeat alarms after day 8 if missing; guard privacyJourneyMonthlyMarker (red on mutation).
...
- [x] **Q303 The suspension banner sits above the Delete Account dialog's backdrop (found by the Q300 owner-path run, 2026-09-23).** Screenshot ~/.lh-shots/q281/owner-path/4-after-delete.png (375, reviewed as defect): with the delete dialog open on /account-banned, the fixed 'Account suspended until ...' StrikeBanner renders un-dimmed across the middle of the screen over the modal overlay. Needs a z-index/portal fix in the banner and a screenshot before/after; not touched by Q281.
  Fixed 09-23: StrikeBanner drops to z-49 under an open Radix modal (59→49); guard persistentChromeUnderModalBackdrop (red on original); shots ~/.lh-shots/quick-c/q303-*. Dev-server verified, not dist/.
