...
- [ ] QUEUED (right after dispute-races lands, same file create-payment): `resolve_revision` re-sends the "revision fixed" notification on a duplicate call. Make it idempotent (conditional flip, skip notify when already resolved) + test red first.
