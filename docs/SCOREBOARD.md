...
| open work | audit bus findings (open / launch blockers) | **FAIL** | 267 | 94 | — | 429 | HEAD (diffed every push) | [ROLLUP.md](audit/launch-2026-09/ROLLUP.md) · `node scripts/audit-bus.mjs list --blockers` | 94 open, 3 open launch blockers; 267 fixed, 14 retracted, 31 duplicate, 1 wontfix, 22 obsolete |
| guards | vacuity: guards proven able to fail / exempt / owed | **PASS** | 934 | 0 | 9 exempt | 943 | HEAD (diffed every push) | [GUARD-BURNDOWN.md](GUARD-BURNDOWN.md) · `npm run vacuity` | registered @mutate per guard; whether each mutation is KILLED is the full-sweep row below |
