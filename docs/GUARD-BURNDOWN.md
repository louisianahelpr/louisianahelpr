...
|---|---|---|---|---|
| **`src/test/*.test.ts*`** | 341 | **341 — COMPLETE** | 0 | **0** |
| **`src/test/edge/` (money)** | 64 | **64 — COMPLETE** | 0 | **0** |
| **colocated beside components** | 350 | **350 — COMPLETE** | 0 | **0** |
| **Playwright `e2e/`** | 65 | **56 — COMPLETE** | 9 | **0** |
| **total** | **820** | **811 (99%)** | **9** | **0** |

`npm run vacuity` prints the same three numbers on every run:

```
registration: 811/820 guards register a mutation (9 exempt with a reason, 0 grandfathered)
```
