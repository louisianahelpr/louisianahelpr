...
- **Queue (this file):** 109 done, 12 partly done (fixed, protection pending), 85 open. Source of truth for work.
...
**Queue: 206 items — 109 done, 12 partly done (fixed, protection pending), 85 open.**
...
- [~] **Q174 sweep_cron_http_failures files agents' manual pg_net probes as cron failures (found working Q42, 2026-09-23).** It reads every net._http_response row (pg_net does not record which cron sent a request) and names a body-less timeout by proximity to the nearest HTTP cron. Measured: the 23 Sep 08:00Z "arrival-confirm-reminder … Timeout of 400 ms" row cannot be the cron (its command uses timeout_milliseconds 30000); the "money-reconciliation returned 500" rows at 08:00Z/08:15Z were manual `?include_seed=1` POSTs at 07:52, 07:54 and 08:12Z (function_edge_logs). Both ledger items closed with that evidence. Fix: have cron commands tag their requests (e.g. an `x-cron-job` header or `?cron=<jobname>`) and have the sweeper skip untagged responses, or have probes use a marker; guard red on a planted untagged 500.
  - STATUS 2026-09-23 (branch cloud/q174-cron-request-ids, NOT yet on main or prod): migration 20260923162402_cron_http_request_ids adds public.cron_http_requests(request_id PK, jobname, created_at) and public.cron_http_tag(request_id, jobname); wraps every live cron.job command containing net.http_post( IN PLACE via cron.alter_job as `SELECT public.cron_http_tag(q.request_id, '<jobname>') FROM (<command byte for byte>) AS q(request_id);` (not re-typed from the migrations: 20260505220500/20260922222716 rewrote the live commands and 20260831190419 left 13 untouched), skipping and RAISE WARNING-ing any with 2+ posts / 2+ statements / not a SELECT, and warning on any live HTTP cron not among the 26 the migrations define (or listed but absent); restates sweep_cron_http_failures from 20260828030000 with an INNER JOIN on cron_http_requests (untagged responses, i.e. manual probes and the sweepers' own Slack posts, are no longer filed; proximity attribution removed); prune_cron_http_requests() hourly at :28 (2 days kept) with a 3h liveness expectation. GUARDS: src/test/cronHttpRequestsAreTagged.test.ts (26 HTTP crons derived from the migrations by src/test/helpers/cronHttpJobs.ts; red with the migration absent, on each of 4 @mutate lines, on a planted unwrapped future HTTP cron and on a wrong-name tag) + src/test/pglite/cronHttpTag.pglite.mjs (31/31 PASS, 3x apply; BEFORE=1 on 20260828030000's sweep: 5 FAIL, the untagged probe filed as auto-release-payment 'ambiguous (3 crons started together)'). LEAD MUST VERIFY LIVE after db-deploy: (1) the deploy log's `Q174:` NOTICE says 26 (or the live count) tagged and shows NO 'NOT tagged' / 'live but in no migration' / 'does not have' WARNING, else wrap those by hand; (2) `select jobname from cron.job where command like '%net.http_post(%' and command not like '%cron_http_tag(%'` returns 0 rows; (3) within an hour `select jobname, count(*) from public.cron_http_requests group by 1` shows the frequent crons (process-email-queue, auto-release-payment...) and cron.job_run_details for them stays 'succeeded'; (4) pg_get_functiondef('public.sweep_cron_http_failures()'::regprocedure) contains `JOIN public.cron_http_requests tag`; (5) pg_proc.proacl of cron_http_tag / sweep_cron_http_failures / prune_cron_http_requests and relacl of cron_http_requests have no anon/authenticated/PUBLIC; (6) a manual `select net.http_post(...)` to a 500ing URL is NOT filed by the next sweep. Tick [x] after that. Needs a REVIEW-ONLY lh-silent-failure pass (it rewrites money crons' commands).
- [ ] **Q207 Slack alert POSTs made by SQL functions have no delivery check once Q174 lands (found by the Q174 lane, 2026-09-23).** sweep_cron_http_failures, sweep_silent_cron_failures, reap_stranded_instant_payouts, check_stale_dispute_settlement_claims, sweep_dead_crons and others call net.http_post to slack-ops-alert directly. Before Q174 a failed one was (mis)filed by the cron-http sweep under a nearby cron's name; after Q174 it is untagged and not filed at all. Only send_ops_daily_digest records its request id and checks delivery (check_ops_digest_delivery). Not measured: how often those POSTs fail on prod. Fix idea: have them tag their request ids too (a distinct 'alert:<fn>' jobname via cron_http_tag, filed as an alert-delivery failure rather than a cron failure). Guard: every net.http_post in a migration-defined function goes through a tagging call.
