// Probe (Q281): 20260923180950_ban_enforcement_everywhere.sql and its class
...
//    same-transaction carve-out), and the probe's measured behaviour
...
//    auth-level ban on a row, a removed carve-out, a dropped marker trigger.
...
const MIG = read("../../supabase/migrations/20260923180950_ban_enforcement_everywhere.sql");
...
console.log("1. BEFORE (prod pre-Q281 shape): class check RED; banned caller writes");
{
  ...
  expect(summary["gate:no-same-txn-carveout"] === 1, "old gate has no same-transaction carve-out: flagged");
...
await plant("drop the marker trigger", `DROP TRIGGER trg_mark_ban_started_in_txn ON public.profiles`, "gate:no-same-txn-carveout", "public.enforce_ban_gate");
