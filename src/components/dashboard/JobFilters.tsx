    ...
    <div className="rounded-lg border border-border bg-background px-3 py-2 space-y-2">
      ...
      {filtersOpen && (
        <div className="space-y-3 pt-2 border-t border-border">
          ...
          <div>
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide mb-1.5">Expires within</p>
            <div className="flex flex-wrap gap-1">
              {[
                ...
                <button key={opt.value} onClick={() => setExpiresWithin(expiresWithin === opt.value ? "" : opt.value)}
                  className={`px-2 py-1 rounded-full text-[11px] font-medium transition-colors ${expiresWithin === opt.value ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
                  {opt.label}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-1">
              {[
                ...
                <button key={opt.value} onClick={() => setSortBy(opt.value)}
                  className={`px-2 py-1 rounded-full text-[11px] font-medium transition-colors ${sortBy === opt.value ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
                  {opt.label}
                </button>
              ))}
            </div>
          </div>
          {hasFilters && <Button variant="ghost" size="sm" onClick={clearFilters} className="text-muted-foreground text-xs h-7"><X className="w-3.5 h-3.5 mr-1" /> Clear all</Button>}
        </div>
      )}
