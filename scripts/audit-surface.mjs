...
const legalTabs = literalsFrom("src/pages/info/legal/legalSections.ts", "VALID_TABS", "[", "]", "legal tabs");
const adminViews = keysFromObject("src/pages/admin/Admin.tsx", "VIEW_LABELS", "admin views");
...
for (const f of readdirSync(MIGRATIONS).filter((n) => n.endsWith(".sql"))) {
  ...
  // ONLY tuples inside an INSERT INTO notification_type_pref_map statement.
  // This used to scan everything AFTER the first mention of the table, which
  // picked up 'banned', 'phase' and 'sweep_daily_job_digest' from unrelated
  // statements — SURFACE.md said 21 types while prod's table held 18
  // (measured 2026-09-23 via execute_sql; 0 types sent without a map row).
  for (const stmt of sql.matchAll(/insert\s+into\s+(?:public\.)?notification_type_pref_map\b[\s\S]*?;[ \t]*$/gim)) {
    for (const m of stmt[0].matchAll(/\(\s*'([a-z0-9_]+)'\s*,\s*(?:'[a-z0-9_]*'|NULL)\s*,/gi)) notifTypes.add(m[1]);
  }
}
...
L.push("");
L.push(`**Every row states its UNIT.** Overlays are counted as instances (${surface.overlayCount}) not`);
L.push(`files (${surface.overlays.length}); emails as exported templates (${surface.emails.length}) not files (${new Set(surface.emails.map((e) => e.file)).size}); admin as files`);
L.push("including `pages/admin/Admin*`; toasts as call sites. Every earlier pass that produced");
...
L.push(`| Admin components (components/admin + pages/admin/Admin*) | **file** | ${surface.adminFiles.length} |`);
L.push(`| Email templates | **exported template** | ${surface.emails.length} |`);
// Parsed from the INSERT statements that fill the lookup table. NT-001 (types
// prod sent with no map row, bypassing every per-category preference) was fixed
// 2026-09-03; measured 2026-09-23: prod's table holds 18 types and 0 sent types
// lack a row. The count below is derived, so it moves with the next migration.
L.push(`| Notification types (defined in notification_type_pref_map) | type | ${surface.notifTypes.length} |`);
