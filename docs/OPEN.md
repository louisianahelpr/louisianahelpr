...
- [ ] **Q208 Real page-load times are not measured (found by Q66, 2026-09-23).** Web: real-user timings go only to Vercel Speed Insights (`SpeedInsights` in src/App.tsx) and no script or CI secret reads them; Lighthouse is one lab load per URL weekly, not a p95. App: nothing records load or launch timing. Until one is recorded, the two Q66 page-load targets show "not measured" on the scoreboard. Do: record navigation timing (web + native, e.g. a sampled `page_load` analytics_events row with duration, or a Speed Insights export if Vercel offers one with a token CI can hold), then give scripts/slo.mjs a measurement for both and remove their `notMeasured`. Guard: src/test/sloTargetsTwoWay.test.ts already fails a metric that claims CI measurement without one.
- [ ] **Q211 main is red on src/test/workflowFalseGreenShapes.test.ts SWALLOW (found by the Q66/Q67 lane, 2026-09-23 ~16:35Z).** .github/workflows/a11y-webkit-prod.yml step "Surface a missing axe baseline (Q181)", added by 81be4ba08, counts the baseline with `node -e ... 2>/dev/null || echo "?"`. It only prints a count in a warning, so either list it in SWALLOW_OK with that reason or drop the `|| echo`. Guard: src/test/workflowFalseGreenShapes.test.ts itself.
...
- [ ] **Q66 Targets ("SLOs") on the scoreboard.** Define "working" as numbers:
  ...
  STATUS 2026-09-23 (branch cloud/q66-q67-slo-morning, NOT ticked: the lead ticks after a real
  scoreboard.yml run): scripts/slo.mjs defines 7 targets (Q66's six, page load split web/app),
  each with its one source; docs/SCOREBOARD.md shows a "target:" row per metric (diffed every
  push) and scoreboard.yml measures them daily (uptime from uptime.yml runs, API 5xx share from
  edge_logs, payment success from stripe_webhook_events, notification delivery from
  notification_logs, p95 time to payout from payout_transfers + jobs.completed_at), FAIL when
  missed, dated record test-results/slo/slo-<date>.json in its artifact. Both page-load targets
  are "not measured (why)" (Q208). No SQL or logs query was run against prod from this lane
  (no prod access): the first scoreboard.yml run is their first live test. GUARD:
  src/test/sloTargetsTwoWay.test.ts (Q66 list <-> slo.mjs <-> scoreboard rows, both ways;
  red when missed; never PASS unmeasured; 4 @mutate).
- [ ] **Q67 An automatic morning page.** Generated daily: what shipped (commits
  ...
  STATUS 2026-09-23 (branch cloud/q66-q67-slo-morning, NOT ticked: the lead ticks after a real
  run): scripts/morning-page.mjs writes docs/morning/<date>.md (in one line, what shipped
  grouped by Q item, what's red = scoreboard FAIL rows + nightly-red issues + alert ledger
  counts, new alerts in 24h, open OWNER items); .github/workflows/morning-page.yml runs it
  daily 12:05 UTC (07:05 Central) with the newest scoreboard artifact, publishes job summary +
  artifact until Q57 (exempt by name in prodWorkflowSpacing: 2 small ledger queries/day;
  watched by schedule-heartbeat). An unread source prints "Could not check: <why>". GUARD:
  src/test/morningPage.test.ts (fixture repo state -> expected sections; 5 @mutate).
