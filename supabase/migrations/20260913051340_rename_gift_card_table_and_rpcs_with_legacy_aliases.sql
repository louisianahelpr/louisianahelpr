-- Rename the gift card table and RPCs to their real names; keep the old names as aliases.
--
...
-- policy "PIF credits are party-only", eleven index/constraint names, and a
-- purge_user_data result key.
--
-- The rename cannot be a clean break. The App Store build (v1.0.x) bundles a
-- client that reads pif_credits directly, and edge functions deployed before
-- functions-deploy.yml finishes still read/write pif_credits and call the old
-- RPCs. So each renamed object leaves an alias under its old name:
--
--   * public.pif_credits           -> a security_invoker view over gift_cards.
--                                     A single-table SELECT * view is
--                                     auto-updatable, so the old service-role
--                                     writers keep working; security_invoker
--                                     means the base table's RLS policy still
--                                     decides what anon/authenticated can see.
--   * public.redeem_pif_credit     -> a SQL wrapper forwarding to redeem_gift_card.
--   * public.restore_pif_credit_for_job -> a SQL wrapper forwarding to
--                                     restore_gift_card_for_job (same defaults).
--
-- Grants are kept identical to the originals as read live from prod on
-- 2026-09-12: the table/view is anon=rxm, authenticated=rxm, service_role=all;
-- both RPCs are EXECUTE for service_role only. Every revoke names its roles
-- (FROM PUBLIC, anon, authenticated), because FROM PUBLIC alone leaves
-- Supabase's explicit per-role grants in place.
--
-- Data: gift_cards held 0 rows on prod when this was written and has no enum
...
-- a table; view only if absent; renames loop over whatever old names remain;
-- functions are CREATE OR REPLACE). Run 3x against PGlite before commit.
...
-- ── 2. Alias view under the old table name ──────────────────────────────
DO $$
BEGIN
  IF to_regclass('public.pif_credits') IS NULL AND to_regclass('public.gift_cards') IS NOT NULL THEN
    EXECUTE 'CREATE VIEW public.pif_credits WITH (security_invoker = true) AS SELECT * FROM public.gift_cards';
  END IF;
END $$;

COMMENT ON VIEW public.pif_credits IS 'LEGACY ALIAS (2026-09-12): drop once the minimum supported app version no longer uses it. App Store v1.0.x and mid-deploy web/edge code still use this name. Tracked in docs/OPEN.md, "Gift card legacy aliases". Forwards to public.gift_cards.';

REVOKE ALL ON public.pif_credits FROM PUBLIC, anon, authenticated, service_role;
GRANT SELECT, REFERENCES, MAINTAIN ON public.pif_credits TO anon, authenticated;
GRANT ALL ON public.pif_credits TO service_role;
...
-- ── 4. Old RPC names become thin forwarding wrappers ────────────────────
-- CREATE OR REPLACE keeps the signature, return type and parameter defaults
-- the deployed callers bind to by name.
CREATE OR REPLACE FUNCTION public.redeem_pif_credit(p_credit_id uuid, p_job_id uuid, p_user_id uuid)
 RETURNS jsonb
 LANGUAGE sql
 SET search_path TO 'public'
AS $function$
  SELECT public.redeem_gift_card(p_credit_id, p_job_id, p_user_id)
$function$;

COMMENT ON FUNCTION public.redeem_pif_credit(uuid, uuid, uuid) IS 'LEGACY ALIAS (2026-09-12): drop once the minimum supported app version no longer uses it. App Store v1.0.x and mid-deploy web/edge code still use this name. Tracked in docs/OPEN.md, "Gift card legacy aliases". Forwards to public.redeem_gift_card.';
REVOKE ALL ON FUNCTION public.redeem_pif_credit(uuid, uuid, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.redeem_pif_credit(uuid, uuid, uuid) TO service_role;

CREATE OR REPLACE FUNCTION public.restore_pif_credit_for_job(p_job_id uuid, p_share_bps integer DEFAULT 10000, p_dry_run boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE sql
 SET search_path TO 'public'
AS $function$
  SELECT public.restore_gift_card_for_job(p_job_id, p_share_bps, p_dry_run)
$function$;

COMMENT ON FUNCTION public.restore_pif_credit_for_job(uuid, integer, boolean) IS 'LEGACY ALIAS (2026-09-12): drop once the minimum supported app version no longer uses it. App Store v1.0.x and mid-deploy web/edge code still use this name. Tracked in docs/OPEN.md, "Gift card legacy aliases". Forwards to public.restore_gift_card_for_job.';
REVOKE ALL ON FUNCTION public.restore_pif_credit_for_job(uuid, integer, boolean) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.restore_pif_credit_for_job(uuid, integer, boolean) TO service_role;
