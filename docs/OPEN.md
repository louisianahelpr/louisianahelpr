...
- **Queue (this file):** 159 done, 34 partly done (fixed, protection pending), 115 open. Source of truth for work.
...
**Queue: 308 items — 163 done, 34 partly done (fixed, protection pending), 111 open.**
...
- [x] **Q272 DONE 2026-09-23 (branch cloud/q272-press): the press restores profiles from ONE baseline per RUN.** A new `snapshot` job (press-every-control.mjs --snapshot-only, PROFILE_SNAPSHOT_OUT) records every shared account's profile before any shard starts; shards of a sharded run take no baseline and restore nothing (shardOwnsProfileRestore / shardProfileBaseline, scripts/audit/pressProdSafety.mjs); the `cleanup` job restores from that one file after every shard (PROFILE_SNAPSHOT). A persona with no baseline, or whose row cannot be read, is residue (red), never a silent skip. Also found and fixed on the way: the cleanup job could never mint the incomplete account (no .env, no password secret), so EVERY run's clean-up failed "personas not minted: incomplete" before the service-role sweep (job 107152562721); the service-role .env step is now one composite action (.github/actions/press-service-env) used by snapshot, press and cleanup. NOT yet measured on a live run (lands with the lead). GUARD: src/test/pressProfileRestoreOncePerRun.test.ts — replays run 35837735324's interleave against a fake PostgREST with the real functions (no PATCH may write the flipped value) and pins the workflow wiring; 5 @mutate, all KILLED by hand (reverting to per-shard baselines writes senior_mode=true back). Was: **Q272 The press sweep's end-of-run profile "restore" races its own shards (found by Q200, 2026-09-23).** press-every-control snapshots each shared account's profile once PER SHARD at start and PATCHes every differing column back at that shard's end (cleanup() in scripts/audit/pressProdSafety.mjs). Four shards run concurrently, so a shard that snapshotted after another shard's press (or after a cancelled run that never restored) writes the wrong value back: run 35837735324 shard 2 at 11:02Z "restored senior_mode, available_until" to the flipped value. Q200 stops toggles reaching the profile, but any other profile column a press can change is still restored from a racy snapshot. Fix: one snapshot per RUN (taken before any shard starts, e.g. a preflight job artifact) and one restore after all shards finish; or restore only columns this shard itself changed. Guard: a test that two interleaved shard snapshots cannot restore a value neither started from.
...
- [ ] **Q214 BulkDismissBar (My Posts selection mode) floats at a hand-typed `calc(env(safe-area-inset-bottom) + 80px)` (found by Q191, 2026-09-23).** Every other bottom clearance reads the shared `--bottom-nav-h` / `safe-nav` token (96px dock default, 0 with no dock) and `--safe-area-bottom`, not env() directly. Not measured whether 80px clears the dock on every device; measure, then move it onto the token.
...
- [ ] **Q247 /pets at desktop shows two "Add a Pet" buttons when the user has no pets. (Q165 carry-over, 2026-09-23.)** Evidence: src/pages/PetProfiles.tsx:172-180 (title-row button, always rendered at lg) + :283-291 (empty-state button). From: docs/archive/VISUAL-2026-08-24.md.
...
- [ ] **Q250 Unused pending-credential count query runs on every public profile view (REPORT, don't remove). (Q165 carry-over, 2026-09-23.)** Evidence: src/pages/userProfile/useUserProfileData.ts:920-957, returned as hasSubmittedCredentials at :1100 with 0 .tsx consumers. From: docs/audit/visual-notes-2026-09-14.md (VN-13).
- [ ] **Q251 The state matrix still emits the retired pending_approval cells as auto-reachable. (Q165 carry-over, 2026-09-23.)** Evidence: e2e/happy-path/state-matrix/stateMatrix.ts:449-455. Needs live check: jobs with status 'pending_approval' = 0, then relabel unreachable. From: docs/archive/STATE_MATRIX.md + docs/archive/IOS_COVERAGE.md.
...
- [x] **Q293 DONE 2026-09-23 (branch cloud/q272-press): a cancelled scheduled prod-load run is reported red.** Measured: e2e-journeys scheduled run 35705405032 (2026-09-22T08:32:55Z) conclusion `cancelled` with 0 jobs — nothing ran, nothing was filed (1 such run in the 100 most recent scheduled runs, 09-19..09-23). Job-level cancels were already covered (every prod-load notify runs on always() and compares needs with == 'success'); a WORKFLOW-level pending cancel runs no job at all, so schedule-heartbeat.yml now runs scripts/ci/cancelled-prod-load-runs.mjs daily: every workflow whose workflow-level group can be 'prod-load' (derived from the files; 19 on 2026-09-23), every scheduled run cancelled in the last 8 days, counted as stalled (schedule-stalled issue + red heartbeat; exit 2 when runs cannot be read). Expect the first heartbeat after landing to go red on run 35705405032 until 2026-09-30 — that is the report working. Not done: re-running a missed month automatically (the privacy journey still needs a human re-dispatch). GUARD: src/test/prodLoadCancelledIsRed.test.ts (the window rule, the heartbeat wiring, and the class check over every prod-load notify: always() + every need == 'success'); 6 @mutate, all KILLED by hand. Was: **Q293 A scheduled prod-load run can be cancelled with nothing reported (found by the Q70 review, 2026-09-23).** GitHub keeps one PENDING run per concurrency group; a third run entering prod-load cancels the waiting one at workflow level, so no job (not even notify) runs and no nightly-red issue is filed. For the monthly privacy journey that is a missed month. schedule-heartbeat.yml derives each workflow's cadence from its cron (weekly here, because of the first-7-days gate), so it cannot tell that the one DUE week was cancelled. Fix: have the privacy journey's green notify record the month it ran (e.g. the nightly-red issue sync, or a dated marker), and alarm when the current month has none after day 8. Guard: a test on that check.
...
- [x] **Q295 DONE 2026-09-23 (branch cloud/q272-press): REAL DEFECT — an opened data: document opens nothing.** Measured in Chromium (Playwright): `window.open(<data:image/png>, "_blank", "noopener")` and a target=_blank link to one open NO page and log nothing (browsers refuse top-level data: navigation); the same bytes as a blob: URL open and render. At run 35837735324 (08:32-11:03Z) the seeded licence's document was still a data: PNG (Q130 moved the seed to a storage object later that day), and safeDocumentUrl allows data:image, so SignedOpenLink's non-storage branch was a dead button. The popup-timing and activation theories were measured false (a noopener popup after a 50ms/600ms/6s await is still seen by ctx.on("page")). Fix: openableDocumentUrl() (src/lib/storagePath.ts) re-serves a data: image as a blob: of the same bytes; used by SignedOpenLink, CredentialsTab.openDoc and every target=_blank document link (8 files); <img src> unchanged. Security guards navigationSinksAreClassified / storagePathRenderIsAllowlisted accept it (it returns only safeDocumentUrl output or its blob:) — wants an lh-appsec REVIEW-ONLY pass. Not verified: native WKWebView opening a blob: in a new window. GUARD: src/test/documentLinksOpenInANewTab.test.ts (4 @mutate, all KILLED; the first re-plants the original SignedOpenLink line). Was: **Q295 /admin?view=credentials "Open" scored "no observable change (focus moved, nothing else)" (press run 35837735324, found by Q128).** SignedOpenLink (AdminCredentialQueue.tsx) awaits createSignedUrl and then `window.open(…, "_blank", "noopener")`; the press listens for new pages on the context, so either the popup landed after the after-snapshot, the click lost user activation across the await, or the signed URL failed without a toast. Not measured (needs a live admin session). Guard to name when fixed: the press run's credentials row green, or a unit test on SignedOpenLink's outcome.
- [x] **Q296 DONE 2026-09-23 (branch cloud/q272-press): the press sends nothing to the prod Sentry project.** Harness-side (Q275 owns src/lib/sentry.ts; it still has no webdriver check on main): every press context answers Sentry ingest itself with a 200 (answerSentryLocally, scripts/audit/pressFailureClass.mjs; measured in Chromium: the envelope POST to the app's own DSN host is fulfilled locally), and the local-preview action builds with VITE_SENTRY_ENV=ci-local-preview so any CI suite's event that does reach Sentry is not tagged production. Also corrected: local-preview's comment claimed leaving VITE_SENTRY_DSN unset kept test builds out of Sentry — false, src/lib/sentry.ts falls back to a hardcoded DSN. Other suites that use local-preview still SEND events (tagged ci-local-preview): that is Q275's decision. GUARD: src/test/pressSendsNoSentryEvents.test.ts (5 @mutate, all KILLED). Was: **Q296 The press sweep's own Sentry events are rate-limited by Sentry (429 on envelope/, 6 presses in run 35837735324; found by Q128).** The press builds the app with the prod VITE_* env, so every sweep error reaches the PROD Sentry project and spends its quota until Sentry answers 429 — which also means a real user's event could be dropped in that window. Now listed in coverage.md ("Request failures that are not the app's", telemetry count, ::warning), no longer counted against the control. Decide: tag or drop sweep events (environment "press"/beforeSend on a test flag), or route them to a separate project. Guard to name when fixed: a test that the press build's Sentry env is not production.
