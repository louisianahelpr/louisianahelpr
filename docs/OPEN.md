...
- OPEN (LOW): `src/pages/activity/activityActions/useLifecycleHandlers.ts` keys the completion moment off `bothDone`, so a duplicate release (now `alreadyReleased: true, bothDone: true`) replays the confetti + tip prompt. Fix: `if (data?.alreadyReleased || data?.alreadyConfirmed) { await refresh(); return; }` before the `bothDone` branch. Held out of the fix commit because the pre-push changed-route a11y sweep is red on /my-jobs (next line).
- OPEN: pre-push a11y-prod sweep red on `/my-jobs` (helper, phone-light): axe `aria-command-name` serious, 2 nodes `.tracking-helper-pin` (a clickable map pin with no accessible name). Pre-existing, blocks any push that touches an activity route.
- OPEN: 21 other edge `jobs` lifecycle writes with no status predicate are baselined in `scripts/race-class-baseline.json` as "not yet re-audited" — each needs a prod race or a reason.
...
- [ ] REPORT (not done, from the types.ts regeneration 2026-09-14): ~15 `(supabase.rpc as any)` / `as never` casts across `src/` carry comments saying "drop the cast once types.ts is regenerated". All 10 single-line ones now resolve in the fresh types (`admin_reverse_violation`, `clear_available_now`, `clear_thread_mute`, `get_muted_threads`, `get_user_last_active`, `record_profile_view`, `respond_to_review`, `set_available_now`, `set_thread_snooze`, `toggle_thread_mute`), plus multi-line ones in `DisputeDialog.tsx`, `AdminDisputes.tsx`, `PostedJobActions.tsx`, `AdminPayoutBatches.tsx`, `AdminSupport.tsx`. Left alone deliberately (types-only commit); they are now removable and each removal is real type coverage regained.
...
- [ ] Write contract snapshot refresh 34745765776 (failed 07:39Z = the moment prod DB started timing out; SQL has no dropped names — re-run when DB answers): `supabase db query -f scripts/audit/write-contract.sql` errors — likely references a dropped/renamed object (the old gift table, dead functions/columns). Fix the SQL, re-run, confirm snapshot current.
...
- [ ] OWNER, blocks the Slack half of both alerts: add repo secret `SLACK_WEBHOOK_URL` (Slack → Incoming Webhooks → repo Settings → Secrets and variables → Actions). Until then, an actual outage or usage warning still opens its issue but the notify step FAILS LOUDLY with the instruction — it is never skipped silently.
- [x] Supabase usage alert DONE (.github/workflows/supabase-usage.yml + scripts/supabase-usage-check.mjs), Sat 01:17 UTC. Management API (SUPABASE_ACCESS_TOKEN + SUPABASE_PROJECT_REF), never SQL — the control plane answers without touching the nano instance. Warns at 70% of 500 MB DB / 1 GB storage by opening `supabase-usage`. The API's metric set is not a contract, so the script probes several endpoints and the report names, per metric, whether it got a number or the API does not expose it (disk IO/CPU are served by the project's own Prometheus endpoint, service-role auth, not the control plane).
