-- RE-APPLY of 20260831232513_address_only_when_offered.sql.
--
-- WHY THIS FILE EXISTS. The 2026-09-01T03:24 deploy failed on the NEXT
-- migration in the batch (20260831232514 carried `.{0,600}?`, and Postgres
-- caps {m,n} repetition at 255 — SQLSTATE 2201B). The batch aborted, but
-- 20260831232513 had already been recorded in schema_migrations without its
-- DDL surviving. The follow-up deploy therefore SKIPPED it: the run log starts
-- at ...232514, and `user_may_see_job_address` still returned PGRST202
-- afterwards while functions from later migrations answered normally.
--
-- The documented remedy is `supabase migration repair --status reverted`, which
-- needs a CLI session against prod. This repo's CLI ref points at STAGING and
-- there is no prod DB password here, so repairing the ledger by hand was not
-- available. Re-applying under a fresh timestamp reaches prod through the one
-- sanctioned path (merge -> db-deploy.yml) and leaves the ledger honest: both
-- rows are real, and this one is the one that actually ran.
--
-- Safe to replay: the original was written guarded (DROP POLICY IF EXISTS,
-- CREATE OR REPLACE FUNCTION, CREATE POLICY guarded by a DROP). Running it a
-- second time against a database where it already applied is a no-op.
--
-- WHAT IT CLOSES, restated because it is still live until this lands: a helper
