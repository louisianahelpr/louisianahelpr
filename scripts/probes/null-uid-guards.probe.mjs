// Probe: 20260915051905 (a NULL auth.uid() is not the service role) and the
...
const MIG = read("../../supabase/migrations/20260915051905_null_uid_is_not_server.sql");
...
{
  console.log("\n== 2. AFTER (20260915051905 applied 3x)");
...
{
  console.log("\n== 5. Replay-safety: no tables, no app_role, 3x");
  const db = new PGlite();
  let ok = true;
  try {
    await db.exec(`${ROLES}
