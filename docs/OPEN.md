...
- [ ] OWNER DECISION: the 5%-of-a-bucket cap trips on ONE orphan in any bucket under 20 objects (id-documents has 1, avatars 4 today), so a single leaked file makes the sweep alert CRITICAL and delete nothing. Keep as specified, or add a minimum count (e.g. more than 3 files)?
- [ ] OWNER DECISION: `accountPurge` still REFUSES the account deletion when identity storage removal fails (deliberate, so an ID scan is never orphaned). The new "removal never blocks deletion" rule was applied to the new job-media and message paths only. With the weekly sweep now a net, should identity-storage failure also stop blocking?
- [ ] GAP (report only): voice notes can neither be uploaded nor deleted by clients. `voice-notes/<jobId>/<senderId>/…` puts the jobId at `storage.foldername(name)[2]`, but the "message-attachments: sender uploads/deletes own" policies compare [2] to `auth.uid()`. The policies need a voice-notes branch on [3]. Authz review also flags: if a client can choose `jobs.id`, a job id equal to the poster's uid would satisfy [2]. Needs a migration + lh-authz-rls.
- [ ] GAP (report only): the `message-attachments` SELECT policy trusts `messages.attachment_url`; if message INSERT does not constrain that column, a sender could name any object and read it. Verify live.
