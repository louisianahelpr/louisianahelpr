...
- **Queue (this file):** 129 done, 18 partly done (fixed, protection pending), 130 open. Source of truth for work.
...
**Queue: 277 items — 129 done, 18 partly done (fixed, protection pending), 130 open.**
...
- [~] **Q269 A retried apply that already landed is told it failed, then "already applied" (found by Q68, code read 2026-09-23).** A lost apply_to_job response shows "Couldn't send your application through" (src/pages/dashboard/useApplyFlow.ts:257); the retry is refused by the RPC and shows "You've already applied to this job." (applyErrorCopy.ts:29). No duplicate row (UNIQUE(job_id, helper_id)), but the user is told twice that something went wrong when it didn't. Fix: map "Already applied" after a failed attempt to the success state. Proof: slow-network apply·drop records a `finding` annotation when the copy shows; turn it into an assertion with the fix.
  STATUS 2026-09-23 (cloud/q267-q270-retries): FIXED IN CODE, [~]. Cause: the retry's "Already applied to this job" refusal was shown as an error. Fix: useApplyFlow remembers jobs whose last apply failed without a server refusal (outcomeUnknownJobIds); on the retry an already-applied refusal (isAlreadyAppliedRefusal, applyErrorCopy.ts) for such a job is success ("Application sent!", attachments patched); a genuine already-applied with no unknown attempt still shows the refusal. No DB change (UNIQUE(job_id, helper_id) already held). GUARD: src/pages/dashboard/useApplyFlow.lostResponse.test.tsx (2 @mutate lines; red on the pre-fix file: no success toast) + the slow-network apply·drop finding is now an assertion. NOT YET SEEN LIVE: slow-network.yml has not run on it (no prod access here).
