...
- **Queue (this file):** 137 done, 13 partly done (fixed, protection pending), 134 open. Source of truth for work.
...
- **Ops alert ledger:** 27 open (2 critical, 20 error, 5 warning), 0 verifying — `node scripts/ops-alert-ledger.mjs list` · /admin?view=health. _(2026-09-23T18:05Z)_
- **nightly-red issues:** 13 open — `gh issue list -l nightly-red`. _(2026-09-23T18:03Z)_
- **Workflows on main:** 12 red, 8 stale, 4 unknown, 30 green of 54 — [SCOREBOARD](SCOREBOARD.md). _(2026-09-23T18:03Z)_
- **Remote branches:** 35 carry patches not on main, 0 fully merged, of 37 (Q79). _(2026-09-23T18:03Z)_
<!-- /live -->
...
**Queue: 284 items — 137 done, 13 partly done (fixed, protection pending), 134 open.**
...
- [~] **Q205 Q193 follow-ups (2026-09-23). (a) and (c) DONE 2026-09-23 (cloud/q205-q197); (b) and (d) OPEN.** (a) DONE: allowPending/allowUnapproved deleted from ProtectedRoute's props, the 6 App.tsx routes that passed them, ProtectedRouteGates/lapsedSuspension tests, and stale comments (LegalTab, useDeleteAccount, Profile, AccountBanned, CompleteProfile, bannedAccountDeletion, audit-capture, probe-state-matrix); emailGateEveryProtectedRoute.test.tsx now renders every route-table entry with the exact props it is mounted with and fails on any bare per-route prop. GUARD: src/test/retiredApprovalReads.test.ts + emailGateEveryProtectedRoute.test.tsx. Was: Strip the ignored allowPending/allowUnapproved props from src/App.tsx once the Q194 lane has landed (ProtectedRoute accepts and ignores them). (b) Decide whether to drop profiles.approval_status: read by 10 SQL readers (`= 'approved'`), 5 trigger functions, the profiles INSERT RLS policy (WITH CHECK 'pending'), cleanup-abandoned-accounts ('pending' = complete-signup never ran), engagement-automations (approval-resend series + admin digest pending count) and several edge fns; a data-model change with its own review. (c) DONE 2026-09-23: complete-signup's denied branches (unauth-path refusal, isResubmission/denied_resubmission, the UPDATE's not-denied .or()), admin-update-email's auto-free of a denied holder's address (both conflict paths now 409; its EF-5 leak went with it, KNOWN_LEAK_FILES and the seed-producer classification lowered), migration 20260923165543 restating enforce_referral_credit_eligibility from its NEWEST definition (20260901035252) with only the denied branch removed (PGlite 3x: banned/temp_banned/no-profile/self still refused + flagged, approved/pending credited, pg_get_functiondef diff = the denied lines only), adminAnalyticsHelpers pending/denied counts + AdminAnalytics "Pending Approval"/"Denied" rows + the UsersDrillDown status filter removed. GUARD: src/test/retiredApprovalReads.test.ts (source tree + newest SQL definitions; red on 9a0582ecf 3/3: 19 prop refs in 6 files, 8 denied reads in 4 files, 1 SQL fn; 5 @mutate red). NOT verified live (no prod access in the lane): db-deploy + pg_get_functiondef after landing. Was: Dead 'denied' reads now unreachable: complete-signup (resubmission branch), admin-update-email, enforce_referral_credit_eligibility, adminAnalyticsHelpers + AdminAnalytics "Pending Approval"/"Denied" rows + UsersDrillDown filter (labels now misleading). (d) MEASURED on prod 2026-09-23 (lh-trust-safety token-swap probe; fix is Q281): a ban is enforced ONLY by the enforce_ban_gate BEFORE trigger on 8 table/op pairs (INSERT on jobs, applications, messages, message_reactions, reviews, job_revisions; UPDATE on jobs, reviews) plus the banned-profile bio/full_name pin; nothing else checks ban_status (no RLS policy, 0 of 35 non-admin writing RPCs call is_caller_banned, 0 storage policies). Method: fixture users A+B minted via the admin API (never a shared account), A banned exactly as apply_consequence_ladder's suspend rung does (ban_status='temp_banned', auto_suspended_until=now()+7d, app.trusted_ladder_write), A's JWT + anon key against PostgREST/Storage; gated-table rows proven by a rolled-back impersonation DO block where an earlier BEFORE trigger (job state, FK) fired first. auth.users.banned_until stayed NULL: refresh_token grant after the ban = 200 NEW JWT, fresh password sign-in = 200. REFUSED account_restricted (42501): INSERT jobs, applications, messages, message_reactions, reviews, job_revisions; UPDATE jobs; respond_to_review (UPDATE reviews). SUCCEEDED while banned: UPDATE own sent message content (rows=1, text replaced) and DELETE own message; UPDATE + DELETE own pending application; INSERT reports (201), user_blocks (201, and DELETE 200), favorite_helpers, saved_jobs, saved_searches, pet_profiles, thread_pins, thread_archives, push_tokens (a banned account can register for push), helper_availability, str_calendar_connections, referral_codes (a banned account can mint a referral code), nps_responses, job_checkins, legal_acceptances, login_history, helper_w9_records; PATCH notification_preferences; PATCH profiles phone/location/avatar_url (200, written; bio/full_name pinned back as designed); Storage upload 200 to avatars, user-documents, proof-photos, application-attachments, job-photos (message-attachments refused by its path rule, not the ban); RPCs set_available_now (200, a banned account can advertise itself as Available now), clear_available_now, save_weekly_availability, record_profile_view, record_job_view, toggle_thread_mute, set_thread_snooze, clear_thread_mute, rpc_record_application_attempt, block_user_and_settle. Not a ban refusal (another rule fired first): analytics_events, error_logs, profile_views, job_views direct INSERT (RLS on other columns), apply_to_job (job unfunded; its INSERT reaches the applications gate). Not exercised: money/lifecycle RPCs (accept_application, poster_cancel_job, helper_* , dispute RPCs) write jobs/applications and so hit the jobs UPDATE gate by static read only; edge functions not probed. Restore evidence: A unbanned (ban_status active, 0 retained_bans rows in the last hour), 5 storage objects deleted, 24 tables' fixture rows deleted, both auth users deleted (admin API 200), a uuid-column scan of public+storage+auth for the 4 fixture ids returned 0 rows; evidence ~/.lh-shots/q205d/ (probe.mjs, results.json). Side finding filed as Q282.
