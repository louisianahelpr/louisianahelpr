...
- **Queue (this file):** 137 done, 13 partly done (fixed, protection pending), 136 open. Source of truth for work.
...
**Queue: 286 items — 137 done, 13 partly done (fixed, protection pending), 136 open.**
...
- [ ] **Q279 The Profile-tab parsers skip `wrapped` (found by Q179, 2026-09-23).** scripts/audit-capture.mjs (PROFILE_TABS) parses TAB_TITLES with /^\s*(\w+):\s*"/, and `wrapped:` is a template literal (`Your ${...}`), so the capture sweep (and the Q179 walk, which copied it) silently skipped /profile?tab=wrapped: 23 parsed of 24. Accept any quote ("'`), and add a check that the parsed count equals Object.keys(TAB_TITLES).
...
- [ ] **Q289 Re-consent copy names a Privacy change that did not happen (found by Q210(d), 2026-09-23).** FIXED IN Q210(d): TermsReconsentDialog.tsx and CompleteProfile.tsx wrote `privacy_version: LATEST_TERMS_VERSION` into legal_acceptances, harmless only while the two were equal; they now write the new client LATEST_PRIVACY_VERSION ("Jun 2026", mirrored from LEGAL_PRIVACY_VERSION), guarded by src/test/termsReconsentOnBump.test.tsx. STILL OPEN: the dialog says "a material update to our Terms and Privacy Policy" though only the Terms changed on 2026-09-23; the copy should name what changed (owner to word it). Guard to extend: src/test/termsReconsentOnBump.test.tsx.
