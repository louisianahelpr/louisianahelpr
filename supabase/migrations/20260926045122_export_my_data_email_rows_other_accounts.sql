-- Q409 follow-up (from #1838, lh-authz-rls review): two email-matched sections
-- could still export a row that belongs to ANOTHER, current account.
--
--   notification_logs: the address branch matched rows already owned by a
--   different user_id (an address the other account used, logged to them).
--   It now matches only rows with no owner (user_id IS NULL).
--
--   gift_cards: the address branch matched a gift another account had already
--   claimed (recipient_id set to them). It now matches only unclaimed gifts
--   (recipient_id IS NULL); a gift this account claimed still matches through
--   recipient_id = v_uid. Still no date bound: a gift predates its owner's account.
--
-- Everything else is 20260926034548 verbatim. Q408's rate-limit half of #1838
-- is moot: since 20260926034548 only service_role can execute this, so the
-- edge function's per-user limiter is the only door.
