...
/**
 * `!min-w-max`, and the `!` is load-bearing — a plain `min-w-max` is INERT on
 * every button in this app.
 *
 * `index.css:1581` carries the HIG tap-target floor:
 *
 *     button:not([role="checkbox"]):not([role="radio"]):not([role="switch"]),
 *     [role="button"], input[type=checkbox], input[type=radio]
 *       { min-height: 44px; min-width: 44px; }
 *
 * That selector is (0,3,1). `.min-w-max` is (0,1,0). Specificity decides before
 * source order ever gets a vote, so `min-width: 44px` wins on every `<button>`
 * and `.min-w-max{min-width:max-content}` — which IS emitted into
 * `dist/assets/*.css`, verified by grep — never applies to one.
 *
 * MEASURED, built bundle, 2026-09-03: with the plain class the two actions stay
 * pinned at 121px each at 320 and never wrap, so "Boost — Free This Month" (188px
 * of label into 89px of text room) clipped exactly as it did before this file was
 * rewritten. Forcing `min-width: max-content` inline on the same DOM flipped it
 * to stacked, commit-above-dismiss, both actions spanning [33,287] against a card
 * right edge of 304 — the intended behaviour, with 17px to spare at the narrowest
 * supported width. The design was right; one class was being thrown away.
 *
 * This is the third member of a family this repo keeps meeting: a class that is
 * present on the element, correct in the source, emitted into the stylesheet, and
 * still does nothing (`data-[state=checked]:btn-grad-primary` compiling to
 * nothing; an inline `background` shorthand resetting `background-image`). A
 * class-presence assertion passes on all three. `popupFooterFit.spec.ts` reads
 * the COMPUTED `min-width` for that reason.
 *
 * `min-height: 44px` from the same global rule is untouched and still applies, so
 * the tap target is unaffected; `max-content` over `px-4`/`px-6` is comfortably
 * wider than 44px for every label in the app anyway.
 */
...
/** See the `!min-w-max` note above POPUP_SECONDARY_CLS — same reason, same `!`. */
