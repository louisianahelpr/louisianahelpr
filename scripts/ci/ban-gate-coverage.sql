...
--                        Owner decision (Q298, 2026-09-23): a banned account
...
--                        enforce_ban_gate no longer lets a ban started in this
--                        transaction finish it, or the trigger that marks such a
--                        ban is gone. Without it the 3rd reliability strike
--                        inside helper_cancel_booking rolls itself back (the
...
WITH table_exempt(tbl, op, why) AS (
  ...
  ('push_tokens', 'INSERT', 'a banned account still signs in (Q298) and must get account notices such as Restriction lifted; app boot registers the device'),
...
carveout AS (
  SELECT 'gate:no-same-txn-carveout'::text AS rule, 'public.enforce_ban_gate'::text AS object, 'app.ban_started_in_txn'::text AS detail
   WHERE NOT EXISTS (
           SELECT 1 FROM pg_proc p
            WHERE p.oid = to_regprocedure('public.enforce_ban_gate()')
              AND p.prosrc ILIKE '%app.ban_started_in_txn%')
      OR NOT EXISTS (
           SELECT 1 FROM pg_trigger tg JOIN pg_proc p ON p.oid = tg.tgfoid
            WHERE tg.tgrelid = to_regclass('public.profiles')
              ...
              AND (tg.tgtype & 16) = 16
              AND p.prosrc ILIKE '%set_config(''app.ban_started_in_txn''%')
)
