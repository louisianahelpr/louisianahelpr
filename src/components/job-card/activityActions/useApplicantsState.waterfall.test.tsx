...
// @mutate src/components/job-card/activityActions/useApplicantsState.ts | const { data: apps, error: appsError } = await supabase.from("applications") | await supabase.from("user_blocks").select("*").eq("blocker_id", jobId);\n    const { data: apps, error: appsError } = await supabase.from("applications")
