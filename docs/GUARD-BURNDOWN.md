...
|---|---|---|---|---|
| **`src/test/*.test.ts*`** | 401 | **401 — COMPLETE** | 0 | **0** |
| **`src/test/edge/` (money)** | 66 | **66 — COMPLETE** | 0 | **0** |
| **colocated beside components** | 358 | **358 — COMPLETE** | 0 | **0** |
| **Playwright `e2e/`** | 67 | **58 — COMPLETE** | 9 | **0** |
| **total** | **892** | **883 (99%)** | **9** | **0** |

`npm run vacuity` prints the same three numbers on every run:

```
registration: 883/892 guards register a mutation (9 exempt with a reason, 0 grandfathered)
```
