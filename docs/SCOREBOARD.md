...
| open work | audit bus findings (open / launch blockers) | **FAIL** | 319 | 42 | — | 429 | HEAD (diffed every push) | [ROLLUP.md](audit/launch-2026-09/ROLLUP.md) · `node scripts/audit-bus.mjs list --blockers` | 42 open, 3 open launch blockers; 319 fixed, 14 retracted, 31 duplicate, 1 wontfix, 22 obsolete |
| guards | vacuity: guards proven able to fail / exempt / owed | **PASS** | 975 | 0 | 9 exempt | 984 | HEAD (diffed every push) | [GUARD-BURNDOWN.md](GUARD-BURNDOWN.md) · `npm run vacuity` | registered @mutate per guard; whether each mutation is KILLED is the full-sweep row below |
