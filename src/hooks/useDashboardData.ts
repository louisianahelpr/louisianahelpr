...
export function useDashboardData() {
  ...
  const {
    ...
    queryFn: async ({ pageParam }): Promise<JobsPage> => {
      ...
      //
      // `profile` is from the outer useDashboardData scope (current user's
      // profile). Default to free-tier when profile is still loading —
      // safer than accidentally granting early access.
      ...
      // For Pro/Elite: no cutoff (pass undefined / skip filter).
