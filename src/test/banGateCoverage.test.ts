 ...
 * the new, 10 planted defects each RED).
 *
...
// @mutate supabase/migrations/20260923180950_ban_enforcement_everywhere.sql | 'messages:UPDATE', 'messages:DELETE', | 'messages:UPDATE',
...
// @mutate supabase/migrations/20260923180950_ban_enforcement_everywhere.sql | IF TG_OP = 'DELETE' THEN | IF false THEN
// @mutate supabase/migrations/20260923180950_ban_enforcement_everywhere.sql |      AND current_setting('app.ban_started_in_txn', true) IS DISTINCT FROM auth.uid()::text THEN\n    RAISE EXCEPTION 'account_restricted' |  THEN\n    RAISE EXCEPTION 'account_restricted'
...
describe("Q281 ban gate: every client write path is gated or exempt with a reason", () => {
  ...
  it("the migration adds the Q281 pairs named in docs/OPEN.md", () => {
    ...
    // Q298: a banned account still signs in, so it keeps registering its device.
  ...
  it("nothing sets an Auth-level ban (owner, Q298: a banned account still signs in to delete itself)", () => {
