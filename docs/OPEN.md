...
- **Queue (this file):** 110 done, 12 partly done (fixed, protection pending), 87 open. Source of truth for work.
...
**Queue: 209 items — 110 done, 12 partly done (fixed, protection pending), 87 open.**
...
- [ ] **Q139 45 notification producers do not carry their subject, so the Q137 seed boundary cannot judge them (found by Q137, 2026-09-23).** Listed exactly in KNOWN_GAP in src/test/seedNeverNotifiesReal.test.ts (two-way): 23 edge sites (arrival-confirm-reminder, auto-expire-jobs x2, charge-recurring-visits, check-pro-subscription, create-payment, execute-dispute-split, payment-confirm-reminder, process-scheduled-payouts x2, release-payout x3, review-nag-cron, stalled-completion-reminder, stripe-idv-webhook admin alert, chargeDisputeClosed x2, chargeDisputeCreated, checkoutSessionCompleted tip + gift credit, void-cancelled-payments x2) and 22 SQL sites (check_referral_bonus x4, track_revision_scope_creep x2, notify_poster_on_status_change, notify_helper_on_tip, notify_helper_on_direct_offer, notify_helper_application_viewed, respond_to_direct_offer, expire_unanswered_offers x2, sweep_dayof_confirm_reminders x3, apply_job_denial_consequence, sweep_release_last_chance, helper_abort_job x2, apply_low_rating_flag, apply_consequence_ladder admin alert), plus the gift-card email (names the donor). Each is about a job or a counterpart but inserts no job_id and no job/actor link. Fix per site: add job_id (or a job/actor link) to the insert, then delete its KNOWN_GAP line (the guard fails until you do). Their risk is lower than Q137's fan-outs: most notify a PARTY to the job.
  - **STATUS (cloud lane, 2026-09-23, branch cloud/q139-notification-subjects, NOT on prod; the lead verifies and ticks).** KNOWN_GAP 45 -> 0 (two-way; src/test/seedNeverNotifiesReal.test.ts). Every one of the 45 now passes its subject: job_id, or for the member/actor cases `&user=<id>` in the link (stripe-idv-webhook admin alert -> /admin?view=people&user=; apply_low_rating_flag / apply_consequence_ladder admin alerts -> &user= on view=fraud / view=banreview, which those views do not read; check-pro-subscription referral and the gift credit -> &user= on /profile, which nothing there reads). The guard's judging was tightened (each SQL INSERT on its own statement, each TS insert on its own balanced literal) and that exposed 6 more producers it had counted as carrying only through a SIBLING insert's job link, fixed too: notify_on_payment_escrowed 'Payout released', open_dispute_as admin 'Job disputed', create-payment 'Job completed!' + 'Dispute resolved — payment released!', process-scheduled-payouts 'Payout account required' + 'Payout sent!'; plus Q202's new chargeDisputeClosed 'Chargeback WON — Helpr paid back' alert (landed on main mid-lane, caught by the guard). auto_restrict_repeat_violators #2/#4/#6 classified own-account. SQL: migration 20260923162545 restates 16 functions verbatim from their newest definitions with only job_id / &user= added (no later migration redefines them); PGlite src/test/pglite/notificationProducersCarrySubject.pglite.mjs (real Q137 boundary + real producers, applied 3x): ALL PASS (76: seed job -> real party gives 0 rows + a suppressed_seed log; real job gives 1 row, now with job_id; seed-to-seed unchanged); NEW_MIGRATION=skip (previous definitions): 57 FAILED. Zero rows are now expected: once a row carries its subject, the trigger's drop returns ZERO rows by design, so every producer that reads the count back asks the boundary before calling it a failure (new supabase/functions/_shared/seedBoundary.ts seedBoundaryDropsRow, asked only after a zero-row insert, so a real row costs nothing extra): arrival-confirm-reminder, stalled-completion-reminder, review-nag-cron, payment-confirm-reminder (marks the job, status suppressed_seed), charge-recurring-visits card-problem rows, check-pro-subscription, auto-resolve-disputes admin reminders (that one already recorded a defect for every seed job since Q137, its links carry &job=). New assertion (inventory-derived): a zero-row-checked producer that carries its subject must ask the boundary. Gift email (sent to an ADDRESS, outside both choke points): giftEmailCrossesSeedBoundary asks the boundary with the donor as actor; no account behind the address = a real person, so a seed donor is not emailed; fails closed with a critical Slack page. Red->green: vacuity 80/81 mutations killed in the changed scope; the 1 survivor was pre-existing (a mutation on a superseded ops_alert_condition) and is retargeted to 20260923133021 and shown red. Edge tests: all 65 src/test/edge files green (972), new cases in stalled-completion-reminder, payment-confirm-reminder, giftCardRefundRevocation. NOT RUN in this lane: typecheck:edge (deno.land is blocked in the container; app tsc -b passed) and anything on prod. For the lead: after db-deploy, pg_get_functiondef of the 16 functions must show the job_id / &user=; then one seed-job producer on prod against an owner account (expect 0 rows + suppressed_seed). HEADS-UP: the owner's own accounts stop getting these rows about seed jobs/members (the rule as specified). Needs an lh-silent-failure REVIEW-ONLY pass (zero-row branches in 7 cron/webhook producers). Also seen: src/test/noOrphanedStorageBuckets.test.ts registers no @mutate (vacuity fails on main for it, not this lane's).
...
- [ ] **Q213 check_referral_bonus sends "Your referral''s first posted job was completed" with a doubled quote (found by Q139, 2026-09-23; not changed there, Q139 restated the body verbatim).** 20260902014651 wrote the function inside `EXECUTE $fn$ ... $fn$`, where no quote unescaping happens, so the `''''` meant for one level of nesting reaches the message as two quotes. Not verified on prod (no prod access in that lane): check `pg_get_functiondef('public.check_referral_bonus()'::regprocedure)` and a real 'Referral bonus!' row. Fix: `''` in a new restatement. Guard: a source scan of notification copy literals for `''''` inside dollar-quoted bodies.
