...
- [ ] READY TO LAND (branch `fix-jobs-completion-columns`, no authed prod session in the routine — lead proves on prod and merges). Completion columns server-owned: the two PROVEN HIGH holes from the 2026-09-15 authz hole hunt (`docs/audit/holes-2026-09-15/authz-rls.md`, H-001 and H-002; branch `origin/holes-authz-rls`), same class as the dispute-marker fix (20260915033734). H-001: an assigned Helpr picks the VALUE of `jobs.helper_completed_at` (passes the completion gates, then PATCHes a backdated stamp), making a funded job instantly due for auto-release and erasing the poster's 24h review window. H-002: a poster PATCHes `jobs.status='completed'` directly (status is on neither poster-money-lock list and the transition matrix is actor-blind), stranding escrow out of every auto-release sweep and shutting the dispute door. FIX: migration `20260915055245_job_completion_columns_server_owned` — one non-SECURITY-DEFINER `BEFORE INSERT OR UPDATE OF status, helper_completed_at` trigger `zz_jobs_completion_server_owned` (modelled exactly on `enforce_dispute_markers_server_owned`: `current_user NOT IN ('authenticated','anon')` passes, admins exempt). A client write: (a) may not move `status` INTO 'completed'; (b) `helper_completed_at` is server-clamped to `now()` on the assigned Helpr's FIRST stamp (the app's raw-PATCH Done keeps working, value ignored), and every other change — a second write, a poster/offered Helpr set or CLEAR — is refused 42501. The app has NO completion RPC (Done is a raw PATCH, `JobTracking.tsx:1118`), so the finding-sanctioned clamp ("refused OR silently replaced with now()") is used rather than a hard 42501 on the Helpr's own first write, which would break Done. Closes KNOWN_OPEN `poster:helper_completed_at` / `offered:helper_completed_at` in the class guard. Service-role edge functions (auto-release-payment, create-payment, auto-resolve-disputes) and SECURITY DEFINER RPCs (rpc_decide_dispute, rpc_withdraw_dispute) run as service_role/postgres and are untouched. GUARD: `src/test/jobsStateColumnGuard.test.ts` (guard registered, two KNOWN_OPEN pairs removed, red-when-dropped + narrowed-copy + structural fail-tests) and `scripts/check-jobs-dynamic-writers.mjs` (second `--inject-fake` for `helper_completed_at`). PROOF: `scripts/probes/job-completion-columns.probe.mjs` (live-shaped PGlite + latest completion gates layered: 11 failing red on the pre-fix shape, green applied 3x, 10 broken copies each caught incl. the current_user-vs-auth.uid distinction via `rpc_withdraw_dispute` restoring a completed job, skip + loud paths). Local gate: typecheck green, the touched vitest files green. LEAD PROD PROBES (rolled back): as a helper on a funded in_progress job with arrival/photos done, PATCH `helper_completed_at = now()-25h` → accepted but stored value = server now() (backdating neutralized; NOT 42501, this is the Done path); a SECOND PATCH changing that stamp → 42501; as the poster, PATCH `helper_completed_at = NULL` → 42501; as the poster, PATCH `status='completed'` on a funded in_progress job → 42501, row unchanged. (found by lh-authz-rls hole hunt 2026-09-15.)
