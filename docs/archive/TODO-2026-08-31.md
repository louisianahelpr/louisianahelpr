...
- [x] **F-DISC-01** — close the legacy street-address leak. **Both shipped fixes are in; only the regression test is still open.** The leaky view is gone (`20260618120000_mask_open_jobs_rpc_drop_leaky_view.sql:64` — `DROP VIEW IF EXISTS public.open_jobs_safe`) and `get_ranked_open_jobs` masks `location` (`:53`, preserved through `20260901031421:171`), so the surface described below no longer exists. **Closed 2026-09-02: the missing regression test now exists** — `src/test/openJobsLocationMasking.test.ts` asserts that every anon-executable open-jobs feed masks `location`, that `open_jobs_safe` is never re-created after its DROP, and it is proven to fail against a negative control that reintroduces both. It runs in CI (filenames-and-text only, no database), which is the point: nothing now stops a future migration un-masking the column — the live post path still writes the full street to `jobs.location` (`src/pages/postjob/jobSubmitHelpers.ts:156`), so the data is there to leak; it is latent only because current rows carry no street numbers. Re-verified 2026-09-02.
