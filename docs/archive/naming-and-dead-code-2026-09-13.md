...
| A10 | `/home` route + `Dashboard.tsx` | `src/App.tsx:182` | Nav label "Home" (`mobileNavHelpers.ts:40`, `DesktopSidebarNav.tsx:54`), page heading "Browse Jobs" (`Dashboard.tsx:361`) — three names for one screen | Route `/browse` (guest already uses it), component `BrowseJobs` | Route is in URLs/deep links | High: 1 route, many `navigate("/home")`, AASA, email links; `/home/post-login` redirect already exists |
...
|---|---|---|---|
| B5 | `/home/post-login` → `/home` (`App.tsx:437`) | 0 refs in src, supabase/functions, public, ios | PROBABLY DEAD (old OAuth return URL; check Supabase Auth redirect allow-list before removing) |
