...
//
// @mutate src/pages/dashboard/useApplyFlow.ts | outcomeUnknownJobIds.current.set(vars.jobId, attemptStartedAt.current.get(vars.jobId) ?? Date.now()); | void vars.jobId;
// @mutate src/pages/dashboard/useApplyFlow.ts | recoveredId = await confirmThisAttemptLanded(rpcError); | recoveredId = null;
// @mutate src/pages/dashboard/useApplyFlow.ts | if (!row \|\| row.status !== "pending") return null; | if (!row) return null;
// @mutate src/pages/dashboard/useApplyFlow.ts | if (Date.parse(row.created_at) < firstStart - CLOCK_SLACK_MS) return null; | void CLOCK_SLACK_MS;
// @mutate src/pages/dashboard/useApplyFlow.ts | if (isNetworkFailure(err) && !outcomeUnknownJobIds.current.has(vars.jobId)) { | if (!outcomeUnknownJobIds.current.has(vars.jobId)) {
