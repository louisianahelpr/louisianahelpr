...
- [ ] **Q118 migration-lint Rule 1 reads SQL comments as DDL.** A header comment "CREATE TABLE IF NOT EXISTS, ..." failed db-deploy 35847561964 as "Table 'IF' created but RLS not enabled"; the whole-history pass (Q25) already flags 'helper_w' (20260609180000) and 'will' (20260915034822) the same way. Strip `--` comments before the CREATE TABLE grep in .github/workflows/migration-lint.yml and prove it on those three.
