-- CJ-004: a pg_cron job that fails INTERMITTENTLY alerted nobody.
--
-- sweep_dead_crons() said 'erroring' only when a job's last 3 runs ALL failed,
-- and sweep_cron_startup_failures() pages only a burst of 3+ failures in 20
-- minutes. A job that raises its own error once a day sits under both: on
-- 2026-09-22 09:00 sweep-cron-http-failures hit a statement timeout and
-- error_logs got no cron row about it (7-day measure 2026-09-24: that was the
-- only non-infra failure fleet-wide, so paging on each is not noise).
--
-- New verdict 'raised': any finished, failed run in the last 24 hours whose
-- message is not a lost connection or a startup timeout (those are the fleet
-- sweep's). Same once-per-job-per-day dedupe, same Slack page. The rest of the
-- body is the live definition, unchanged.
  ...
  FOR r IN
    ...
    live AS (
      SELECT e.jobname,
             ...
             (SELECT d4.return_message
                FROM cron.job_run_details d4
               WHERE d4.jobid = j.jobid
                 AND d4.end_time IS NOT NULL
                 AND d4.status <> 'succeeded'
                 AND d4.start_time > now() - interval '24 hours'
                 AND coalesce(d4.return_message, '') NOT ILIKE '%connection failed%'
                 AND coalesce(d4.return_message, '') NOT ILIKE '%startup timeout%'
               ORDER BY d4.start_time DESC
               LIMIT 1)                                     AS raised_msg
        FROM expected e
