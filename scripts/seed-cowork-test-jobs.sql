...
--
-- Column values are validated against the live prod check constraints:
--   payment_status ∈ {unpaid, escrow, payout_pending, released, refunded, cancelled, abandoned}
--   pricing_mode   ∈ {set_price, accept_bids, smart_price}
--   category       ∈ {cleaning, yard_work, moving, errands, handyman, painting,
--                      delivery, pet_care, assembly, other, storm_prep, events}
--
...
--
-- HEADS-UP: on a multi-user DB the "other poster" rows are inserted under an
-- EXISTING approved account (oldest non-Lexi). On prod that surfaces the jobs
-- in everyone's Browse feed and posts 5 jobs under a real account. Only run
-- against prod if that's acceptable; otherwise point at a staging DB.
--
-- HOW TO RUN (pick one):
...
--
-- TO REMOVE THE SEED LATER (run just these two lines):
--   DELETE FROM public.applications WHERE job_id IN (SELECT id FROM public.jobs WHERE special_requirements LIKE '[COWORK-SEED]%');
...
my_posts AS (
  ...
  SELECT customer_id, title, description, category::job_category, budget, date_needed, location,
         ...
         poster_completed_at, helper_completed_at, special_requirements, created_at
  FROM (VALUES
    ((SELECT lexi FROM ids), 'Help me unload moving truck Saturday',
     ...
     'open', 'set_price', 'escrow', NULL, NULL, NULL, '[COWORK-SEED] my-posts open no-apps', NOW() - INTERVAL '2 hours'),
    ((SELECT lexi FROM ids), 'Lawn mowing + edging — half acre',
     ...
     'open', 'set_price', 'escrow', NULL, NULL, NULL, '[COWORK-SEED] my-posts open with-apps', NOW() - INTERVAL '6 hours'),
    ((SELECT lexi FROM ids), 'Deep-clean kitchen before family visit',
     ...
     'accepted', 'set_price', 'escrow', (SELECT other FROM ids), NULL, NULL, '[COWORK-SEED] my-posts accepted', NOW() - INTERVAL '1 day'),
    ((SELECT lexi FROM ids), 'Hang 3 ceiling fans',
     'Three identical fans, boxes already in the rooms. Should take 1.5 hrs.',
     'handyman', 180, CURRENT_DATE, 'Lafayette, LA 70508',
     'in_progress', 'set_price', 'escrow', (SELECT other FROM ids), NULL, NULL, '[COWORK-SEED] my-posts in-progress', NOW() - INTERVAL '8 hours'),
    ((SELECT lexi FROM ids), 'Pick up + deliver dining table',
     ...
     'completed', 'set_price', 'released', (SELECT other FROM ids), NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 days', '[COWORK-SEED] my-posts completed', NOW() - INTERVAL '5 days')
...
applied_jobs AS (
  ...
  FROM (VALUES
    ((SELECT other FROM ids), 'Pressure-wash driveway + walkway',
     ...
     'open', 'set_price', 'escrow', NULL, '[COWORK-SEED] my-jobs pending-1', NOW() - INTERVAL '4 hours'),
    ((SELECT other FROM ids), 'URGENT: babysit 2 kids tonight 6-10pm',
     'Last-minute work event. 4 and 7 yo, both easy. Dinner already made.',
     'other', 80, CURRENT_DATE, 'Lafayette, LA 70501',
     'open', 'set_price', 'escrow', NULL, '[COWORK-SEED] my-jobs pending-urgent', NOW() - INTERVAL '1 hour'),
    ((SELECT other FROM ids), 'Walk 2 friendly dogs daily, 30 min',
     ...
     'accepted', 'set_price', 'escrow', (SELECT lexi FROM ids), '[COWORK-SEED] my-jobs accepted', NOW() - INTERVAL '2 days'),
    ((SELECT other FROM ids), 'Move 1-bedroom apt across town',
     ...
     'open', 'set_price', 'escrow', NULL, '[COWORK-SEED] my-jobs rejected', NOW() - INTERVAL '3 days'),
    ((SELECT other FROM ids), 'Tutor algebra II — 8th grader',
     '4 sessions over 2 weeks, weekday evenings. Propose your hourly rate when you apply.',
     'other', 200, (CURRENT_DATE + INTERVAL '2 days')::date, 'New Iberia, LA 70560',
     'open', 'accept_bids', 'escrow', NULL, '[COWORK-SEED] my-jobs pending-bid', NOW() - INTERVAL '12 hours')
  ) AS t(customer_id, title, description, category, budget, date_needed, location,
         status, pricing_mode, payment_status, helper_id, special_requirements, created_at)
...
-- ── 5. Application on Lexi's own with-apps post ─────────────────────
self_post_app AS (
...
browse_jobs AS (
  ...
  SELECT customer_id, title, description, category::job_category, budget, date_needed, location,
         ...
         is_group_job, special_requirements, created_at
  FROM (VALUES
    ((SELECT other FROM ids), 'Paint a 12x14 bedroom — one color',
     'Walls only, ceiling stays. Paint + supplies provided.',
     'painting', 220, (CURRENT_DATE + INTERVAL '6 days')::date, 'Lafayette, LA 70503',
     'open', 'set_price', 'escrow', false, 0, 1, false, '[COWORK-SEED] browse paint', NOW() - INTERVAL '20 minutes'),
    ((SELECT other FROM ids), 'Build IKEA wardrobe (PAX)',
     '2-bay PAX. Boxes on-site, tools available. Solo job, ~2.5 hrs.',
     'assembly', 130, (CURRENT_DATE + INTERVAL '3 days')::date, 'Youngsville, LA 70592',
     'open', 'set_price', 'escrow', false, 0, 1, false, '[COWORK-SEED] browse ikea', NOW() - INTERVAL '40 minutes'),
    ((SELECT other FROM ids), 'Group cleanup — flooded backyard',
     'After last week''s storm. Need 3 helpers for ~2 hours of debris-clearing.',
     'storm_prep', 300, (CURRENT_DATE + INTERVAL '2 days')::date, 'New Iberia, LA 70560',
     'open', 'set_price', 'escrow', true, 25, 3, true, '[COWORK-SEED] browse group urgent', NOW() - INTERVAL '15 minutes'),
    ((SELECT other FROM ids), 'Senior tech help — set up new iPad',
     '1 hr, patient teacher please. My mom wants to FaceTime grandkids.',
     'other', 50, (CURRENT_DATE + INTERVAL '1 day')::date, 'Lafayette, LA 70506',
     'open', 'set_price', 'escrow', false, 0, 1, false, '[COWORK-SEED] browse tech', NOW() - INTERVAL '3 hours'),
    ((SELECT other FROM ids), 'Grocery run + put-away',
     'List provided. About 20 items from Rouses, then put away when you get back.',
     'errands', 45, CURRENT_DATE, 'Delcambre, LA 70528',
     'open', 'set_price', 'escrow', false, 0, 1, false, '[COWORK-SEED] browse errands', NOW() - INTERVAL '5 minutes')
  ) AS t(customer_id, title, description, category, budget, date_needed, location,
         status, pricing_mode, payment_status, is_urgent, urgent_fee, helpers_needed,
         is_group_job, special_requirements, created_at)
  RETURNING id
