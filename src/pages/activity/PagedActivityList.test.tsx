 ...
 */
// @mutate src/pages/activity/PagedActivityList.tsx | export const ACTIVITY_PAGE_SIZE = 50; | export const ACTIVITY_PAGE_SIZE = 5000;
// @mutate src/pages/activity/PagedActivityList.tsx | setShown((n) => n + ACTIVITY_PAGE_SIZE) | setShown((n) => n)
// @mutate src/pages/activity/PagedActivityList.tsx | {items.length > shown && ( | {false && (
// @mutate src/pages/activity/ActivitySectionedView.tsx | <PagedActivityList items={bucketItems} getKey={getKey} renderItem={renderItem} onHiddenChange={reportHidden[key]} /> | <div>{bucketItems.map((i) => <div key={getKey(i)}>{renderItem(i)}</div>)}</div>
// @mutate src/components/activity/PostedJobsTab.tsx | <PagedActivityList items={visibleJobs} | <PagedActivityList items={visibleJobs.slice(0)}
// @mutate src/components/activity/AppliedJobsTab.tsx | <PagedActivityList items={apps} | <PagedActivityList items={apps.slice(0)}
// @mutate src/pages/activity/PagedActivityList.tsx | onHiddenChange?.(hidden); | onHiddenChange?.(0);
// @mutate src/pages/activity/ActivitySectionedView.tsx | onHiddenChange?.(totalHidden); | onHiddenChange?.(0);
// @mutate src/components/activity/PostedJobsTab.tsx | if (moreHidden \|\| !onSelectStatusFilter | if (!onSelectStatusFilter
...
describe("Activity lists page at 50 with Show older", () => {
  ...
  it("the flat single-status lists on both tabs page through the same component", () => {
    for (const [f, v] of [["PostedJobsTab", "visibleJobs"], ["AppliedJobsTab", "apps"]]) {
      const src = readFileSync(join(__dirname, "..", "..", "components", "activity", `${f}.tsx`), "utf8");
  ...
  it("the Posted tab's end-of-list line never says \"that's everything\" while cards are hidden", () => {
    const src = readFileSync(join(__dirname, "..", "..", "components", "activity", "PostedJobsTab.tsx"), "utf8");
