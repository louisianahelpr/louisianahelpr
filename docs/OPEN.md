...
- **Queue (this file):** 96 done, 10 partly done (fixed, protection pending), 87 open. Source of truth for work.
...
**Queue: 193 items — 96 done, 10 partly done (fixed, protection pending), 87 open.**
...
- [ ] **Q193 `npm run typecheck` is red on origin/main e992a4901 and still on 49747af6b (found by the Q135/Q136 lane 2026-09-23).** src/test/stripeWebhooksRefuseUnverified.test.ts(86,26): TS2550 Property 'at' does not exist on type 'RegExpExecArray[]' (the app tsconfig lib predates es2022). The file came in with d7295a8ef (Q156). Fix: index with `[arr.length - 1]` (or raise lib), then make sure the lane that landed it would have seen typecheck red.
- [ ] **Q194 Whole-repo vitest is red on origin/main e992a4901 (both still red on 49747af6b): 2 guards (found by the Q135/Q136 lane 2026-09-23; same 2 failures in a clean origin/main worktree, so not caused by that lane).** Whole-repo run 2026-09-23 14:53Z: 712/714 files, 7098/7101 tests passed. (1) src/test/workflowFalseGreenShapes.test.ts SET_PLUS_E: .github/workflows/stripe-webhook-guard.yml step "Self-test — the guard must go red on an undelivered event (Q156)" uses `set +e` without being registered as re-reading its codes (came in with d7295a8ef, Q156). (2) src/test/offeredHelperPrivacy.test.ts: e2e/prod-audit/fundedOpenJob.ts:212 reads `/rest/v1/jobs?...` with no `select=` (SELECT * is 42501 for authenticated since 20260915045110); name the columns. Both guards already exist; this item is fixing the offenders.
