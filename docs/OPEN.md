...
- [ ] Decide + clean (owner-owned deletes): 14 public `avatars` and 3 `user-documents` credential scans belong to users that no longer exist, so they were removed outside `accountPurge`. There are also 14 job-keyed objects (`proof-photos` 7, `message-attachments` 5, `application-attachments` 2) whose job row is gone, and 1 message attachment no message references. Nothing sweeps any of these.
- [ ] Report only: `deleteMessage` (`src/pages/Messages.tsx`) deletes the row but not its storage object. `business-documents` bucket is dead (`public.businesses` gone, 0 objects).
