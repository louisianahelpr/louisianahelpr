 ...
 *     Rebuilt on public.is_server_context() by 20260915051905.
 *
...
if (defaults.length) {
  ...
  console.error(
    "Every new relation in public would arrive with those grants (the open_jobs_browse re-grant). " +
      "Fix: ALTER DEFAULT PRIVILEGES FOR ROLE <owner_role> IN SCHEMA public REVOKE ALL ON TABLES|SEQUENCES FROM anon, authenticated (see 20260915051534).",
  );
}
if (nullUid.length) {
  failed = true;
  ...
  console.error(
    "An anon request has a NULL auth.uid() too. Fix: test public.is_server_context() instead (see 20260915051905), " +
