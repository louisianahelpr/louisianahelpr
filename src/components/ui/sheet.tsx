...
const SheetCloseButton = ({ top, right }: { top: string; right: string }) => (
  <SheetPrimitive.Close
    ...
    // `.ctl-exit` replaces `rounded-md`: the ONE shape every way out of a
    // surface wears (src/index.css). The square wash this used to paint is
    // half of the owner's 2026-09-19 report; guarded by
    // src/test/backControlSameness.test.ts.
    className="absolute inline-flex h-11 w-11 items-center justify-center ctl-exit ring-offset-background ctl-tint hover:text-foreground active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none"
    style={{
      top,
      right,
