...
const JobCard = ({ job, effectiveFee, currentUserId, showApply = true, onApply, onReport, onSelect, index = 0, isExpanded = false, onToggleExpand }: JobCardProps) => {
  ...
  const earnings = (job.budget - feeAmt - feeTax).toFixed(2);
    ...
    >
      ...
      <div className="px-4 py-2 border-t border-border/40 bg-muted/15 flex items-center justify-between">
        <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <a href={`/user/${job.customer_id}`} onClick={(e) => e.stopPropagation()} className="flex items-center gap-0.5 hover:text-primary transition-colors">
            <Star className={`w-3 h-3 ${(job.posterReviewCount ?? 0) > 0 ? "fill-accent text-accent" : "text-muted-foreground"}`} />
