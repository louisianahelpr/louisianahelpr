...
|---|---|---|---|---|
| **`src/test/*.test.ts*`** | 306 | **306 — COMPLETE** | 0 | **0** |
| **`src/test/edge/` (money)** | 60 | **60 — COMPLETE** | 0 | **0** |
| **colocated beside components** | 351 | **351 — COMPLETE** | 0 | **0** |
| **Playwright `e2e/`** | 62 | **54 — COMPLETE** | 8 | **0** |
| **total** | **779** | **771 (99%)** | **8** | **0** |

`npm run vacuity` prints the same three numbers on every run:

```
registration: 771/779 guards register a mutation (8 exempt with a reason, 0 grandfathered)
```
