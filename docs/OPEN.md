...
- **Queue (this file):** 151 done, 28 partly done (fixed, protection pending), 125 open. Source of truth for work.
...
**Queue: 304 items — 151 done, 28 partly done (fixed, protection pending), 125 open.**
...
- [x] **Q232 DONE 2026-09-23 (cloud/q232-admin): AdminUsers pages server-side (50 rows per `.range()`, Load more), tab/search/alpha+joined sorts run in Postgres (src/components/admin/adminusers/adminUsersQuery.ts), each tab count is its own `count: "exact", head: true` query, deep links and the auto-restricted rail fetch a user by id instead of searching the loaded array. The last-login/pay/standing sorts need other tables, so they reorder the loaded rows and the list says so (server-side for those is Q306). Guard: src/test/adminProfilesReadsAreBounded.test.ts (every admin `profiles` read must be paged, keyed or counted; exact two-way list of the 9 remaining whole-table reads), red on origin/main's AdminUsers.tsx (`AdminUsers.tsx#1`) and on both @mutate lines. Not verified in a browser against prod (no prod access in this lane). Was:** AdminUsers fetches every profile with select("*") and no page size; tab counts depend on the full array. (Q165 carry-over, 2026-09-23.) Evidence: src/components/admin/AdminUsers.tsx:149-151,243. From: docs/archive/launch-2026-09/OVERNIGHT-2026-09-07.md.
- [x] **Q233 DONE 2026-09-23 (cloud/q232-admin): one shared helper, src/components/admin/seedAware.tsx (isSeedRow, realRows, fetchSeedUserIds/fetchSeedJobIds for RPC rows, <DemoBadge>, <DemoExcludedNote>, DEMO_EXCLUDED_SUFFIX). Subscriptions, Payouts (batches by helper profile, ledger by job), Disputes, Tiers and Users now badge every seed row; the Subscriptions, Payouts and Tiers totals count non-seed rows only and their labels say "(excl. demo)". "Payments Collected" (Analytics and Dashboard, one function: src/components/admin/paymentsCollected.ts) counts card-captured jobs only; escrow jobs with no PaymentIntent (redeem_pif_credit gift-card jobs, or unrecorded) are shown beside it as their own figure, not dropped. Guard: src/test/adminSeedAware.test.tsx (render with a demo subscriber, the money split, and a source scan of the five views + no hand-rolled `is_seed` test), red on origin/main's AdminSubscriptions.tsx and on all three @mutate lines. NOT measured live (no prod access in this lane): the item's count of non-seed escrow jobs with no PaymentIntent is still unrun, and the Users tab counts still include demo accounts (the list shows them, badged). Was:** Admin console mixes seed rows into Subscriptions/Payouts/Disputes/Tiers/Users with no Demo badge; "Payments Collected" counts escrow jobs with no PaymentIntent. (Q165 carry-over, 2026-09-23.) Evidence: is_seed is read only in AdminAnalytics.tsx and pages/Admin.tsx; src/components/admin/adminAnalyticsHelpers.ts:141. Needs live check: select count(*), sum(budget) from jobs where not is_seed and payment_status in ('escrow','payout_pending','released') and stripe_payment_intent_id is null. From: docs/archive/launch-2026-09/OVERNIGHT-2026-09-07.md.
- [x] **Q234 DONE 2026-09-23 (cloud/q232-admin): DeleteUserDialog's "Delete Permanently" stays disabled until the admin types DELETE (the field clears for each new target; submit also refuses unarmed). "Manually Verify" (which sets idv_status = 'verified') is offered only when identity is NOT already verified by Stripe Identity or Connect: canManuallyVerify in adminUserHelpers.tsx, the same OR as isIdentityVerified (src/lib/awardGate.ts). Guards: src/components/admin/DeleteUserDialog.test.tsx ("typed confirmation (Q234)") and src/components/admin/AdminUserDetailDialog.test.tsx ("offers Manually Verify only to an unverified identity (Q234)"), red on origin/main's DeleteUserDialog.tsx and ActionsTab.tsx and on their three @mutate lines. Was:** Admin low-severity controls: one-click Delete Account (no typed confirm); "Manually Verify" offered on an already-verified user. (Q165 carry-over, 2026-09-23.) Evidence: src/components/admin/DeleteUserDialog.tsx:45,87; src/components/admin/userDetail/ActionsTab.tsx:130-131. From: docs/archive/launch-2026-09/OVERNIGHT-2026-09-07.md.
...
- [~] **Q238 PARTLY DONE 2026-09-23 (cloud/q232-admin): all eight admin views now render POPULATED in a component test: src/test/adminViewsPopulated.test.tsx seeds one waiting row per view (pending credentials, IDV review, verification exceptions, ban review, fraud flags, support tickets, broadcasts, expired subscriptions), asserts the row shows and neither the empty nor the error state does, and proves each view's empty state still renders with no rows; red on both @mutate lines (credential queue fetcher emptied, ban-review read limited to 0). Fixture values pass fixtureSchemaContract. The data layer is a table-keyed stub, so this proves the render path, not the live queries. STILL OPEN: the item's live check (these views on prod with seeded is_seed rows, screenshotted and recorded with recordReview) and helper accept-then-cancel, which needs a helper fixture with a Stripe payout account; neither is possible in a lane with no prod access. Was:** Coverage gap: admin views never seen POPULATED (pending credentials, IDV review, exceptions, ban review, fraud flags, support tickets, broadcasts, expired subs) and helper accept-then-cancel (needs a helper fixture with a Stripe payout account). (Q165 carry-over, 2026-09-23.) Evidence: the report shot these EMPTY only; needs live check with seeded rows. From: docs/archive/launch-2026-09/OVERNIGHT-2026-09-07.md.
...
- [ ] **Q306 Nine admin reads still load whole `profiles` row sets (found by the Q232 lane, 2026-09-23).** Listed exactly in KNOWN_UNBOUNDED_ADMIN_PROFILE_READS (src/test/adminProfilesReadsAreBounded.test.ts): AdminAnalytics loads every non-seed profile with select("*") (#1), its subscriptions and Users drill-downs list them all (#2, #3); AdminSubscriptions' two lists; the health fan-out parish map (useHealthData); the dashboard's new-user sparkline windows (Admin.tsx #3, #4); AdminExport (whole table by design, stays). Also AdminUsers' last-login / pay / standing sorts only reorder loaded pages: server-side needs an admin RPC ordering by login_history / jobs / user_violations aggregates. Fix each with counts, an aggregate RPC or paging, and remove its entry (the guard fails on a stale one). Guard: src/test/adminProfilesReadsAreBounded.test.ts.
- [ ] **Q307 ManualVerifyDialog copy still says "approval will be set to approved" (found by the Q234 lane, 2026-09-23).** src/components/admin/ManualVerifyDialog.tsx body text; admin-user-actions manual_verify writes only idv_status/idv_confidence/idv_failure_reason/legacy_manual_review, and approval review was retired (Q193/Q205b). Drop the approval half of the sentence. Guard: a ManualVerifyDialog.test.tsx assertion that the body names only what the edge function writes.
