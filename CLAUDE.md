...
- **Every alert, from anywhere, is fixed AND verified fixed (owner, 2026-09-23).** Slack #ops-alerts, error_logs, Sentry, `nightly-red` issues, CI: read them FIRST each session; each gets a root cause, a fix (or an owner pop-up if it needs credentials/money), and its own detector re-run to show it cleared. Recurring noise is fixed at the source. Posted is not handled.
- **Every number we track stays current (owner, 2026-09-23).** Any count anywhere (docs, CLAUDE.md, comments, inventories of routes/buttons/workflows) is generated, bound to the tool that measures it, or a dated historical record; nothing undated goes stale. Every baseline, budget, allowlist, KNOWN_/LEGACY_ list and floor is EXACT: when a fix lowers a count, lower its baseline in the same commit, and the check fails in both directions (a stale loose baseline hides the next regression). When you change what a test or workflow measures, update it in the same commit.
...
- **Stripe stays in SANDBOX (test mode) until launch (owner, 2026-09-12).** Money journeys pay for real on Stripe test mode with 4242 4242 4242 4242, nightly. Switching to live (`scripts/e2e/stripe-sandbox-off.sh`) is a launch-day step only, on the launch checklist in docs/OPEN.md.
- **NO MOCK MODE, EVER. Every audit, sweep and journey runs against PROD (owner, 2026-09-12, said twice: "get rid of mock entirely, just use prod since no one uses it"; "no mock mode ever").** Mocked-Supabase results do not count as verification, and work done on mocks gets redone on prod. Use the shared test accounts, mark created data `is_seed`, clean up after. Destructive actions (delete, ban, pay, release) only ever target test-owned records. Do not add new mocked-Supabase specs; migrate existing ones.
...
- **"I looked" is recorded, not claimed.** For every screenshot you say you inspected, call `recordReview` (`e2e/reviewLog.ts`, or `npm run review:record -- <png> <screen> <checked> <ok|defect> [note]`). `npm run review:report` must show no UNREVIEWED failure or changed screenshot before visual work is called done.
...
- **`npm run gate` IS the gate** — typecheck, eslint, repo-wide vitest, and the standalone guards that live outside vitest (race-class, migration, loading-state, deferred-vendor, CLAUDE.md). It PRINTS every step and marks a skipped one, because the failure it exists for is running a subset and believing it was the whole. `--fast` drops only the vitest run and says so. Never report a `--fast` run as clean. [L](docs/lessons/CLAUDE-lessons.md#repo-wide)
...
- **A push to main does NOT deploy the web app (Q271, Hobby cap 100/day).** `vercel.json` has `git.deploymentEnabled: false`; `.github/workflows/prod-deploy.yml` ships main HEAD in batches (push + every 20 min, at most one deploy per 20 min), so a fix is live up to ~20 min later. Need it now: `gh workflow run prod-deploy.yml`. Never re-enable per-push deploys.
...
- **Local RAM is the bottleneck (8 GB Mac; 2026-09-14 it sat at 78 MB free with 2 GB swap).** Spawn agents with the Agent tool INSIDE one session (they share its process), never as separate terminal `claude -p`/`claude "<prompt>"` processes (~350 MB each plus MCP servers). Long or heavy lanes (audits, journeys, race proofs, overnight work) go to cloud sessions or routines, not this Mac. Tests locally: vitest 2 threads, Playwright 1 worker (the configs' defaults; don't raise them).
...
- **Stale worktrees and merged local branches are pruned AUTOMATICALLY** at session start (`.claude/hooks/git-hygiene.sh`, once per 6h, log `~/.lh-hygiene/hygiene.log`); by hand: `npm run hygiene` (dry run) / `npm run hygiene -- --apply`. Never clean them with a hand-rolled loop. Unused exports are ratcheted by `src/test/deadcodeRatchet.test.ts` (lower `scripts/deadcode-baseline.json` when they fall).
