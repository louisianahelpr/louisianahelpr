-- Q40 (owner decision 2026-09-23, "Remove id"): users never submit an ID to us;
-- Stripe Identity collects it. This retires the "upload your ID to us" flow in
-- the database:
--
--   1. profiles.id_document_url is EMPTIED and PINNED NULL, not dropped yet.
--      Measured on prod 2026-09-23: 53 non-null rows, 52 of them is_seed test
--      profiles holding a data: placeholder, one the owner's own storage path
--      (that object was deleted through the Storage API the same day; the
--      bucket now holds 0 objects). After this commit no client or edge code
--      reads or writes the column (guard: src/test/retiredIdUploadStaysGone.test.ts).
--      WHY NOT DROP: shipped iOS/Android builds and cached web bundles run the
--      old useUserProfileData, which names this column in an explicit select on
--      EVERY public profile view. Dropping it would make that query 42703 on
--      those builds and silently degrade the trust signals it feeds (the error
--      only reaches Sentry as a warning). Keeping an always-NULL column is the
--      PostgREST-safe fallback; the DROP is queued in docs/OPEN.md (Q196) for
--      after the store build without the select has shipped.
--      The CHECK is the real guarantee: anon and authenticated hold TABLE-level
--      INSERT on profiles (measured), so a column revoke cannot stop an insert.
--      The column-level UPDATE grant authenticated held is revoked as well.
--   2. The four storage.objects policies on the id-documents bucket (own
--      upload, own view, own delete, admin view) are dropped. The bucket is
--      private and no other storage policy lacks a bucket_id predicate, so it
--      is now closed to every client; the service role (accountPurge,
--      storage-orphan-sweep) still reaches it. The bucket itself is kept, as
--      20260828011811 did for business-documents.
--   3. purge_user_data() no longer names the column (step 4e), so the later
--      DROP cannot break account deletion: plpgsql binds columns at call time
--      and a stale body would fail every call with 42703. The body below is
--      20260913051340's (md5-identical to the live prosrc, 2026-09-23) with only
--      that one SET line removed.
--
-- Replay-safe: CREATE OR REPLACE, DROP POLICY IF EXISTS, an UPDATE that is a
-- no-op once the values are NULL, DROP CONSTRAINT IF EXISTS before its ADD.
-- Roles are named on every REVOKE.
...
DROP POLICY IF EXISTS "Users can upload their own ID docs"      ON storage.objects;
DROP POLICY IF EXISTS "Users can view their own ID docs"        ON storage.objects;
...
UPDATE public.profiles SET id_document_url = NULL WHERE id_document_url IS NOT NULL;

REVOKE UPDATE (id_document_url) ON public.profiles FROM PUBLIC, anon, authenticated;

ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_id_document_url_retired;
ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_id_document_url_retired CHECK (id_document_url IS NULL);

COMMENT ON COLUMN public.profiles.id_document_url IS
  'RETIRED (Q40, 2026-09-23): always NULL. Kept only so shipped app builds that still select it do not 42703; dropped by Q196.';
