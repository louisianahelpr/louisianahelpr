...
- **Queue (this file):** 91 done, 10 partly done (fixed, protection pending), 87 open. Source of truth for work.
...
**Queue: 188 items — 91 done, 10 partly done (fixed, protection pending), 87 open.**
...
- [ ] **Q188 weekly-helper-report has no send-dedupe of its own (found by Q30, 2026-09-23).** No sent-marker in supabase/functions/weekly-helper-report (grep for sent_at/dedupe/already: none), so any second invocation in a week (manual re-run, retry) emails every Helpr twice; that is why Q30 lists it as NOT catch-up-safe. Add a per-helper per-week sent marker, with a test that a second run sends nothing, then it can join the catch-up allowlist.
- [ ] **Q189 Other axe callers still hardcode the pre-wcag22aa tag list (found doing Q181, 2026-09-23, cloud session).** `e2e/happy-path/axeTags.ts` (AXE_TAGS: wcag2a/2aa/21a/21aa/22aa + best-practice) now backs the prod route sweep (sweepCore.ts) and the journey spot-check helper (fixtures.ts checkA11y) — see Q181. Grep 2026-09-23 for the OLD 4-tag literal still finds it hardcoded, independently, in: `e2e/happy-path/device-pass-measure.spec.ts` (x2), `e2e/happy-path/activity-card-density.spec.ts` (x2), `e2e/happy-path/home-chrome.spec.ts`, `e2e/happy-path/empty-state-sweep.spec.ts` (wired into `ui-sweep.yml` on every push/PR), `e2e/happy-path/overlay-sweep.spec.ts` and `e2e/happy-path/error-state-sweep.spec.ts` (both weekly in `ui-sweep.yml`), `e2e/a11y.spec.ts` (wired into `e2e-real-backend.yml`). NOT migrated in Q181's pass: each of these runs against mocked/controlled-state data (empty collections, forced errors) rather than prod, so widening its tag set needs its OWN two-way baseline generated against ITS OWN data shape (a shared baseline keyed the same way would misclassify — a violation real on prod's empty-state screen may not reproduce on the mocked one, or vice versa), which is more than one session should do unreviewed. Migrate each to `import { AXE_TAGS } from "./axeTags"` + its own `knownAxeViolations`-shaped baseline (or share the mechanism from `e2e/happy-path/knownAxeViolations.ts` if its per-screen+variant+id keying turns out to fit), one spec at a time, each proven red/green before the next.
...
- [ ] **Q135 A plain whole-repo `npx vitest run` deadlocks on its own gate lock (found by the Q129 lane 2026-09-23).** src/test/gateLockGlobalSetup.ts is the root `globalSetup`, and both projects in vitest.config.ts (`unit`, `tz-sweep`) `extends: true`, so it runs twice in ONE process: the first call takes ~/.lh-gate.lock with this pid, the second finds the owner pid alive (itself) and waits (log printed "acquiring the gate lock" twice, then "waiting for pid 70270" = its own pid; hung 30+ min, killed). `npm run test:unit` / `node scripts/gateLock.mjs -- npx vitest run` are unaffected (LH_GATE_LOCK_HELD=1). Fix: make acquireGateLock re-entrant for `owner.pid === process.pid` (and release only on the last teardown), with a case in src/test/gateLockGlobalSetup.test.ts that calls setup() twice in-process.
...
- [ ] **Q181 Accessibility gate: WCAG 2.2 AA + best-practice axe run in CI (from TODO.md, 2026-08-31).** No workflow runs axe with the `wcag22aa` tag (grep on e2e/ and .github/ 2026-09-23: none). Overlaps Q71; do them together.
  - STATUS 2026-09-23 (cloud session, branch `cloud/q181-axe-wcag22`, no prod access): the prod route sweep now runs `wcag2a/2aa/21a/21aa/22aa` + `best-practice` from one shared constant (`e2e/happy-path/axeTags.ts`, used by both `sweepCore.ts`'s route sweep and `fixtures.ts`'s `checkA11y`), driven by the existing route inventory (`e2e/happy-path/auditRoutes.ts`, checked against `src/App.tsx` by `src/test/auditCatalogRoutes.test.ts`). Added a two-way baseline (`e2e/happy-path/knownAxeViolations.ts`, JSON file `e2e/happy-path/axe-known-violations.json`, same shape as `knownContrastFailures.ts`) so a NEW violation fails and a fixed one left in the baseline fails too. Guard: `src/test/axeGateCoversWcag22aa.test.ts` (proved red by hand: removing `"wcag22aa"` from `axeTags.ts` failed it, restoring it passed; `// @mutate` line included for the vacuity gate). **Cannot be ticked from here**: no prod access, so `e2e/happy-path/axe-known-violations.json` does not exist yet — `assertSweepGate` in `sweepCore.ts` detects that, writes what it finds to `<OUTPUT_DIR>/axe-baseline-generated.json` (inside the `a11y-prod-*` artifact `.github/workflows/a11y-webkit-prod.yml` already uploads), and fails with "axe baseline missing". **Lead, on the Mac**: run/wait for `a11y-webkit-prod.yml` (Mon/Wed/Fri, or `workflow_dispatch` it now), download the `a11y-prod-chromium` and `a11y-prod-webkit` artifacts, review each generated entry (this grandfathers real measured debt — read every line), merge them into `e2e/happy-path/axe-known-violations.json`, commit, register it in `src/test/baselinesAreTwoWay.test.ts`'s `JSON_TWO_WAY` (declaration text is in `knownAxeViolations.ts`'s header), confirm the sweep goes green, THEN tick Q181 naming `src/test/axeGateCoversWcag22aa.test.ts` + the committed baseline file. See Q189 for axe callers this pass did NOT migrate to `AXE_TAGS`.
