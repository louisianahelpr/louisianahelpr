...
- [ ] QUEUED (next prod slot): prod has 0 group jobs, so group-job screens (GroupJobHelpers flat card, 70f93a220) were proven only on a local render. Add an is_seed group job + group_job_helpers rows to scripts/audit/prod-seed.mjs, apply just that part once, screenshot the poster card at 375 light/dark on prod, and extend the a11y-prod sweep to include it.
