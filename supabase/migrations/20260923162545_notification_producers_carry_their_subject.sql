...
-- reach a real admin.
--
-- THE CHANGE. Each function is restated VERBATIM from its newest definition
-- (named above each one) with one addition per insert: the job_id column set
-- to the job the row is about, or, for the two member alerts to admins, the
-- member named in the link as '&user=<id>'. Nothing else changes: same
-- recipients, same copy, same links (the two admin links gain a parameter no
-- admin view reads), same types. For a real subject the row lands exactly as
-- before; job_id also lets the notification centre open the job directly.
--
...
--
-- REPLAY-SAFETY: CREATE OR REPLACE only, with the signatures the newest
-- definitions already have, so grants and triggers are untouched; no later
-- migration defines these functions.

-- check_referral_bonus: newest definition is 20260902014651_account_deletion_purges_the_no_fk_tables.sql; restated verbatim except: each of the 4 notifications carries job_id = NEW.id (the completed job).
...
-- track_revision_scope_creep: newest definition is 20260418082051_cdd00eb4-9fb7-42d0-a20d-1b99db2620bd.sql; restated verbatim except: both notifications carry job_id = NEW.id.
...
-- notify_poster_on_status_change: newest definition is 20260829061546_helper_mark_on_the_way_atomic.sql; restated verbatim except: the notification carries job_id = NEW.id.
...
-- notify_helper_on_tip: newest definition is 20260923152630_notification_links_point_direct_not_at_redirects.sql; restated verbatim except: the notification carries job_id = NEW.job_id (the tipped job).
...
-- notify_helper_on_direct_offer: newest definition is 20260831203052_dedupe_application_notifications_and_fix_offer_links.sql; restated verbatim except: the notification carries job_id = NEW.id (AFTER INSERT/UPDATE trigger, so the job row exists for the FK).
...
-- notify_helper_application_viewed: newest definition is 20260612540000_notify_on_application_viewed.sql; restated verbatim except: the notification carries job_id = NEW.job_id (the application's job).
...
-- respond_to_direct_offer: newest definition is 20260820000000_respond_to_direct_offer.sql; restated verbatim except: the decline notification carries job_id = the offered job.
...
-- expire_unanswered_offers: newest definition is 20260824243000_reliability_ladder_temp_ban_and_cancel_booking.sql; restated verbatim except: both notifications carry job_id = v_locked.id.
...
-- sweep_dayof_confirm_reminders: newest definition is 20260831193039_cron_sql_error_reporting.sql; restated verbatim except: all 3 notifications carry job_id = rec.id.
    ...
    BEGIN
      ...
      IF rec.helper_dayof_confirmed_at IS NULL
        ...
        VALUES (rec.helper_id, 'job_update', 'Still on for tomorrow?',
                ...
                '/my-jobs?filter=offered', false, rec.id);
      ...
      IF rec.poster_confirmed_at IS NULL THEN
        ...
        VALUES (rec.customer_id, 'job_update', 'Still on for tomorrow?',
                ...
                '/my-posts?filter=offered', false, rec.id);
        total_pushed := total_pushed + 1;
      END IF;
...
-- apply_job_denial_consequence: newest definition is 20260923152630_notification_links_point_direct_not_at_redirects.sql; restated verbatim except: the Elite-shield notification carries job_id = p_job (the job the strike is about).
...
-- sweep_release_last_chance: newest definition is 20260831193039_cron_sql_error_reporting.sql; restated verbatim except: the notification carries job_id = rec.id.
...
-- helper_abort_job: newest definition is 20260825191500_helper_abort_in_progress_job.sql; restated verbatim except: both notifications carry job_id = v_job.id.
...
-- apply_low_rating_flag: newest definition is 20260826020000_low_rating_flag_server_side.sql; restated verbatim except: the admin alert's link names the flagged member ('&user=<id>', the actor the seed boundary reads; nothing on view=fraud reads it).
...
-- open_dispute_as: newest definition is 20260915071502_reapply_dispute_settlement_objects.sql; restated verbatim except: the admin 'Job disputed' alert carries job_id = _job_id (found by the tightened guard, as above).
