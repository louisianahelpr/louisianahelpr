...
- **OPEN — `customer.subscription.created` is subscribed with no handler.** Verified live 2026-09-12 against `acct_1RQbAfKp2H4b7tEC` test mode (`livemode:false`), endpoint `we_1U8AbhKp2H4b7tECpsHaYA7D`: 16 subscribed events vs 15 keys in `EVENT_HANDLERS`. The extra event is delivered and dropped on the floor as "Unhandled event type". Decide which way to close it — add a `customerSubscriptionCreated` handler, or drop the subscription (the next `stripe-sandbox-on.sh` run drops it automatically, since the list is now generated from the handler map). Left undecided deliberately: whether subscription creation needs its own handler is a subscriptions-lane call, not a scripts call. The new guard reports this as a failure today.
