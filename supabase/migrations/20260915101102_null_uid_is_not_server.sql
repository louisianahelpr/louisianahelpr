...
--
-- RECONCILED 2026-09-15 (sec-hardening-v2). Every body below is the CURRENT
-- live definition on prod. Five guards were re-derived against migrations that
-- landed after this file was first drafted, so their bodies here are NOT the
-- earlier capture — shipping the earlier one would have reverted a real fix:
--   enforce_helper_completion_gates, enforce_helper_jobs_column_whitelist,
--   enforce_poster_jobs_money_lock  <- 20260915044137 (VN-33 arrival: BOTH
--     stamps required, helper_arrived_at off the helper allow-list and into the
--     poster money-lock, the status door);
--   prevent_job_field_escalation    <- 20260915074058 (arrival + poster-confirm
--     stamps locked from the offered-helper seat);
--   enforce_jobs_insert_column_lock <- live (near-miss stamps cleared on insert,
--     is_seed derived from the posting account).
-- The trust swap is the ONLY change applied to each; the reconciled bodies are
-- otherwise verbatim from the live definition (per-function before-md5 noted).
-- enforce_job_completion_server_owned (20260915073143) is deliberately NOT in
-- this set: it is SECURITY INVOKER and already tests the REAL role
-- (current_user NOT IN ('authenticated','anon')), which is exactly what this
-- migration is about — a NULL uid never fools it. Left untouched.
--
...
-- current_user is deliberately NOT used: 22 of the 26 are SECURITY DEFINER, so
...
-- no EXECUTE to fire; PUBLIC and anon are revoked on all 26 anyway (several
-- still carried both). is_server_context() is EXECUTE for anon and
-- authenticated because four of the 26 are SECURITY INVOKER triggers that run
...
-- body is not resolved against tables at creation — so this runs three times
-- over. One reconciled guard, enforce_job_tracking_arrival_gate (as in
-- 20260915044137, which first defined it), DECLAREs `v_job public.jobs`, so the
-- composite TYPE public.jobs must exist when it is created. Every from-scratch
-- replay has it: public.jobs is created by a migration long before this one
-- runs, exactly as 20260915044137 relies on. is_server_context() is LANGUAGE
-- sql and IS checked at creation, but references only auth.uid()/auth.role()
-- (defined by earlier migrations) and the built-in current_setting.
...
-- enforce_helper_completion_gates  (live md5(prosrc) before: 0d401fde667f4b6cfd176e2c8904ffbd; body re-derived from 20260915044137 — VN-33 status door + BOTH-stamp arrival, grandfather clause removed)
...
-- enforce_helper_jobs_column_whitelist  (live md5(prosrc) before: b5d739b289705fcd0a2e62deabcabd3f; body re-derived from 20260915044137 — helper_arrived_at off the allow-list, admitted only under app.arrival_rpc)
...
-- enforce_jobs_insert_column_lock  (live md5(prosrc) before: a42afaa31bb0f6165dc4811a10aa34d5; body re-derived from live — near-miss stamps cleared on insert, is_seed derived)
...
-- enforce_poster_jobs_money_lock  (live md5(prosrc) before: 5df67c818ef0e12d85bbbc993b209258; body re-derived from 20260915044137 — helper arrival stamps added to locked_always)
...
-- prevent_job_field_escalation  (live md5(prosrc) before: 8713ea5b97edc2db19c14edd2a0c3393; body re-derived from 20260915074058 — arrival + poster-confirm stamps added to poster_locked_always)
...
-- RECONCILIATION 2026-09-15 (sec-hardening-v2). Three more guards match the
-- same NULL-uid-trust class on the CURRENT live catalog but landed AFTER this
...
--     live body kept, only the auth.uid() IS NULL gate swapped)
-- enforce_jobs_arrival_integrity is the fourth live match and is EXEMPT, not
-- rewritten: its flagged branch also requires auth.uid() = OLD.customer_id,
-- which a NULL uid can never satisfy, so pre-excluding NULL changes nothing;
-- the reset block above it is not uid-gated and runs for every writer. Reason
-- recorded in scripts/ci/null-uid-trust.sql.

-- enforce_completion_on_live_job  (live md5(prosrc) before: b1f... caller-role gated)
...
BEGIN
  ...
  -- and check status themselves. A NULL uid alone is not the service role
  -- (anon has one too, 20260915101102), so the status door below now applies
  -- to an anon stamping write and waves through only a real server context.
  IF public.is_server_context() THEN
    RETURN NEW;
  END IF;
...
-- enforce_dispute_evidence_append_only
...
BEGIN
  ...
  -- A NULL uid alone is not the service role — anon has one too — so a server
  -- context is tested here rather than a bare NULL uid (20260915101102).
  IF public.is_server_context() OR public.has_role(_uid, 'admin') THEN
    RETURN NEW;
  END IF;
...
-- enforce_job_tracking_arrival_gate  (20260915044137 body + bad-pin near-miss, live)
...
-- Reconciled 2026-09-15: three more guards in the same class (SECURITY DEFINER
-- trigger functions; never called by a client role).
REVOKE ALL ON FUNCTION public.enforce_completion_on_live_job() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.enforce_dispute_evidence_append_only() FROM PUBLIC, anon;
