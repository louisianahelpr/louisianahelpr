...
**119 files render form controls, across 22 route groups.** Primitives in `src/components/ui/` are excluded. Messy-input coverage: `e2e/happy-path/messy-input.spec.ts`.
...
| `src/components/dashboard/JobCard.tsx` | date/calendar×2 |  |  | Just posted · Boosted · Instant book |
...
| File | Controls | input types | maxLength | hints |
|---|---|---|---|---|
| `src/pages/Legal.tsx` | input×1 | text |  | Search all policies · Search all policies… · Clear search · Close search |
...
| `src/components/postjob/detailsSection/DescriptionField.tsx` | textarea×1 |  |  | description |
...
| `src/components/postjob/detailsSection/TitleField.tsx` | input×1 | text |  | title · title-too-long |
...
| `src/components/profile/ProfileEditForm.tsx` | input×4, textarea×1 | tel, text | 5 | phone · location · zipCode · bio · skillsOther · Anything else you offer… |
