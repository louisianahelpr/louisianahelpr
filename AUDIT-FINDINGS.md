...
- 🟡 `src/pages/BenefitsPage.tsx` — orphan `max-w-lg` narrow column (siblings use `max-w-5xl`) → dead desktop gutters; raw `text-white`/`text-lg` instead of `text-ds-*`/`rounded-ds-*`; no `usePageTitle`. Widen + tokenize + add title.
...
- 🟠 **HIGH — no marketing/push/SMS consent captured at signup.** `src/pages/signup/SignupStep1.tsx:240-268` — only a single bundled Terms/Privacy checkbox; no separate opt-in, no `marketing_opt_in`/`sms_consent` column. Later marketing sends have no per-user consent to honor. Add explicit unchecked-by-default opt-in + persist + have `send-marketing-blast`/`engagement-automations` read it.
