/**
 * PGlite proof for 20260923162545_notification_producers_carry_their_subject
 * (docs/OPEN.md Q139).
 *
...
const NEW_FILE = "20260923162545_notification_producers_carry_their_subject.sql";
...
/** The newest CREATE FUNCTION statement for `name` in migrations strictly before `before`. */
...
// The producers: new migration x3, or the previous definitions.
if (MODE === "skip") {
  for (const fn of FNS) await db.exec(previousDefinition(fn, NEW_FILE));
} else {
  for (let i = 0; i < 3; i++) await db.exec(mig(NEW_FILE));
