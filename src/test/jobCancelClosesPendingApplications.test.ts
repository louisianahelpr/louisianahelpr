// @mutate supabase/migrations/20260923185622_close_pending_applications_on_job_cancel.sql | WHEN (NEW.status = 'cancelled' AND OLD.status IS DISTINCT FROM NEW.status) | WHEN (NEW.status = 'completed' AND OLD.status IS DISTINCT FROM NEW.status)
// @mutate supabase/migrations/20260923185622_close_pending_applications_on_job_cancel.sql | SET status = 'rejected',\n         closed_reason = 'job_cancelled'\n   WHERE job_id = NEW.id | SET status = 'rejected'\n   WHERE job_id = NEW.id
// @mutate supabase/migrations/20260923185622_close_pending_applications_on_job_cancel.sql | IF NEW.closed_reason = 'job_cancelled' THEN | IF false THEN
