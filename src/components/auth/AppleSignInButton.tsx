// AppleSignInButton — thin compatibility shim around the shared
// SocialAuthButtons single-button render. New code should use
// <SocialAuthButtons mode="signin" /> directly to render both providers
// together; this export stays so existing call sites that only want the
// Apple button (and pass a custom label) keep working.
...
export const AppleSignInButton = ({
  ...
  const handleClick = async () => {
    setLoading(true);
    const result = await signInWithProvider("apple", { redirectTo });
    switch (result.kind) {
      case "success": {
