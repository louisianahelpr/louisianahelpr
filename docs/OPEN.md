...
- OPEN (HIGH, was already there, from the lh-money-escrow review of the fix above): `charge.dispute.created` overwrites `dispute_status` and `disputed_at` on a payable job whatever hold was already on it. `warning_closed` then clears `disputed_at`, the only guard in process-scheduled-payouts, so it lifts holds the chargeback never placed. Two ways this pays out wrongly: (1) a decided internal dispute whose split never ran (`rpc_decide_dispute` leaves `payment_status` and `disputed_at` alone) goes back to payout_pending with no hold, and the helper is paid in full over the decided refund; (2) a `transferReversed` `reversal_hold` job gets paid a second time. Fix: `chargeDisputeCreated` must not overwrite a non-null `dispute_status`, and `warning_closed` must keep `disputed_at` and page ops when an unexecuted `disputes` row or a reversed `payout_transfers` row exists. Needs a guard test for both cases.
...
- [ ] QUEUED FIRST (with the chargeback HIGH): stop tests loading www.louisianahelpr.com (nightly sweeps/prod audits run against a local preview on the prod DB); add Vercel edge-request + storage usage to the weekly usage alert at 70%. Owner to set Deployment Retention to shortest.
