 ...
 *
 * Inventory: every .ts/.tsx under src/pages/info/legal and src/pages/info/helpCenter
 * (plus HelpCenter.tsx), tests excluded; floor so an emptied glob fails.
 *
 * @mutate src/pages/info/legal/TermsSection.tsx | Every account verifies its email address before it can post or take jobs. | All accounts are reviewed before approval.
 * @mutate src/pages/info/legal/TermsSection.tsx | You must verify your email address to use your account. | Denied accounts may reapply.
 */
...
function files(): string[] {
  const roots = ["src/pages/info/legal", "src/pages/info/helpCenter"].map((r) => join(REPO, r));
  return [...walkSource(roots), join(REPO, "src/pages/info/HelpCenter.tsx")]
