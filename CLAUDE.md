...
- **Monitoring numbers stay current (owner, 2026-09-23).** Every baseline, budget, allowlist, KNOWN_/LEGACY_ list and floor is EXACT: when a fix lowers a count, lower its baseline in the same commit, and the check fails in both directions (a stale loose baseline hides the next regression). When you change what a test or workflow measures, update it in the same commit.
