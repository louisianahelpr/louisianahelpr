...
- **Queue (this file):** 147 done, 24 partly done (fixed, protection pending), 127 open. Source of truth for work.
...
**Queue: 298 items — 147 done, 24 partly done (fixed, protection pending), 127 open.**
...
- [x] **Q239 DONE 2026-09-23 (branch cloud/q206-perf): the Applicants panel loads in 2 network rounds instead of 4.** Measured by the guard's round counter (every Supabase call held until the whole in-flight set is released as one round trip): before, 4 dependent rounds / 12 requests before the ranked list could render — applications, THEN user_blocks, THEN get_safe_profiles + get_public_profile_stats + profiles.available_until, THEN (after the list rendered and useApplicantSignals mounted) 3x get_neighbor_hire_count + completed / repeat-hire / on-time / distance-band RPCs, which PostedJobsTab waits for behind the skeleton. After: 2 rounds / 12 requests (none duplicated): applications + user_blocks together; then enrichment + every ranking signal together, prefetched by src/pages/activity/activityActions/useApplicantsState.ts into React Query under useApplicantSignals' own keys (prefetchApplicantSignals in src/components/activity/postedJobs/useApplicantSignals.ts), so the hook reads a warm cache. userBlocks is now a static import (already in the signed-in shell via useNavUnreadCount), so no lazy chunk sits in front of the block read. Round 1 is irreducible client-side (every later read needs the applicant ids); one server-side RPC would make it 1 round, not done (new SECURITY DEFINER RPC, needs an authz review). Throttled wall-clock (~7 s before) NOT re-measured live: no prod/browser access in this lane (Q301). GUARD: src/pages/activity/activityActions/applicantsPanelRounds.test.tsx (rounds exactly 2 and each round's request list exact; red on origin/main: 4 rounds; 4 @mutate registrations, all killed). Was: Applicants panel loads as a 3-hop ~7 s waterfall. (Q165 carry-over, 2026-09-23.)** Evidence: needs live check: throttled time from opening Applicants on /my-posts to the first applicant row. From: docs/archive/launch-2026-09/OVERNIGHT-2026-09-07.md.
...
- [x] **Q258 DONE 2026-09-23 (branch cloud/q206-perf): the signup confirmation is rendered once and sent inline.** supabase/functions/auth-email-hook/index.ts sends SEND_INLINE types (signup) straight to Resend via sendWithResend with a 3 s budget (INLINE_SEND_TIMEOUT_MS, under GoTrue's hook deadline), marks the pending email_send_log row sent, and enqueues nothing; on a Resend error or timeout it falls back to q_auth_emails with the same message_id (process-email-queue's 'sent' lookup still dedupes). Every other auth type and all bulk mail keep the queue. Render once: supabase/functions/_shared/email-templates/render.ts renders the React tree ONCE and converts the text part with html-to-text + react-email's plainTextSelectors (what renderAsync({plainText:true}) does internally); byte-identical html AND text for all 12 auth template exports vs the old two renders; signup render 2.36 -> 1.40 ms (node, warm, mean of 200) — every renderEmail caller gets it. Delivery latency before: up to 5 min (cron '3-58/5 * * * *') + 2 renders; after: one Resend round trip inside the hook. NOT measured live (no prod access from this lane): Q300. GUARD: src/test/edge/auth-email-hook-inline-signup.test.ts (runs the REAL hook through the edge harness; red on origin/main 4 of 5; 5 @mutate registrations, all killed). Was: Signup email latency: the auth hook renders the template twice, then waits for a 5-minute queue drain. (Q165 carry-over, 2026-09-23.)** Evidence: supabase/functions/auth-email-hook/index.ts:211-214; process-email-queue '3-58/5 * * * *' (supabase/migrations/20260831190419_schedule_http_crons_missing_from_migrations.sql:95). Needs live check of cron.job and hook duration. From: docs/archive/FABLE-LEAD-2026-08-23.md.
...
- [ ] **Q300 Q258 follow-ups (2026-09-23).** (a) Measure live after functions-deploy ships auth-email-hook: sign up a test account (is_seed) and record hook duration (edge logs) and the email_send_log pending->sent gap; before = up to 5 min. (b) Decide whether recovery / magiclink / reauthentication / email_change also go inline (one-line change: add the type to SEND_INLINE in supabase/functions/auth-email-hook/index.ts; they are just as interactive as signup; Q258 was scoped to signup). (c) A Resend timeout inside the hook can still deliver, and the queued retry then sends a second copy; accept or add an idempotency key (Resend supports `Idempotency-Key`). Guard to extend: src/test/edge/auth-email-hook-inline-signup.test.ts.
- [ ] **Q301 Q239 follow-up: measure the Applicants panel live, and decide the 1-round RPC (2026-09-23).** (a) With the test poster account on a job with 3+ applicants, record throttled (1.6 Mbps / 150 ms) time from opening Applicants on /my-posts to the first ranked row, before (origin/main before cloud/q206-perf) and after; the item said ~7 s. (b) Optional: one SECURITY DEFINER RPC (poster-owned job only, block-filtered server-side) returning applications + safe profile + stats + signals would make it 1 round; needs lh-authz-rls review and a PGRST202 fallback to the current 2-round path. Guard to extend: src/pages/activity/activityActions/applicantsPanelRounds.test.tsx.
