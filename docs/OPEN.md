...
- **Queue (this file):** 51 done, 6 partly done (fixed, protection pending), 75 open. Source of truth for work.
...
**Queue: 132 items — 51 done, 6 partly done (fixed, protection pending), 75 open.**
...
- [ ] **Q130 helper_credentials.document_url is any text a member types (Q127 lane, 2026-09-23; MEDIUM).** Measured on prod: members may INSERT/UPDATE their own helper_credentials rows (RLS policies "Users can insert/update own credentials" key only on user_id; column grants and triggers not checked), and the only document test is CHECK helper_credentials_pending_review_needs_document = NULLIF(btrim(document_url),'') IS NOT NULL, which chr(8203) passes (evaluated on prod). get_pending_credentials lists such rows for admins. 1 of 1 rows with a document_url is not under its owner's folder. Not written to prove it (no test row created). Fix like Q127: allowlist the storage-path shape under the owner's folder + object existence, and check who may set status. **Confirmed live, read-only (Q127 re-verify lane, 2026-09-23):** RLS on (not forced); policies INSERT CHECK auth.uid()=user_id, UPDATE USING auth.uid()=user_id (no WITH CHECK), SELECT own, all TO public; no DELETE policy. Grants: authenticated has table SELECT only plus column INSERT (id, user_id, credential_type, license_number, license_state, trade_category, issuing_authority, document_url, expiration_date, created_at, updated_at) and column UPDATE (credential_type, document_url, license_number, expiration_date, trade_category, license_state, issuing_authority, updated_at) — status is not grantable. One trigger, trg_credential_status_server_owned (BEFORE INSERT OR UPDATE, enforce_credential_status_server_owned, SECURITY DEFINER, search_path=public, proacl postgres/service_role): a member INSERT is forced to user_id=auth.uid(), status='submitted'; a member UPDATE keeps OLD.status and is a no-op on a 'verified' row (RETURN OLD) but NOT on 'submitted'/'rejected'/'expired'. It never looks at document_url. So: (a) a member INSERT of a trade_license/insurance row with document_url=chr(8203) lands 'submitted' (the CHECK is the only test and btrim does not strip U+200B); (b) a member can swap document_url on a row already 'submitted' (queued for review) to anything, status unchanged; (c) credential_type identity/background_check/bond are exempt from the CHECK, so a 'submitted' row with no document at all is allowed; (d) a member can change credential_type on an unverified row. get_pending_credentials() reads helper_credentials and returns document_url (proacl includes authenticated). Rows: 3 total (statuses expired, rejected, submitted); the 1 with a document_url is a `data:image/png;base64,...` value on the seed helper-e2e account (trade_license, submitted, created 2026-09-13), i.e. the class already exists on prod. No client code writes helper_credentials (src/ reads only; stripe-webhook writes as service role), so an allowlist costs no real flow.
...
- [ ] **Q133 complete-signup fails the WHOLE signup if its credential path ever leaves the Q127 shape (Q127 re-verify lane, 2026-09-23; LOW, latent).** Read, not run (signup can't run on prod): complete-signup uploads first (service role), then one profiles UPDATE carries approval_status='approved' AND license_url/insurance_url; auto_pending_credentials refuses (22023) a path whose extension is not pdf/png/jpe?g/webp/heic, and the function answers 500 "Failed to update profile", so the account stays unapproved and the uploaded object is orphaned. Today it is safe only by coincidence of three lists: safeDocumentExt maps image/gif, heif, tiff to gif/heif/tiff and falls back to "bin" (supabase/functions/_shared/storageKeys.ts), but the user-documents bucket's allowed_mime_types (jpeg, png, webp, heic, pdf, measured live) refuse those uploads first (502). Add a guard that the trigger regex's extension set equals the bucket's allowed types mapped through safeDocumentExt, or make complete-signup write the credential in a separate update so a refusal cannot unapprove a signup.
