-- Q287 (docs/OPEN.md): an automatic close for 'cron-http-untagged'.
--
-- sweep_cron_http_failures (20260923172145, Q218) files one error_logs row per
-- day per ACTIVE cron.job whose command calls net.http_post without
-- cron_http_tag( (tags.source 'cron-http-untagged', tags.job = its name). The
-- row reaches ops_alert_ledger through trg_error_logs_zz_ledger, but
-- ops_alert_condition had no branch for it, so the item was 'manual' and could
-- only be closed by hand.
--
--  * ops_alert_condition: restated from its NEWEST definition
--    (20260923182022_ops_route_probe_close_rule.sql, Q94; its body matched live
--    pg_proc.prosrc by md5 on 2026-09-23) verbatim, plus ONE branch:
--    'cron-http-untagged' with a job is still failing while an active cron.job
--    of that name calls net.http_post without cron_http_tag( — the sweep's own
--    predicate. New items get verify_kind 'sql_condition' from ops_alert_record's
--    default hook; existing ones keep theirs (none existed live on 2026-09-23).
--  * 'cron-missed-slot' (Q207: a caught-up slot whose HTTP call failed) stays
--    MANUAL by design: the missed run cannot be re-asked, and the job's next
--    regular success does not redo the missed slot's work. No branch, so the
--    default hook labels it 'manual' (as every live item of it is).
--
-- Replay-safe: CREATE OR REPLACE, grants restated.
-- Grants: FROM PUBLIC, anon, authenticated; service_role only.

-- ledger close rule (newest body, 20260923182022 (Q94), + the Q287 branch)
