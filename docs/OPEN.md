...
- **Queue (this file):** 65 done, 13 partly done (fixed, protection pending), 82 open. Source of truth for work.
...
**Queue: 160 items — 65 done, 13 partly done (fixed, protection pending), 82 open.**
...
- [ ] **Q124 write-contract.snapshot.json may be stale on function grants (Q120 lane, 2026-09-23).** scripts/audit/write-contract.snapshot.json (last committed 2026-09-15) marks auto_pending_credentials `authenticated: true`; live has_function_privilege('authenticated', ...) is false. The other 176 functions it marks true are unchecked. Re-derive the snapshot from prod, diff, and make whatever check reads it fail when it disagrees with live grants.
...
- [ ] **Q154 Dashboard feed cards overlap after a job is optimistically removed (found by Q131, 2026-09-23).** After applying from the job sheet (card removed from the feed by useApplyFlow's onMutate), every remaining card's category tag sits on the bottom edge of the card above it and the gaps between cards are gone, at 375 and 1440, still so at +40s. Evidence: ~/.lh-shots/q131/after-375/5-after-retry-4000.png, after-1440/5-after-retry-4000.png (compare before-375/5-after-retry-1500.png, spaced correctly after a full reload). Cause not investigated (a virtualised list keeping stale row offsets is a guess, not a measurement).
...
- [ ] **Q157 HIGH (review of Q137, 2026-09-23): an admin action on a seed job with a REAL party says "could not be notified", falsely.** src/components/admin/AdminJobs.tsx notifyJobParty inserts with `.select("id")` + unwrapMutation; when trg_notifications_seed_boundary correctly drops the row the insert returns 0 rows, unwrapMutation throws WriteRejectedError, reports a `mutation_rejected` error, and the admin sees "That change is saved, but the poster could not be notified". Prod has 13 seed jobs with a real customer_id (1 disputed, 6 in_progress). Fix: ask the boundary first and treat TRUE as an expected skip with honest copy (never lower min to 0).
- [ ] **Q158 MEDIUM (review of Q137, 2026-09-23, dormant): charge-recurring-visits pages Slack every night for a seed series with a real party.** supabase/functions/charge-recurring-visits/index.ts treats `notifyRows.length < 2` as a defect; the Q137 trigger drops the real party's row by design, so the count is 1 and the run posts "Recurring visit funding had failures". Fix: ask notification_crosses_seed_boundary per row first and expect only the rows it allows.
- [ ] **Q159 MEDIUM (review of Q137, 2026-09-23): send-notification-email SENDS when the seed-boundary RPC is missing (PGRST202), with only a console.warn.** Fix: fail closed on every check error (a mail about a fake job cannot be recalled), make it loud, and fail the deploy when an RPC an edge function calls is missing on prod.
- [ ] **Q160 LOW (review of Q137, 2026-09-23): nothing tells "correctly suppressed" from "the check errored and dropped the row".** Both land in notification_logs; a bug that drops real-to-real notifications would be invisible. Fix: a monitor that raises an ops ledger item on any `seed boundary check failed` row.
- [ ] **Q161 HIGH (silent-failure review of Q131, 2026-09-23): error reporting and analytics can go permanently dark for the rest of a page session.** src/lib/errorLogger.ts flush() (and fanOutToObservability for sentry/posthog), src/lib/analytics.ts and src/hooks/useLoginTracking.ts load their clients through backgroundImport() with a bare catch that drops; the browser caches a failed dynamic import() for the life of the document, so one transient chunk failure drops every later report()/track() (worst case: the supabase-client chunk fails and the reports that would explain it are the ones lost). Q131 correctly removed the self-healing preloadError reload. Also LOW: backgroundImport's gate stays held if load() never settles. Fix: errorLogger persists via plain fetch to /rest/v1/error_logs (no lazy chunk); one "background import failed: <name>" row per session for analytics/posthog/sentry failures; gate released on a timeout.
