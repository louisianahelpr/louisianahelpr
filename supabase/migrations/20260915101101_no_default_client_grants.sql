...
--     no relation in public (all tables, views, sequences are postgres-owned).
--
...
-- scripts/check-migration-relation-grants.mjs fails CI on any new table or
-- view (a create statement) in a migration newer than this one without them.
-- The default entries staying gone is checked on the replayed schema
-- (db-smoke) and live (scripts/check-live-privileges.mjs, after every deploy
-- and nightly), both reading scripts/ci/client-default-privileges.sql.
--
