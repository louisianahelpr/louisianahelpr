...
- **Queue (this file):** 101 done, 10 partly done (fixed, protection pending), 87 open. Source of truth for work.
...
**Queue: 198 items — 101 done, 10 partly done (fixed, protection pending), 87 open.**
...
- [ ] **Q55 Three real errors in the 09-22 postgres logs, unrelated to the outage:**
  ...
  STATUS 2026-09-23 (cloud/q55-no-prod-from-unit-tests, not on main): (a) FIXED in tests, pending merge and a prod-log re-measure. src/test/prodNetworkGuard.ts (installed by setup.ts) refuses and records every fetch/XHR/WebSocket to *.supabase.co or the configured client host; afterEach fails the spec. Full unit suite with the guard: 13 specs leaked (8 Messages specs via thread_pins/thread_archives, flexibleScheduleUnlocksSubmit, backgroundImport, 3 complete-signup edge specs to x.supabase.co), all now mocked. vitest.config.ts test.env VITE_SUPABASE_URL moved to https://unit-test.invalid. Live-test allowlist is empty (none needed). Guard: src/test/prodNetworkGuard.test.ts (3 @mutate lines, each red). Re-measure: the `user-1` uuid errors from user-agent `node` should fall to 0 once merged. (b) gone; (c) still open.
