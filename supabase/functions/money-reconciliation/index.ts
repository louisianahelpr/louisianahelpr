...
serve(async (req) => {
  ...
  try {
    ...
    const checks = {
      ...
      // The mirror of releasedNoTransfer, and the class check for MS-2 / MS-3 /
      // HM-2: a refund that landed on a job whose Helpr was already paid. The
      // budget then leaves the platform twice — once to the Helpr (never clawed
      // back), once back to the poster — and 'refunded' is treated as "settled"
      // everywhere else, so nothing but this check ever sees it. Fires for any
      // job whose payment_status is 'refunded' while a LIVE (paid / pending-with-
      // id, i.e. isSettledTransfer) payout_transfers row exists for it.
    ...
    for (const job of jobRows) {
      ...
      // MS-2 / MS-3 / HM-2: the inverse divergence. A 'refunded' job that STILL
      // carries a live payout means the refund walked over a settled payout with
      // no reversal — the double-outflow. `paidJobIds` is exactly the settled
      // (paid / pending-with-id) set, so a 'refunded' job in it is the hole.
