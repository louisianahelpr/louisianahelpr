...
- **Queue (this file):** 44 done, 5 partly done (fixed, protection pending), 70 open. Source of truth for work.
...
**Queue: 120 items — 44 done, 5 partly done (fixed, protection pending), 71 open.**
...
- [ ] **Q120 A whitespace-only license_url/insurance_url marks a profile credential 'pending' with nothing to review (found by the Q102 review, 2026-09-23).** The BEFORE UPDATE trigger auto_pending_credentials on public.profiles treats a URL as present with `NEW.license_url IS NOT NULL AND NEW.license_url <> ''` (untrimmed), so ' ' sets license_status/insurance_status 'pending' and is_licensed/is_insured true. Only get_pending_credentials()'s nullif(btrim(..),'') keeps it off the admin queue; the member's profile would say "pending review" for ever and every other reader of license_status believes it. Fix the trigger at the source (blank = absent, normalise blank to NULL), measure prod rows, guard with a PGlite proof + a newest-definition vitest.
