...
- **Queue (this file):** 148 done, 26 partly done (fixed, protection pending), 129 open. Source of truth for work.
...
**Queue: 303 items — 148 done, 26 partly done (fixed, protection pending), 129 open.**
...
- [x] **Q225 V-008 (second half) saved-search / job-match notifications go out at funding with no Early Access delay. (Q165 carry-over, 2026-09-23.)** Evidence: supabase/migrations/20260911201653_job_match_notification_preference.sql:159-277 notifies title + budget with no early_access_cutoff(); the public-teaser half is fixed (20260904203654:338). From: docs/archive/launch-2026-09/VERIFIED_REPORT.md. **DONE (cloud/q283-q225-q289, 2026-09-23), migration 20260923185634:** every job_match notification is sent at jobs.created_at + `early_access_delay_minutes(recipient)` (the same tier table as `early_access_cutoff()`, so the same instant browse shows the job) or held in `job_match_holds`: the two trigger fan-outs deliver through `deliver_job_match()` (in-app + push + email), a BEFORE INSERT backstop `trg_notifications_zz_early_access_hold` holds instant-job-match's direct inserts, and `release_job_match_holds()` (cron release-job-match-holds, every minute, liveness 15 min) sends what is due, dropping holds whose job was hired/cancelled or whose recipient muted matches meanwhile. Guards: `src/test/jobMatchEarlyAccess.test.ts` (tier-table parity, every job_match writer classified two-way, backstop ordering, ACLs; 4 planted defects each red) + `src/test/jobMatchPreferenceGate.test.ts` updated; PGlite `src/test/pglite/jobMatchEarlyAccess.pglite.mjs` ALL PASS (30) applied 3x, NEW_MIGRATION=skip -> 10 FAILED. NOT YET VERIFIED LIVE (no prod access in the lane): after db-deploy, check `pg_get_functiondef('public.deliver_job_match'::regproc)`, `cron.job` has release-job-match-holds, and `job_match_holds` drains. The digests' young-job edge is Q305. Needs a REVIEW-ONLY pass (lh-silent-failure: the fail-open WARNING paths).
...
- [x] **Q283 No "message sent" analytics event (found by the Q72/Q222 lane, 2026-09-23).** Q72 names "message sent" as a key product event, but no `track(` call emits one, so it cannot be monitored. Build: emit `message_sent` where a message insert succeeds on the client (once per message), add it to KEY_EVENTS in scripts/lib/analyticsFreshness.mjs with ground truth from public.messages (non-seed senders), and a driver in src/test/keyEventsFire.test.tsx. Guard: src/test/analyticsFreshness.test.ts (emitted⇔listed) + keyEventsFire.test.tsx's KEY_EVENTS inventory. **DONE (cloud/q283-q225-q289, 2026-09-23):** `src/lib/messageSentEvent.ts` emits `message_sent` once per server row from the one client send path (`createSendHandlers` dispatchMessage, sendHandlers.ts), with job_id / has_attachment / is_reply and never the text; KEY_EVENTS monitors it against non-system `public.messages` rows (sender or job is_seed = test). Guards: `src/test/keyEventsFire.test.tsx` (driver + "message_sent counts a message once": a failed send emits nothing, a 23505 retry that reconciles emits once) and `src/test/analyticsFreshness.test.ts` (emitted⇔listed, "message sent" milestone). Red: emit removed -> 2 failed; dedupe removed -> 1 failed; KEY entry removed -> 2 failed.
...
- [~] **Q289 App Store / Play reviews that report a bug reach nobody (found by Q64, 2026-09-23).** Q64's text names App Store reviews, but no source ingests them: apple-app-store-notifications handles subscription server notifications only, and nothing reads the App Store Connect customer-reviews API or Play's reviews API. Build: a scheduled pull of new reviews (App Store Connect API key is an OWNER credential) that records rating <= 3 reviews as `user-report` ledger items (source app-store-review, verify manual). Guard when built: add the ingester to src/test/userReportsReachTheLedger.test.ts SURFACES. **BUILT (cloud/q283-q225-q289, 2026-09-23), first live run pending:** `scripts/check-store-reviews.mjs` runs daily in quota-monitor.yml (store_reviews job) with the existing ASC_* key (the TestFlight one): it reads App Store Connect customerReviews and records every review rated <= 3 stars newer than the ledger cursor as a `user-report` item (source app-store-review, verify manual, link to App Store Connect ratings); review text goes to the ledger only, never the public log. Fails closed on every read; a 403 means the key's role lacks Customer Reviews access and the message names that OWNER step. Google Play: no Android project in this repo, so nothing to read. Guards: `src/test/storeReviews.test.ts` (stub ASC + Management API; red on 3 planted defects), `src/test/userReportsReachTheLedger.test.ts` SURFACES (scan now covers scripts/), `src/test/liveCheckScriptsFailClosed.test.ts` (ASC marker + 3 hermetic cases). Tick [x] after the first scheduled run is green (or its 403 is resolved by the owner).
...
- [ ] **Q305 The two match digests can name or count a job inside a free member's Early Access window (found by Q225, 2026-09-23).** Q225 holds every real-time job_match notification until the job enters the recipient's browse feed, but the two daily digests are not held: daily-match-digest (edge, 13:12 UTC) names its oldest queued job by title with link /dashboard, so a job queued minutes before the run can be named inside the window, and sweep_daily_job_digest counts jobs created in the last 24h including ones minutes old. Build: skip queue rows / jobs younger than early_access_delay_minutes(recipient) (they roll into the next day). Guard: remove daily-match-digest from EDGE_KNOWN_GAPS and sweep_daily_job_digest's young-job note in src/test/jobMatchEarlyAccess.test.ts.
- [ ] **Q306 sweep_daily_job_digest counts jobs browse will not show (found by Q225, 2026-09-23).** Its newest definition (20260923172405) counts `jobs WHERE status = 'open'` with no funding filter (payment_status escrow/payout_pending/released) and no live-direct-offer filter, so "3 new jobs in Orleans" can include unfunded drafts and jobs addressed to one Helpr that the tapped-through dashboard never lists. Build: the same predicate the browse view and the two real-time producers use. Guard: extend src/test/jobMatchEarlyAccess.test.ts (or jobMatchPreferenceGate) to assert the digest's new_jobs CTE carries the funded + offer predicate.
