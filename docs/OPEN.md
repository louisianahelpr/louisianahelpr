...
- **Queue (this file):** 145 done, 27 partly done (fixed, protection pending), 126 open. Source of truth for work.
...
**Queue: 298 items — 145 done, 27 partly done (fixed, protection pending), 126 open.**
...
- [~] **Q224 CJ-003 no age-based retention for job_views, profile_views, notification_logs, login_history. (Q165 carry-over, 2026-09-23.)** Evidence: no age-based DELETE in any migration or edge function (only per-user purge); the Q167 pruners (20260923143321) skip these four; bus CJ-003 verified. From: docs/archive/launch-2026-09/VERIFIED_REPORT.md.
  STATUS 2026-09-23 (cloud/q282-q262-q224): FIXED IN CODE, [~]. Migration 20260923185658_prune_old_activity_logs adds prune_old_activity_logs() (daily cron prune-old-activity-logs '45 4 * * *', cron_work_expectations 30h, cron_catchup_policy catch_up=true with the 20260923145117 rows restated verbatim). Retention, justified from every reader in the migration header: job_views 90d (no reader today), profile_views 90d (only reader looks back 30d; compared with LOCALTIMESTAMP, the column is timestamp without tz), notification_logs 180d (widest monitor window 14d), login_history 180d EXCEPT each user's newest row (keeps get_user_last_active / admin last-login real for dormant users). Timestamp indexes added for the three tables that lacked one. GUARD: src/test/activityLogsHaveRetention.test.ts (every table named *_views/_log(s)/_history/_events/_requests/_details/_runs has an age DELETE in the newest body of a cron-scheduled function, or is on the exact NO_RETENTION list; red on the pre-fix tree listing exactly the four; 3 @mutate lines, each killed) + src/test/cronCatchUpPolicy.test.ts CATCH_UP_SAFE + src/test/pglite/pruneOldActivityLogs.pglite.mjs (old state red; after, 3x apply, per-table boundaries, dormant user keeps only the newest login, second run deletes 0, cron/expectation/policy once). NOT YET SEEN LIVE. NEXT (lead): land, verify `SELECT schedule FROM cron.job WHERE jobname='prune-old-activity-logs'`, run `SELECT public.prune_old_activity_logs()` once and record the counts, then tick.
...
- [~] **Q262 public.messages sender_id / receiver_id have no foreign key. (Q165 carry-over, 2026-09-23.)** Evidence: no migration defines one (grep supabase/migrations). Needs live check: pg_constraint on public.messages, and a dirty-row audit before adding FKs. From: docs/archive/OVERNIGHT-2026-08-18.md.
  STATUS 2026-09-23 (cloud/q282-q262-q224): FIXED IN CODE, [~]. sender_id: FK to auth.users(id) ON DELETE CASCADE (messages_sender_id_fkey), purge-compatible: purge_user_data 4c already deletes exactly `messages WHERE sender_id = p_user_id` before the auth delete, so the cascade only acts on paths that skip the purge. Added NOT VALID and VALIDATEd only when no existing row names a missing sender; otherwise a NOTICE gives the count and the rows are NOT deleted blind (attachment_url is the only pointer to their storage object). NOT VALID still enforces new rows and cascades. Index messages_sender_id_idx added for the cascade. receiver_id: NO FK, on purpose: purge keeps the messages the COUNTERPARTY wrote to the departed user; CASCADE would delete them, SET NULL fails (NOT NULL, threads key on it), NO ACTION breaks account deletion. Instead trigger messages_receiver_exists (BEFORE INSERT OR UPDATE OF receiver_id) refuses a message to a user who does not exist with 23503 (today only true by accident, via notifications_user_id_fkey, and not for flagged messages). GUARD: src/test/userIdColumnsHaveForeignKey.test.ts (sender FK with CASCADE, no receiver FK, purge deletes only sender rows, the trigger exists; @mutate lines killed) + PGlite personColumnsReferenceARealUser.pglite.mjs (old state red: sent messages of a deleted user stayed, a message to a missing receiver was accepted; after: cascade, purge order still succeeds and keeps the counterparty's message, 23503 on a missing receiver, orphan sender row leaves the FK NOT VALID and is kept). Needs lh-authz-rls / lh-silent-failure REVIEW (data model). NOT YET SEEN LIVE. NEXT (lead): land, read db-deploy's NOTICE; if it reports orphan senders, erase them with their attachments and `ALTER TABLE public.messages VALIDATE CONSTRAINT messages_sender_id_fkey`; verify pg_constraint, then tick.
...
- [~] **Q282 notification_preferences has no foreign key, so deleted accounts leave their row behind (found by Q205(d) cleanup, 2026-09-23).** Measured: 255 notification_preferences rows whose user_id has no auth.users row; pg_constraint shows 0 FKs on the table. Deleting the Q205(d) fixtures via the admin API left both rows (removed by hand). Add ON DELETE CASCADE to auth.users (after deleting the 255 orphans) or delete it in the account-deletion path; guard: a schema test that every table with a user_id column referencing a person has an FK or is on an exact allowlist.
  STATUS 2026-09-23 (cloud/q282-q262-q224): FIXED IN CODE, [~]. Decision: the column means the auth user id (handle_new_user inserts NEW.id from the auth.users trigger; the client upserts auth.uid(); the fan-out self-heal uses notifications.user_id, which references profiles.user_id, which CASCADEs from auth.users), and the 255 orphans were measured against auth.users, so the FK is to auth.users(id). Migration 20260923185657_person_columns_reference_a_real_user adds notification_preferences_user_id_fkey ON DELETE CASCADE NOT VALID, deletes rows whose user no longer exists (NOTICE gives the count), then VALIDATEs (NOT VALID first so a user deleted mid-migration is cascaded, not orphaned). The fan-out self-heal insert cannot hit the FK (its user has a profile). GUARD: src/test/userIdColumnsHaveForeignKey.test.ts (every table the migrations leave with a user_id uuid has an FK or is on the exact NO_FK list with its reason; a NO_FK table must be cleared by the newest purge_user_data or be on NOT_PURGED; red on the pre-fix tree: notification_preferences listed; 3 @mutate lines, each killed) + src/test/pglite/personColumnsReferenceARealUser.pglite.mjs (old state red: an admin-API delete left the row; after, 3x apply, orphan deleted, FK validated, cascade on delete). Layers read: DB (constraint, handle_new_user, fan_out_push_on_notification newest), client write (upsert own row), purge_user_data 4c. NOT YET SEEN LIVE (no prod access here). NEXT (lead): land, watch db-deploy, read its NOTICE (expect ~255 deleted), verify `SELECT convalidated, confdeltype FROM pg_constraint WHERE conname='notification_preferences_user_id_fkey'` = (true, c) and 0 orphans, then tick.
...
- [ ] **Q300 notification_dedupe_suppressions survives account deletion (found by Q282, 2026-09-23).** It has a user_id and no FK, and the newest purge_user_data (20260923145614) never names it, yet each row keeps the suppressed notification's title and link for that user. Fix: delete the user's rows in purge_user_data (restate from its newest definition) or give user_id an FK ON DELETE CASCADE, plus an age prune. Guard: src/test/userIdColumnsHaveForeignKey.test.ts NOT_PURGED (remove the entry; the two-way check then holds it).
- [ ] **Q301 Four log tables still have no age retention, and admin_audit_log's is undecided (found by Q224, 2026-09-23).** application_rate_log and profile_search_rate_log (read over at most 1 day), cron_catchup_runs (one claim row per caught-up slot) and str_processed_events (str-ical-sync's processed-event ledger) are never pruned; admin_audit_log is kept forever by default (owner decision: how long). Add them to a scheduled pruner with windows justified from their readers. Guard: src/test/activityLogsHaveRetention.test.ts NO_RETENTION (remove each entry as it is pruned; the list is exact).
