...
const EARLY_EXIT_OK: Allow[] = [
  ...
  { file: ".github/workflows/functions-deploy.yml", match: "Verify every function serves HEAD's build :: exit 0", reason: "the SUCCESS path (BR-024): check-edge-build-stamps.mjs read HEAD's stamp back from every function; it exits non-zero on a failed probe or missing header, and the retry loop fails after two redeploys" },
