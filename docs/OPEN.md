...
- **Queue (this file):** 145 done, 29 partly done (fixed, protection pending), 123 open. Source of truth for work.
...
**Queue: 297 items — 145 done, 29 partly done (fixed, protection pending), 123 open.**
...
- [~] **Q214 BulkDismissBar (My Posts selection mode) floats at a hand-typed `calc(env(safe-area-inset-bottom) + 80px)` (found by Q191, 2026-09-23).** Every other bottom clearance reads the shared `--bottom-nav-h` / `safe-nav` token (96px dock default, 0 with no dock) and `--safe-area-bottom`, not env() directly. Not measured whether 80px clears the dock on every device; measure, then move it onto the token. **FIXED 2026-09-23 (branch cloud/q265-layout), screenshots pending (lead ticks):** `bottom: var(--dock-clearance)` (the one dock token, Q265) — the bar now sits on the dock's 96px top edge + safe area instead of 80px via bare env() (16px lower than the dock clearance every other surface uses, and 0 inset inside PageTransition on WebKit). Messages' identical bulk bar moved with it. Guard: src/test/dockClearanceIsOneToken.test.ts ("Q214" case + zero-offender scan; red on base, @mutate lines shown red). Lead: screenshot /my-posts selection mode at 375 (bar above the dock, not touching it).
- [~] **Q215 /profile?tab=gift_card at 1440 showed content running under the right rail (seen in Q191's desktop shots, 2026-09-23).** Identical before and after Q191. The shots came from a scratch context with a mobile user agent at 1440, so it may be that context; shell-spacing.spec's real desktop context passed its past-the-right-edge check. Not verified either way: re-shoot in a desktop context and fix if real. **NOT REPRODUCED 2026-09-23 (cloud/q265-layout lane), lead screenshot pending:** measured on a local production build at 1440 (signed-in scratch session, Supabase network stubbed — diagnosis only, not prod verification): html `app-shell web-desktop desktop-rail side-panel-open`, .app-shell-frame x 0–1192, rail from x 1192, 0 elements inside the frame reach past x 1192; the same with an iPhone user agent at 1440, and the same on the pre-Q179 build (b381e8218^). Content card ends at x 1168. No code change. Lead: screenshot /profile?tab=gift_card at 1440 on prod (with received gift cards if the test account has any) and tick if clean.
...
- [~] **Q217 /legal search at 1440: the desktop right rail intercepts the magnifier click (found by the Q143 lane, 2026-09-23).** e2e/prod-audit/expanding-search-geometry.spec.ts "legal ... @1440" failed on a local build (this lane's code, which does not touch /legal or the rail): page.click on `button[aria-label="Search all policies"]` timed out because a button inside `nav[aria-label="Primary"]` (the fixed right rail) "subtree intercepts pointer events". Either the legal page's search trigger sits under the rail (a rail-inset defect, CLAUDE.md "desktop rail") or the spec clicks a covered point. Not investigated further; the shell lane (Q191) was editing shells at the time, so re-run on main first. Log: ~/.lh-shots/q142-q143/after.log. Guard to name when fixed: the same spec's legal@1440 case going green. **FIXED BY Q179 (b381e8218, 10:48, after this was filed at 10:04), verified by measurement 2026-09-23 (cloud/q265-layout lane):** on a local build of b381e8218^ at 1440 signed in, .app-shell-frame spanned x 0–1440 (right 0px, #root padding-right 248px) and the magnifier sat at x 1343–1387, elementFromPoint at its centre = the rail (the reported interception). On the current branch build: frame x 0–1192 (right 248px, #root padding 0), magnifier x 1095–1139, 53px clear of the rail at x 1192. (Scratch signed-in session, Supabase network stubbed: diagnosis only.) Shared-layer guard: src/test/rootRightInsetVsAppShellFrame.test.ts (both @mutate lines re-shown red today). NOT yet re-run: the named guard e2e/prod-audit/expanding-search-geometry.spec.ts legal@1440 needs prod test sessions this lane does not have. Lead: run it and tick when green.
...
- [~] **Q247 /pets at desktop shows two "Add a Pet" buttons when the user has no pets. (Q165 carry-over, 2026-09-23.)** Evidence: src/pages/PetProfiles.tsx:172-180 (title-row button, always rendered at lg) + :283-291 (empty-state button). From: docs/archive/VISUAL-2026-08-24.md. **FIXED 2026-09-23 (branch cloud/q265-layout), screenshot pending (lead ticks):** the title-row action is not rendered while the empty state is (`showEmpty`, one expression now gates the title-row action and both empty-state CTAs); with pets it returns as the only desktop add. Guard: src/pages/PetProfiles.oneAddWhenEmpty.test.tsx (one "Add a Pet" per breakpoint, empty and non-empty; red on base: 2 at desktop; @mutate shown red). Lead: screenshot /profile?tab=pets at 1440 with no pets and with one.
...
- [~] **Q265 Bottom-dock clearance is re-implemented per screen instead of owned by AppShell / PageScaffold (the BulkDismissBar part is Q214). (Q165 carry-over, 2026-09-23.)** Evidence: src/components/activity/AppliedJobsTab.tsx:287 (negative margin); BrowseMap.tsx:881,899; EmptyState.tsx:116; FormStep.tsx:187; BulkDismissBar.tsx:31; LegalTab.tsx:137; AppShell.tsx:44,120 applies clearance only when scrollable. From: docs/archive/OVERNIGHT-2026-08-18.md + docs/archive/01-screens.md (F-SCR-05). **FIXED 2026-09-23 (branch cloud/q265-layout), screenshots pending (lead ticks):** one token, `--dock-clearance` = `calc(var(--safe-area-bottom) + var(--bottom-nav-h))`, declared once in index.css :root (collapses with html.no-bottom-nav); Tailwind `safe-nav` = token + 1rem, new `dock` = token; AppShell's reserveBottomNav IS the token. Inventory from source: 22 hand-typed copies on base (AppShell, AppPage, Profile ×2, JobDetail, Activity, AppliedJobsTab, EmptyState, FormStep (bare env()), BulkDismissBar (env() + 80px), LegalTab (env() + 6rem stacked on Profile's column: a double inset, removed), BrowseMap ×2, browseMap/config, BrowseTasksFeed ×2, ConversationList ×2, sonner, ScrollToTop, MobileNav curtain, PublicLayout) → 0. Guard: src/test/dockClearanceIsOneToken.test.ts — two-way: offender scan is exact-zero (hand-typed dock literal beside the bottom inset, bare env(safe-area-inset-bottom), or a direct --bottom-nav-h read, in any non-test ts/tsx), consumer inventory floor (19 files) + AppShell/BulkDismissBar/ConversationList/Profile/AppPage must consume it; red on origin/main (22 offenders, all 6 cases), 7 @mutate lines each shown red. dist CSS checked: `.pb-safe-nav{padding-bottom:calc(var(--dock-clearance) + 1rem)}`, `.pb-dock{padding-bottom:var(--dock-clearance)}`, `:root{…--bottom-nav-h:96px;--dock-clearance:calc(var(--safe-area-bottom,0px) + var(--bottom-nav-h,96px))}`. Values unchanged for signed-in phone except BulkDismissBar/Messages bulk bar (+16px, now on the token), Legal tab (−96px double inset) and guest AppShell pages (no dock → no reserve).
...
- [ ] **Q300 Desktop web reserves the 96px dock clearance for a dock it hides (found by the Q265 lane, 2026-09-23; from source, not measured).** `html.web-desktop .mobile-nav-frame { display: none !important }` (index.css) hides the dock at >=900px, but nothing zeroes `--bottom-nav-h` there, so `--dock-clearance` (Q265) — AppShell's scroll padding, every `pb-safe-nav` page, the bulk-select bars, the toast floor — still reserves safe-area + 96px under the last row on desktop. Messages alone special-cases it (ConversationList `isWebDesktop`, guarded by inboxBottomClearanceMatchesTheDock.test.tsx, whose header names this exact class). Likely fix at the shared layer: `html.web-desktop { --bottom-nav-h: 0px }` inside the 900px media block, then drop Messages' special case; measure every AppShell/pb-safe-nav route at 1440 before and after (no dead band, last row not on the viewport floor). Visual: lead screenshots. Guard to extend: src/test/dockClearanceIsOneToken.test.ts.
