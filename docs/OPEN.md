...
- [ ] OWNER, blocks the numbers in the Supabase usage alert: add repo secret `SUPABASE_SERVICE_ROLE_KEY` (Supabase → Project Settings → API). Until it exists the weekly run reports UNMEASURED rather than a false "ok" — the workflow is already wired for it, no code change needed.
