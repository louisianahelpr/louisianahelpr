 ...
 * Q300: a banned account still signs in and can delete itself from
...
if (offenders.length) {
  const hint = {
    ...
    "auth-ban:writer": "a function sets auth.users.banned_until: remove it; a banned account must still sign in to reach /account-banned and delete itself (Q300)",
    "auth-ban:set": "an Auth-level ban locks the account out of in-app deletion: clear banned_until and ban through profiles.ban_status instead (Q300)",
    "gate:no-same-txn-carveout": "restore the app.ban_started_in_txn carve-out in enforce_ban_gate and trg_mark_ban_started_in_txn on profiles (20260923185224)",
