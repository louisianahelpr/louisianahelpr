 ...
 * src/components/job-card, src/pages/posts and src/pages/jobs. A new step file is covered the day it is written.
 *
...
const META_ROW = "src/components/job-card/JobCardMetaRow.tsx";

function files(): string[] {
  const list = ["src/components/job-card", "src/pages/posts", "src/pages/jobs"].flatMap((d) => walk(join(ROOT, d))).map((p) => relative(ROOT, p));
...
describe("activity card action buttons — ONE type size, one typeface", () => {
  ...
  it("finds the buttons at all (a scanner that matches nothing passes everything)", () => {
    ...
    expect(tags.some((t) => t.where.startsWith("src/components/job-card/JobActionRow.tsx"))).toBe(true);
  });
...
const ROW_PRIMITIVE_FILES = [
  ...
  "src/pages/jobs/appliedJobCard/DirectionsButton.tsx",
];
...
// @mutate src/components/job-card/JobActionRow.tsx | export const JOB_ROW_LABEL_CLASS = "text-ds-11 leading-tight font-medium"; | export const JOB_ROW_LABEL_CLASS = "text-ds-12 leading-tight font-medium";
