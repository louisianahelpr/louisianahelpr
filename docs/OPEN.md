...
- [x] 24h messaging lockout + embedded double-card: LANDED 2026-09-14 as 6e3d2f7d1 (embedded cards) + fda1d23aa (lockout, migration 20260914201350, supersedes 20260831053124 and the 90d23935d draft); branch `lockout-embedded` deleted. db-deploy run 34893651774 log: "Applying migration 20260914201350… Finished supabase db push" (functions-deploy not triggered: no function files). Vercel build-commit meta = fda1d23aa. PROD PROOF (375, prod web + prod DB):
  ...
  - [ ] REPORT, not fixed (found by the widened check, outside the approved scope): `GroupJobHelpers` (root `rounded-2xl liquid-glass p-5`) renders inside PostedJobCard's JobCardShell on group jobs, a bordered box inside the poster card. Exempted by name in `noNestedTrackerCard.test.ts` (REPORTED_NOT_FIXED, which fails if it stops being nested). Owner to decide whether to flatten.
...
- [ ] is_party_to_job(_job_id, _user_id) is callable by any signed-in user with an arbitrary user id. BUILT on branch `party-to-job`, awaiting prod proof + landing: migration 20260914210443 adds caller-bound `can_send_message_to_in_job(job, receiver)` (true only if auth.uid() may post in the job AND receiver is a party), the messages INSERT policy calls it, and is_party_to_job loses PUBLIC/anon/authenticated EXECUTE (service_role only). Live inventory 2026-09-14: its ONLY dependent is that policy; the 4 proof-photos storage policies call `is_party_to_job_folder(text)` (auth.uid()-bound, untouched). Wrapper also refuses a banned caller (mirrors trg_ban_gate_messages). PGlite: `node scripts/probes/party-to-job.probe.mjs` (44 expectations, 3x replay, 10 broken copies all caught, skip path no-op). Prod proof: proacl on is_party_to_job has no authenticated/anon; real two-account send poster→Helpr still 201; stranger RPC is_party_to_job → 42501; write-contract snapshot refresh.
- [ ] Design gap (lh-authz-rls review 2026-09-14, pre-existing, not introduced by party-to-job): is_party_to_job admits every APPLICANT as a message target, so any party who may post in a job (assigned Helpr, roster member, an applicant the poster messaged) can message, and so learn the identity of, the job's other applicants. Accepted residuals of the new wrapper, limited to jobs the caller may already post in: it does not model the block trigger or the 30/hour message rate. Owner call whether a non-poster should reach applicants at all.
