...
// @mutate src/pages/giftCards/CreditCard.tsx | const redeemable = !isExpired && isFunded && isMine && spendableStatus; | const redeemable = !isExpired && isMine && spendableStatus;
// @mutate src/pages/giftCards/CreditCard.tsx | const redeemable = !isExpired && isFunded && isMine && spendableStatus; | const redeemable = !isExpired && isFunded && spendableStatus;
// @mutate src/pages/giftCards/StatusPill.tsx | const s = MAP[status] ?? UNKNOWN; | const s = MAP[status] ?? MAP.available;
// @mutate src/pages/giftCards/CreditCard.tsx | perspective === "received" &&\n    !isExpired && isFunded | !isExpired && isFunded
