// @mutate supabase/migrations/20260923090536_db_saturation_monitor.sql | ELSIF p_source = 'db-saturation' THEN | ELSIF p_source = 'db-saturation-x' THEN
...
// @mutate supabase/migrations/20260923090536_db_saturation_monitor.sql | JOIN firsts f ON f.jobname = m.jobname | CROSS JOIN LATERAL (SELECT COALESCE((SELECT min(m2.rn) FROM marked m2 WHERE m2.jobname = m.jobname AND NOT m2.suspicious), 2147483647) AS first_ok) f
