...
- **Queue (this file):** 145 done, 24 partly done (fixed, protection pending), 127 open. Source of truth for work.
...
**Queue: 296 items — 145 done, 24 partly done (fixed, protection pending), 127 open.**
...
- [x] **Q227 DONE 2026-09-23 (cloud/q297-tooling): the css-minifier lesson (docs/lessons/CLAUDE-lessons.md#css-minifier) now prescribes `grep -o "backdrop-filter:none" dist/assets/*.css | wc -l`, with the `-c` failure mode spelled out (counts matching LINES, not occurrences, so it collapses every count above 1 on a minified one-line bundle down to "1"). No script in the repo had the same bug (grep -c usages elsewhere are one-match-per-line SQL dumps, unaffected). GUARD: src/test/cssMinifierLessonCountsOccurrences.test.ts (red on the old wording, green on the fix). Was:** The CSS-minifier lesson prescribes grep -c, which counts LINES of a one-line bundle and cannot tell the two states apart. (Q165 carry-over, 2026-09-23.) Evidence: docs/lessons/CLAUDE-lessons.md:246; use grep -o ... | wc -l. From: docs/archive/launch-2026-09/VERIFIED_REPORT.md.
...
- [x] **Q230 DONE 2026-09-23 (cloud/q297-tooling): direct offer, saved-search match, job-match fan-out, tip and cron each get a named test in notifications.spec.ts calling skipUncovered() with a code-grounded reason (the exact trigger function and why the two shared REST accounts cannot drive it — notify_helper_on_direct_offer, notify_saved_searches_on_new_job, notify_helpers_on_job_post, notify_helper_on_tip, sweep_job_start_reminders et al.). GUARD (two-way): src/test/notificationsSpecAnnotatesEveryNamedLeg.test.ts — asserts each of the 5 exact annotation phrases is present in the spec AND that each named trigger function still exists in the migration corpus; red on origin/main (all 5 phrases absent), green on the fix. NOT driven live in this session (no prod/browser access) — building real coverage for these 5 legs (vs. an honest skip) is follow-up work, filed as Q299. Was:** notifications.spec.ts says untestable legs are "annotated uncovered", but direct offer, saved-search match, job-match fan-out, tip and cron notifications have neither an annotation nor live coverage. (Q165 carry-over, 2026-09-23.) Evidence: e2e/journeys/notifications/notifications.spec.ts:39 vs the only uncovered calls at :100/:114/:168/:188. From: docs/archive/notification-inventory.md.
...
- [x] **Q245 DONE 2026-09-23 (cloud/q297-tooling): race-class client/edge keys are now `${kind}:${file}:${line}::${what}` (call site), not `${kind}:${file}::${what}` disambiguated only by a scan-order occurrence count — the old scheme let a NEW unguarded write inserted above an already-baselined one of the same shape silently inherit the baselined bare key while the real, already-reviewed write shifted onto the "new" key instead. scripts/race-class-baseline.json's 32 client/edge keys migrated (all 56 hits still baselined, 0 new/0 stale). GUARD: src/test/raceClassGuard.test.ts updated to match on shape (kind/file/what) at any line rather than a hardcoded line number, so a fixture's own line count can move without going stale; proved red→green with a synthetic prepend-above-an-existing-hit scenario (the old scheme's genuinely-new write silently took the baselined key; the new scheme never does). Was:** Race-class baseline keys are file::columns, so a new unguarded write of the same shape in an already-audited file stays green. (Q165 carry-over, 2026-09-23.) Evidence: scripts/race-class-baseline.json (e.g. :44), guard scripts/check-race-class.mjs. From: docs/archive/lifecycle-writes-audit-2026-09-14.md.
...
- [~] **Q251 FIXED IN CODE 2026-09-23 (cloud/q297-tooling), [~]: posterCells() now marks every pending_approval cell `reachable: "unreachable"` with a code-grounded reason and `shots: []` (state-sweep.spec.ts's drive loop filters on `shots.length > 0`, not `reachable`, so shots had to change too or the relabel would be cosmetic). Code-level proof the status is retired, not a live row count (no prod access this session): jobSubmitHelpers.ts — "NO `status` KEY IS EVER WRITTEN HERE… the businesses / business_members tables were removed, no call site ever set it… Don't reintroduce a status override here"; migration 20260831232522_retire_business_approval_residue.sql migrated every existing pending_approval row to in_progress and deliberately kept the (undroppable) enum label. GUARD: src/test/stateMatrixRetiresPendingApproval.test.ts (imports enumerateStates() directly — added e2e/happy-path/state-matrix/stateMatrix.ts to tsconfig.app.json's include for it — red on the pre-fix code, green after). NOT YET VERIFIED LIVE: the item's own "jobs with status='pending_approval' = 0" check needs prod access this session didn't have. Was:** The state matrix still emits the retired pending_approval cells as auto-reachable. (Q165 carry-over, 2026-09-23.) Evidence: e2e/happy-path/state-matrix/stateMatrix.ts:449-455. Needs live check: jobs with status 'pending_approval' = 0, then relabel unreachable. From: docs/archive/STATE_MATRIX.md + docs/archive/IOS_COVERAGE.md.
...
- [x] **Q256 DONE 2026-09-23 (cloud/q297-tooling): sweepCore.ts's VARIANTS default (unset SWEEP_VARIANTS) is now `[phone-light, phone-dark]`, and ui-sweep.yml's own default falls back to `'phone-light,phone-dark'` in lockstep. Desktop stays opt-in (`all`, or an explicit tag list) — it's a genuinely different layout, not a repaint. COST NOTE: roughly doubles a default run's screenshot/comparison volume (two themes at one width instead of one); accepted because audit finding #11 (docs/archive/FULL-SURFACE-2026-08-31.md) is the same 35-screen dark-mode-only defect class this item names. GUARD: src/test/uiSweepDarkModeDefault.test.ts (text-based, avoids importing sweepCore.ts's Playwright deps into src/test — same convention as axeGateCoversWcag22aa.test.ts); red on origin/main (neither file had the new default pattern), green on the fix. NOT changed (report, filed as Q299): empty-state-sweep.spec.ts and error-state-sweep.spec.ts each carry their OWN separate variant-parsing copy (not sweepCore.ts's), still defaulting to phone-light only when run standalone (outside the workflow, e.g. vacuity's mutation phase) — this item's evidence named only ui-sweep.yml:203 and sweepCore.ts:159, so left alone rather than guessed at. Was:** The UI sweep runs phone-light only by default; dark mode is opt-in. (Q165 carry-over, 2026-09-23.) Evidence: .github/workflows/ui-sweep.yml:203 (|| 'phone-light'); e2e/happy-path/sweepCore.ts:159. From: docs/archive/FULL-SURFACE-2026-08-31.md.
...
- [~] **Q263 FIXED IN CODE 2026-09-23 (cloud/q297-tooling), [~]: check-migration-grants.mjs now resolves the ARRAY[]+FOREACH+`EXECUTE format('GRANT/REVOKE … ON FUNCTION %s …', f)` idiom back to real function names (dollar-quote-tagged DO-block scoped, so two sibling loops reusing the same loop-var name for different arrays — exactly 20260919192559's client_fns/trigger_fns shape — don't collapse into one). `--all` now flags 15 (12 distinct names), down from 25; the 7 group-roster false positives (rpc_group_member_confirm etc.) are gone. The remaining 15 are turned into an exact two-way baseline, scripts/migration-grants-baseline.json (registered in src/test/baselinesAreTwoWay.test.ts), so `--all` reports "0 new, 0 stale, N known-unverified" instead of a raw unexplained dump; proved both directions red (dropping a real entry → new-violation red; adding a fake entry → stale red) then restored green. Changed-files mode (the actual migration-lint.yml PR gate) is fixed outright — a migration using this idiom, like 20260919192559, no longer fails. NOT YET LIVE-VERIFIED: the 15 baselined entries still need pg_proc.proacl against prod (no prod access this session) before they can shrink further. From: docs/archive/OVERNIGHT-2026-08-18.md. Was:** check-migration-grants --all flags 25 functions; at least 7 are false positives (it cannot see the dynamic EXECUTE format('GRANT ...') at 20260919192559:1039-1045), the rest unverified. (Q165 carry-over, 2026-09-23.) Evidence: needs live check: pg_proc.proacl for count_profiles, get_approved_helpers, is_category_taxable, get_hero_parishes, can_review_job, cleanup_observability_tables, cleanup_stripe_webhook_events, get_user_business_ids, is_user_verified_business_member, get_time_credit_balance, redact_audit_snapshot, cancellation_fee_percent.
...
- [x] **Q297 DONE 2026-09-23 (cloud/q297-tooling): the @mutate find-string now reads `export const PROFILE_TAB_BODY_CLASS = "space-y-section";`, matching the current declaration (occurs exactly once). The ASK to "make vacuity fail CI when a registration's find-string is absent anywhere" was ALREADY TRUE going in — scripts/vacuity/run.mjs's collectMutations() checks every registered guard (tracked + untracked, not just changed-since-origin/main) for a find-string occurring exactly once, and index.mjs unconditionally fail()s on every such error on every push (vacuity.yml runs on push to main and on PRs) — so this exact stale registration was already wired to keep the workflow red, it just hadn't been fixed. Proved with collectMutations() directly: the old find-string errors "not in ProfileTabBody.tsx", the new one is clean. GUARD: the mechanism itself, re-verified above (no new code needed for the second half of the ask). Was:** Vacuity registration broken on main: e2e/journeys/profile-tab-shell-parity.spec.ts:169 @mutate find-string `export const PROFILE_TAB_BODY_CLASS = "space-y-4";` is no longer in src/components/profile/ProfileTabBody.tsx (seen by the Q94/Q128/Q132 lane's vacuity run, 2026-09-23; 14a40c072 Q191 changed the section scale). Update the find-string to the current declaration. Guard: scripts/vacuity/index.mjs itself (it reports the stale registration).
...
- [ ] **Q299 Gaps noticed fixing Q230/Q256/Q251/Q263 (cloud/q297-tooling, 2026-09-23), none driven live (no prod/browser access in that session).** (a) empty-state-sweep.spec.ts and error-state-sweep.spec.ts each carry their OWN separate variant-parsing copy (not e2e/happy-path/sweepCore.ts's, which Q256 fixed), still defaulting to phone-light only when EMPTY_SWEEP_VARIANTS/ERROR_SWEEP_VARIANTS is unset (e.g. vacuity's mutation phase, which sets RUN_EMPTY_SWEEP/RUN_ERROR_SWEEP but not the variants var) — decide whether to default those to dark too, or point them at sweepCore.ts's ALL_VARIANTS so there is one list, not three. (b) The 5 notification legs Q230 named (direct offer, saved-search match, job-match fan-out, tip, cron) got an honest skipUncovered() each, not live coverage — building real tests needs verifying the direct-offer write path (jobs.offered_to_helper_id via the client, not just the trigger), a saved_searches fixture that actually matches a freshly posted job, and confirming the shared helper account's profile matches on job-match fan-out; none of that could be verified without prod/browser access this session. (c) Q251's "jobs with status='pending_approval' = 0" and Q263's 15 migration-grants-baseline.json entries both still need a live check (pg_proc.proacl / a row count) against prod before they can shrink further — code-level evidence is strong for Q251 (no write path exists) but neither was measured live.
