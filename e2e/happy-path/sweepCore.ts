...
import {
  loadAxeBaseline,
  classifyAxeAgainstKnown,
  toBaselineEntries,
  normalizeRoute,
  AXE_BASELINE_REL_PATH,
  type AxeViolationHit,
} from "./knownAxeViolations";
...
export interface ScreenResult {
  ...
  /**
   * EVERY axe violation's rule id on this screen (unlike topViolations,
   * which is ranked and capped at 5) — what the two-way baseline gate
   * (knownAxeViolations.ts) matches against. See assertSweepGate.
   */
  axeViolations?: { id: string; impact: string | null }[];
  /**
...
export async function captureScreen(
  ...
  try {
    ...
    result.axeViolations = axe.violations.map((v) => ({ id: v.id, impact: v.impact ?? null }));
 ...
 *
 * FIVE things fail the run:
 *   - any axe violation (AXE_TAGS — wcag2a/2aa/21a/21aa/22aa + best-practice,
 *     see axeTags.ts, Q181) not on the two-way baseline (knownAxeViolations.ts);
 *     a violation that IS on the baseline is reported, not failed — see below;
 ...
 *
 * The colour-contrast checks were added 2026-09-02 and are not a refinement —
 * they are the check itself. axe never put a single colour-contrast result in
 ...
 *
 * The axe-violation baseline (Q181, 2026-09-23) exists for the SAME reason
 * knownContrastFailures.ts does — turning AXE_TAGS on to wcag22aa +
 * best-practice for the first time can surface real, pre-existing debt across
 * ~150 screens in one commit, and an all-or-nothing gate over that either
 * blocks everything until it is all fixed, or gets muted by whoever needs to
 * ship. Neither is acceptable, so the known set is written down and tracked
 * instead — see knownAxeViolations.ts's header for the two-way rules and for
 * why the baseline file itself does not ship in this change (no prod access
 * from the session that wrote this: the file is generated by this gate's
 * FIRST run in CI and must be reviewed and committed by hand).
 *
 * If this is red on a violation NOT on the baseline, FIX THE SCREEN. Do not
 * narrow the tag set, filter by impact, or add an undeserved baseline entry
 * to get it green — a muted gate is the state this change was made to get
 * out of.
 */
export function assertSweepGate(testInfo: TestInfo): void {
  ...
  // axe violations against the two-way baseline (knownAxeViolations.ts). See
  // the GATE doc-comment above for why this is baselined at all and where the
  // baseline file comes from.
  const axeHits: AxeViolationHit[] = results.flatMap((r) =>
    (r.axeViolations ?? []).map((v) => ({
      screen: r.name,
      variant: r.variant ?? "?",
      route: normalizeRoute(r.url),
      id: v.id,
      impact: v.impact,
      line: `${r.index} ${r.name} (${r.variant}) @ ${r.url} — ${v.id} (${v.impact ?? "?"})`,
    })),
  );
  const sweptScreenVariants = new Set(results.filter((r) => r.status === "ok").map((r) => `${r.name}|${r.variant}`));
  const axeBaseline = loadAxeBaseline();

  if (axeBaseline === null) {
    // First run against this tag set (or the baseline file was deleted): there
    // is nothing to classify against yet. Write out what THIS run found, in
    // the exact shape a human commits as the baseline, into OUTPUT_DIR — which
    // a11y-webkit-prod.yml already uploads as a build artifact — and fail
    // loudly rather than silently treating "no baseline" as "no violations".
    const generated = toBaselineEntries(
      axeHits,
      `auto-generated ${new Date().toISOString().slice(0, 10)} on the axe tag set extending to wcag22aa+best-practice (Q181) — review before trusting, then commit`,
    );
    const genPath = resolve(OUTPUT_DIR, "axe-baseline-generated.json");
    writeFileSync(genPath, JSON.stringify(generated, null, 2));
    expect(
      false,
      `axe baseline missing: ${AXE_BASELINE_REL_PATH} does not exist. Generated ${generated.length} ` +
        `violation(s) found in THIS run at ${genPath} (uploaded in this job's a11y-prod-* artifact). ` +
        `Review each entry — this is a decision to grandfather real WCAG 2.2 AA / best-practice debt, ` +
        `not a formality — then commit it as ${AXE_BASELINE_REL_PATH} (and register it in ` +
        "src/test/baselinesAreTwoWay.test.ts's JSON_TWO_WAY, see knownAxeViolations.ts's header). " +
        "This gate stays red until that file exists.",
    ).toBe(true);
  }

  const axeClassified = classifyAxeAgainstKnown(axeHits, axeBaseline ?? [], sweptScreenVariants);

  if (axeClassified.allowed.length) {
    testInfo.annotations.push({
      type: "known-axe-violations",
      description:
        `${axeClassified.allowed.length} recorded, unfixed axe violation(s) — not new:\n  - ` +
        axeClassified.allowed.join("\n  - "),
    });
  }

  expect(
    axeClassified.fresh.map((f) => f.line),
    `NEW axe violations (AXE_TAGS: ${AXE_TAGS.join("/")}), not in ${AXE_BASELINE_REL_PATH} — this change ` +
      `introduced them; fix them, do not baseline them (full report: ${OUTPUT_DIR}/a11y-report.json):\n  - ` +
      axeClassified.fresh.map((f) => f.line).join("\n  - "),
  ).toEqual([]);
  expect(
    axeClassified.stale,
    `Stale entries in ${AXE_BASELINE_REL_PATH} — recorded but not seen on a screen this run DID sweep. ` +
      `The baseline is only allowed to shrink, so a fixed entry has to be deleted:\n  - ` +
      axeClassified.stale.join("\n  - "),
  ).toEqual([]);
