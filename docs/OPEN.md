...
- **Queue (this file):** 102 done, 10 partly done (fixed, protection pending), 91 open. Source of truth for work.
...
**Queue: 203 items — 102 done, 10 partly done (fixed, protection pending), 91 open.**
...
- [ ] **Q192 Q180 follow-ups from the lh-authz-rls review (2026-09-23).** (a) OWNER DASHBOARD: confirm Auth > URL Configuration allow-lists https://www.louisianahelpr.com/account-pending (Signup.tsx and both resends pass it as emailRedirectTo; /auth/v1/settings does not expose the list and config.toml sets none, so it is unverified; on a miss GoTrue falls back to the Site URL, which still works via MarketingRedirect). (b) The server enforces email confirmation ONLY by GoTrue refusing unconfirmed sessions; no RLS policy or RPC checks auth.jwt() email_verified, so a session minted another way (admin createUser without email_confirm) could write. Decide whether Q180 needs a server half (data-model change, own review). (c) An unconfirmed account that is also banned/denied lands on /account-pending, not /account-banned or /account-denied, until it confirms (0 such rows live). (d) Not checked: whether the desktop right rail renders for an unconfirmed user on public pages (/help, /support, legal).
...
- [ ] **Q184 Code health (from TODO.md).** `any` in non-test src/: 164 on 2026-09-23 (was 385), burn down money/auth paths first; keep extracting god components; move Profile/Dashboard/Messages to `useProfile()` (0 call sites today); raise test coverage; fix `~/.npm/_cacache` ownership on the dev box.
  - STATUS 2026-09-23 (`any` half, branch cloud/q184-any-burndown): measured by TypeScript AnyKeyword nodes (`node scripts/any-baseline.mjs`), non-test src/ went 154 -> 67 (48 -> 21 files) against origin/main bef41564c; the line grep above went 163 -> 84 (it also counts the word in comments and misses `Record<string, any>`, hence the AST measure). Every money/auth/admin file is at 0 except one deliberate cast in AdminJobs.tsx (Q204) (payments, tips, boosts, W-9, referral cash-out, disputes, settlements, admin, signup, complete-profile). GUARD: src/test/anyRatchet.test.ts holds scripts/any-baseline.json exact per file, both ways. Still open: the 67 left (useUserProfileData 21, archivedConversations 8, useMapKitJs 6, …), god-component extraction, useProfile(), coverage, the ~/.npm ownership.
- [ ] **Q203 Admin "Earned (Worked)" ignores group-job splits (found by Q184, 2026-09-23).** `JobsTab.calcEarning` divides by `j.helpers_needed`, but the jobs query in `src/components/admin/adminusers/useOpenProfile.ts` never selects `helpers_needed`, so it is always undefined and the divisor is always 1: a group job's full budget is credited to each helper in the admin user-detail totals and sort. Fix: add `helpers_needed` to that select (runtime change, admin display only), then make `AdminProfileJob.helpers_needed` required. Guard idea: a test that every column `calcEarning` reads is in the select.
- [ ] **Q204 Admin status override is a jobs write the race-class guard cannot see (found by Q184, 2026-09-23).** `AdminJobs.tsx` status override calls `(supabase.from("jobs").update as any)(updates).eq("id", …)`. Typed as a plain `.update(updates)`, `scripts/check-race-class.mjs` reports it as a new hit (`client:src/components/admin/AdminJobs.tsx::opaque:updates`): a status write with no `.eq("status", previousStatus)` predicate, so an override can silently clobber a concurrent transition (e.g. a payout release). The cast was kept so Q184 stays type-only. Fix: add the status predicate (the `unwrapMutation` zero-row message already covers "it may have already moved"), drop the cast, lower scripts/any-baseline.json. GUARD: src/test/raceClassGuard.test.ts then covers it.
