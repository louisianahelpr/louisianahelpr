...
- **Queue (this file):** 173 done, 37 partly done (fixed, protection pending), 102 open. Source of truth for work.
...
**Queue: 312 items — 173 done, 37 partly done (fixed, protection pending), 102 open.**
...
- [~] **Q306 Re-consent copy names a Privacy change that did not happen (found by Q210(d), 2026-09-23).** FIXED IN Q210(d): TermsReconsentDialog.tsx and CompleteProfile.tsx wrote `privacy_version: LATEST_TERMS_VERSION` into legal_acceptances, harmless only while the two were equal; they now write the new client LATEST_PRIVACY_VERSION ("Jun 2026", mirrored from LEGAL_PRIVACY_VERSION), guarded by src/test/termsReconsentOnBump.test.tsx. STILL OPEN: the dialog says "a material update to our Terms and Privacy Policy" though only the Terms changed on 2026-09-23; the copy should name what changed (owner to word it). Guard to extend: src/test/termsReconsentOnBump.test.tsx.
...
- [ ] **Q315 ops_alert_condition restatements can silently drop a branch (Q287 review nit, 2026-09-23).** routeProbeCloseRule.test.ts now pins its comparison to the Q94 file, so nothing fails if a future `CREATE OR REPLACE ops_alert_condition` loses a `p_source = '...'` branch. Only 4 of the 11 are spot-checked in cronHttpRequestsAreTagged.test.ts. Fix: add a guard that each definition's set of p_source values is a superset of the previous definition's. A second nit: the ledger fingerprint normalises digits, so the jobs `cleanup-7d` and `cleanup-30d` would share one item. Live cron.job has no job names containing digits today (measured by the reviewer).
