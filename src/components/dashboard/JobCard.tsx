...
const JobCard = ({ job, effectiveFee, currentUserId, showApply = true, onApply, onReport, onSelect, index = 0, isExpanded = false, onToggleExpand }: JobCardProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Footer — always visible: apply + flag */}
      <div className="px-4 py-2 border-t border-border/40 bg-muted/15 flex items-center justify-between" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 text-[11px] text-muted-foreground min-w-0">
          ...
          {!isExpanded && (job.posterReviewCount ?? 0) > 0 && (
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {showApply && !isOwnJob && (
            <Button size="sm" className="h-7 px-3 text-xs bg-primary text-primary-foreground hover:bg-primary/90 rounded-lg" onClick={() => onApply(job.id)}>
              <Send className="w-3 h-3 mr-1" /> Apply
            </Button>
          )}
          <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-destructive hover:bg-destructive/10" onClick={() => onReport(job.id)}>
