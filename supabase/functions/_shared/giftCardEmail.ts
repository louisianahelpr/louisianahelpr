 ...
 */
export async function sendGiftCardEmail(opts: GiftCardEmailOpts): Promise<boolean> {
  const apiKey = Deno.env.get("RESEND_API_KEY");
  if (!apiKey) {
    console.error("[giftCardEmail] RESEND_API_KEY not configured — gift email not sent");
    return false;
  }
  try {
    ...
    console.error("[giftCardEmail] failed to send gift email", err);
    return false;
  }
}
