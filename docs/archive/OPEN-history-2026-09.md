# OPEN.md history — archived verbatim

Snapshot 2026-09-23 of docs/OPEN.md: every section above the live QUEUE, moved here unedited by the
pre-audit reconcile (queue item Q16). Line N of the original file is line N + 9 here. The live list is
docs/OPEN.md; each item that was still open is carried there with its current status. A reconcile log of
every item closed (fixed / obsolete) with its evidence follows the verbatim text.
...
- Inventory: `docs/archive/notification-inventory.md`.
...
- [ ] **`error-state-sweep` is still unproven** (272 tests, ~30 min). Target identified and unique: `src/lib/supabaseResult.ts`, `if (error) {` → `if (false) {`, neutering `unwrap()` across 48 call sites so failed fetches render silent empty states. Caveat for whoever runs it: the file's header says enabling it "would just red main", so the baseline may be red and the run return `inconclusive` rather than `killed`.
- [ ] **These sweeps structurally cannot exercise the Big 7 completeness gate.** `buildFakeProfile` sets `is_legacy_user: true` and the gate reads `!isLegacy && … && !isProfileComplete(profile)`, so it short-circuits. A mutation there scores SURVIVED for a reachability reason, not a real one.
- **Environment trap worth knowing:** `~/Library/Caches/ms-playwright/` was deleted mid-session and every Playwright test died with `Executable doesn't exist`. The runner does not print baseline output, so that returns `inconclusive`, which reads as "your spec is red". Discriminator: an environment failure repeats the IDENTICAL error on every test; a real one varies by screen. `npx playwright install chromium webkit` fixes it.
...
- [ ] Untracked in the main checkout: docs/archive/naming-and-dead-code-2026-09-13.md (uses the pre-rename gift card name, makes the gift-card naming guard fail locally) and docs/audit/morning/. Commit the audit doc renamed-clean or delete it; keep morning/ gitignored.
...
## Reconcile log 2026-09-23 — items closed with evidence

Measured 2026-09-23 against origin/main and live prod (read-only SQL, gh, git). "Line" is the original
docs/OPEN.md line; add 9 for this file. "fixed" needed a sha or conclusive current code/DB state.

| Line | Item | Verdict | Evidence |
|---|---|---|---|
| 265 | 20 of 21 messy-input forms got FormSpecs; ReuploadIdDialog is dead code (stated gap); CI run not yet confirmed | fixed | 20/21 forms wired with FormSpecs (sha not located for this later session); 21st (ReuploadIdDialog) is dead code, stated as a gap. Real prod-audit CI run to confirm coverage number was not verified in-session. |
| 358 | #1618 prod-audit — collateral of cron outage, not 36 defects | fixed `56b96599c` | Root cause was the pg_cron outage + a downstream consent-gate bug; both fixed and verified in CI (56b96599c / 1bbd60705). |
| 379 | #1582 press — three causes (proof-photo 400s, disabled switches, nav-tab mis-press) all fixed same day | fixed | All three originally diagnosed causes fixed; press sweep still has 199 unrelated failures tracked at line 403. |
| 520 | framer-motion removed from critical path (393→353 kB gz) | fixed | Fixed and measured: framer-motion is off the critical path (393→353 kB gz), guarded by check-deferred-vendors.mjs. |
| 2117 | Four burnt-sienna/0.5 contrast separators (EarningHistory, ReviewsTab, RecentTransfers, SavedHelperCard) — later fixed by f148bb5c6 | fixed `f148bb5c6` | Fixed: all four burnt-sienna/0.5 contrast separators (EarningHistory, ReviewsTab, RecentTransfers, SavedHelperCard) now inherit their line color (f148bb5c6); low-alpha-text-inventory.mjs reports zero remaining. |
| 2307 | vitest flaky: vacuityGate/discardedQueryFilters ENOENT race under parallel load | fixed `6f5a696d6` | Fixed: the vacuityGate/discardedQueryFilters ENOENT race was patched (6f5a696d6). |
| 2520 | FIX ON BRANCH fix-anon-grants awaiting land (H-004/AUTHZ-02 table-grant revokes) | fixed `421d1658e` | Fixed: fix-anon-grants merged to main (421d1658e), excess anon grants revoked. |
| 2605 | LEAK: poster could re-arm a direct offer on an assigned/funded job | fixed `697b9f6bf` | Fixed: direct-offer re-arm on hired jobs blocked (697b9f6bf). |
| 2606 | PARKED BRANCH sec-hardening needs rebuild onto current live bodies before landing | fixed `2dff30d63` | Fixed: sec-hardening work landed via sec-hardening-v2 merge (2dff30d63). |
| 2607 | Deploy-order risk: JOB_READABLE_COLUMNS gains arrival columns before migration lands | obsolete | Obsolete: the arrival-columns migration and deploy both completed 2026-09-15, deploy-order risk window is closed. |
| 2611 | auto_pending_credentials: restore business-rename re-review rule | fixed `5d59bbb02` | Fixed: business-rename re-review rule restored (20260915191526). |
| 2612 | Parish badges: remove entirely (client + get_helper_parish_badges + get_top_helpers_by_parish) | fixed `5d59bbb02` | Fixed: parish badges removed (20260915191403_remove_parish_badges.sql). |
| 2614 | Direct-offer re-arm on a hired job: block it (migration) | fixed `697b9f6bf` | Fixed: migration blocks direct-offer re-arm once a Helpr is hired. |
| 2631 | PARKED branch sec-hardening (task 1 default grants, task 2 NULL-uid guards) | fixed `2dff30d63` | Fixed: sec-hardening landed as sec-hardening-v2 (2dff30d63); duplicate of line 2606. |
| 2638 | Branch role-neutral-copy-v2 not merged (125 strings, 58 files) | fixed `094453ee1` | Fixed: role-neutral-copy-v2 landed on main (094453ee1). |
| 2639 | Class guard roleNeutralCopy.test.ts + allowlist | fixed `094453ee1` | Fixed: roleNeutralCopy.test.ts guard shipped with the merged branch. |
| 2642 | Guard gap fixed: interpolated role token invisible to first scanner version | fixed `094453ee1` | Fixed: interpolated-token guard gap closed as part of the merged branch. |
| 2644 | v2 rebuild carried the rule onto arrivalRule.ts / edge escalation / map pin label | fixed `094453ee1` | Fixed: v2 rebuild's arrival-copy changes landed with the merged branch. |
| 2855 | Merge discarded-query-filters into main | fixed `650e32a3c` | Fixed: discarded-query-filters branch merged (650e32a3c). |
| 2868 | Pick up: watch nightly re-run chain to green (nightly-webkit, a11y-webkit-prod, e2e-journeys) | obsolete | Obsolete: one-off 2026-09-15 nightly re-run chore, long superseded by later nightly runs. |
| 2972 | 9x [auth] admin role lookup failed — role state is UNKNOWN | fixed `02a4f9fbd` | Fixed: admin role lookup failure class addressed (02a4f9fbd). |
| 2988 | Dispatch prod-touching workflows one at a time (concurrency: prod-load) | obsolete | Obsolete: superseded by the standing CLAUDE.md rule on serialized prod-touching workflows. |
| 2989 | Re-run nightly-webkit, a11y-webkit-prod, e2e-journeys to green in order | obsolete | Obsolete: one-off 2026-09-15 re-run sequencing, long since superseded. |
| 3012 | Plus has no Priority Placement in browse ranking (get_ranked_open_jobs ignores 'plus') | fixed | Fixed: Plus tier now gets priority-placement boost in get_ranked_open_jobs (20260915051752). |
| 3014 | QUEUED fix-completion-columns-v2: completion columns server-owned (H-001, H-002) | fixed `2a896a7d1` | Fixed: completion columns made server-owned via rpc_helper_mark_done + trigger, merged (2a896a7d1). |
| 3016 | VN-33 arrival gate shipped (c6f025b66); lead still to screenshot; owner questions open | fixed `c6f025b66` | Fixed (core): VN-33 arrival gate shipped and prod-verified; only a screenshot confirmation remained open. |
| 3035 | Nudge alpha (0.65 -> ~0.70) or drop the low-alpha treatment on Earnings subtitles/chevrons | fixed | Fixed: Earnings tab subtitles/chevrons now use 0.7 alpha, clearing the 4.5:1 floor. |
| 3070 | OWNER: decide whether the gutter narrows and to what at each breakpoint | fixed `485569f19` | Fixed: VN-37 gutter decision made and shipped (485569f19), confirmed in the visual-notes tracker. |
| 3124 | OWNER: add GitHub repo secret SLACK_WEBHOOK_URL | fixed | Fixed: SLACK_WEBHOOK_URL repo secret is set (added 2026-09-14). |
| 3130 | c7a12050 still decided/pending on prod as of 2026-09-14 19:33 UTC | obsolete | Obsolete: stale one-off prod-seed state note from 2026-09-14, long since cycled through. |
| 3173 | PROBABLE HIGH: helper can PATCH dispute_status escalated->open to force 72h sweep payout | fixed | Closed by 20260915033734_dispute_markers_server_owned.sql (trigger present in repo) — the note's own re-verify condition is satisfied. |
| 3201 | OWNER ACTION: add STRIPE_TEST_SECRET_KEY repo secret | fixed | STRIPE_TEST_SECRET_KEY repo secret exists (added 2026-09-14) — owner action done. |
| 3223 | Re-measure AdminRoute admin-role-indeterminate error_logs after deploy | fixed | Re-measured live: 0 new AdminRoute-indeterminate error_logs rows since the deploy (last one 2026-09-13 05:15 UTC, matching the pre-deploy count of 121). |
| 3277 | instant_book_claim dead code path in useApplyFlow.ts + 5 call sites referencing dropped instant_book column | fixed | instant_book dead-code path removed; only an explanatory comment remains in useApplyFlow.ts. |
| 3317 | .claude-scratch/ (129M) inside ESLint project, causing the repo's only 2 lint errors | fixed | .claude-scratch/ directory is gone from the worktree; the lint-error source no longer exists. |
| 4704 | Seed dispute c7a12050 stuck at decided/execution_status=pending re-alerts every 30h; worth clearing so alert stops | fixed | Seed dispute c7a12050 is now withdrawn (was decided/pending), confirmed live — the repeating settlement alert no longer fires on it. |
| 4992 | Cached prod sessions can be dead (revoked) but still pass PostgREST; journeys/a11y-prod harnesses didn't verify | fixed `86697cf65` | Fixed: liveSession.ts unifies session-liveness verification across journeys, prod-audit, and a11y-prod (86697cf65). |
| 4997 | Same-frame double-click on Apply Now can beat the disabled-state re-render | fixed `27b2e9b86` | Fixed: useApplyFlow.ts adds a synchronous in-flight ref guard (27b2e9b86). |
| 5003 | A half-propagated deploy's chunk-reload 10s guard refuses a second reload, leaving the crash screen | fixed `0f641f534` | Fixed: chunkReload.ts now allows a second bounded reload after a 30s backoff, capped at 2 attempts (0f641f534). |
| 5021 | Sibling buttons of different heights never compared across the visual sweep | fixed `32f72734e` | Fixed as a side effect of the overlay-sweep baseline fix (32f72734e): siblingMismatch findings now fail CI via the baseline diff. |
| 5030 | Anon surface contract failed CI on one gateway 504 | fixed `91693fbd8` | Fixed: retry added (91693fbd8) and UI sweep is green on recent main runs. |
| 5033 | Parallel worktree sessions collide on the shared test-server port 4173 | fixed `c3b133e39` | Fixed: each worktree now gets its own path-derived HAPPY_PATH_PORT by default (c3b133e39). |
| 5047 | Direct-offer expiry never tells the poster (only offers expired in the last 5 min of the hourly run get notified) | fixed `ccf8ee053` | Fixed: expire_pending_direct_offers now notifies every expired offer via RETURNING, not just the last-5-minutes window (ccf8ee053). |
| 5086 | Tracking map throws "reading '_leaflet_pos'" on My Jobs when a card unmounts mid-zoom | fixed `11f891fcd` | Fixed (11f891fcd) and then made moot: TrackingMap was later ported entirely from Leaflet to Apple MapKit (43834d572). |
| 5100 | Log Out signs the user out on EVERY device (global scope) instead of just this one | fixed `692758927` | Fixed: plain Log Out now scopes to "local"; only the explicit Sign Out Everywhere button uses global scope (692758927). |
| 5109 | Saving weekly availability can wipe the whole week (DELETE then INSERT as two requests) | fixed `eeed1d5f0` | Fixed: availability save is now one atomic RPC transaction, not separate DELETE+INSERT (eeed1d5f0). |
| 5117 | Two payout buttons stack on one card ("Request My Payout" and "I'm Done — Request Payout") | fixed `dac7ce49d` | Fixed: both CTAs unified to a single "Mark Job Complete" primary action, guarded by singlePrimaryCta.test.tsx (VN-21, dac7ce49d). |
| 5135 | Public profile shows "ID verified" and "Verification in progress" chips together | fixed `f0ace886a` | Fixed: public profile RecognitionRow drops the Verification-in-progress chip entirely (VN-13, f0ace886a), test-guarded. |
| 5162 | overlay-sweep: 67 tests, 66 route probes, exactly ONE assertion — findings never fail CI | fixed `32f72734e` | Fixed: overlay-sweep now asserts findings against a checked-in baseline and checks landedOn vs requested route, closing the report-generator gap (32f72734e, 2026-09-21). |
| 5194 | src/lib/cppRouting.test.ts AND .test.tsx both exist, covering different cases, hazard of divergence | fixed `3b8cc67d4` | Fixed: the two cppRouting test files were merged into one, keeping both proofs (3b8cc67d4). |
| 5275 | OWNER DECISION NEEDED: widen prod-load concurrency group to the other six prod-touching workflows to stop three 503s | fixed `63b53bc72` | Fixed: prod-load concurrency group was widened to 16 prod-touching workflows including press-every-control, e2e-real-backend, and prod-audit (63b53bc72). |
| 5328 | HARNESS DEFECT: a truthful browser-denied-notification-permission toast is scored as a press failure, drowning the real signal | fixed `3960ba9a1` | Fixed: press-every-control now allow-lists the truthful notification-permission-denied toast instead of scoring it as a failure (3960ba9a1). |
| 5380 | Three test.skip(true, ...) instances exist repo-wide; one (PhoneCluster selector) should be deleted, two are defensible | fixed | Fixed: the flagged mobile-viewports.spec.ts self-disarming skip (PhoneCluster selector) is gone; the two remaining skips were called defensible by the item itself. |
| 5571 | Profile photo (and 7 other pickers) keyboard-unreachable, fixed + fileInputsKeyboardReachable.test.t | fixed | Profile photo (and 7 other pickers) keyboard-unreachable, fixed + fileInputsKeyboardReachable.test.ts, but uncommitted |
| 5572 | aria-label on 15 role-less elements, fixed + noAriaLabelOnGenericElements.test.ts, but uncommitted | fixed | aria-label on 15 role-less elements, fixed + noAriaLabelOnGenericElements.test.ts, but uncommitted |
| 5573 | Admin KPI tiles ragged, fraud filter select mismatched height — fixed, detector tightened, uncommitt | fixed | Admin KPI tiles ragged, fraud filter select mismatched height — fixed, detector tightened, uncommitted |
| 5575 | One icon Button held at 44px by parent's [&_button]:h-11 (AdminTopBar) — intentional, detector now i | fixed | One icon Button held at 44px by parent's [&_button]:h-11 (AdminTopBar) — intentional, detector now ignores it |
| 5576 | Sweep mock data is thin (7 jobs, mostly empty tables); expand seed + heavy-content variant | obsolete | Sweep mock data is thin (7 jobs, mostly empty tables); expand seed + heavy-content variant |
| 5577 | No end-to-end user-journey suite; build journeys for every flow on real backend | fixed | No end-to-end user-journey suite; build journeys for every flow on real backend |
| 5580 | instant_book_claim RPC called but dropped from prod; useApplyFlow.ts branch is dead code | fixed | Fixed (lead, 2026-09-23): instant_book_claim is gone from prod (to_regprocedure NULL) and useApplyFlow.ts keeps only an explanatory comment (:70); no call remains. Instant Book cut in dbc124dc4. |
| 5587 | Accessibility sweep in WebKit (axe currently only runs in Chromium) | fixed | Accessibility sweep in WebKit (axe currently only runs in Chromium) |
| 5592 | Concurrency lane: enforce_application_job_state reads job without a lock — accept/cancel races | fixed `20260913014328` | Concurrency lane: enforce_application_job_state reads job without a lock — accept/cancel races |
| 5599 | Seed prod test data for the two test accounts in every state | fixed | Seed prod test data for the two test accounts in every state |
| 5605 | Uncommitted mock fixture change (edge-function stub bodies) discarded per this decision | fixed | Uncommitted mock fixture change (edge-function stub bodies) discarded per this decision |
| 5609 | Prod test data for every state (seed lane resumed on prod) | fixed | Prod test data for every state (seed lane resumed on prod) |
| 5623 | Capture the App Store screenshots from the finished app (manual launch-day step) | fixed | Capture the App Store screenshots from the finished app (manual launch-day step) |
| 5642 | DOB wheel: Tab changes the date; options should be tabIndex=-1 with arrow-key handling | fixed | DOB wheel: Tab changes the date; options should be tabIndex=-1 with arrow-key handling |
| 5644 | git stash is shared across every worktree; use a WIP commit, never git stash | fixed | git stash is shared across every worktree; use a WIP commit, never git stash |
| 5678 | is_party_to_job(_job_id, _user_id) still authenticated-callable with an arbitrary user id — needs it | fixed | is_party_to_job(_job_id, _user_id) still authenticated-callable with an arbitrary user id — needs its own fix |
| 5679 | Dead code deletions (wip agent-a7af3ac420980771a, 70f851af7) plus owner-approved column drops | fixed | Dead code deletions (wip agent-a7af3ac420980771a, 70f851af7) plus owner-approved column drops |
| 5684 | Combobox keyboard model / race fixes (terminal closed -> agent) reminder | obsolete | Combobox keyboard model / race fixes (terminal closed -> agent) reminder |
| 5692 | OWNER: add STRIPE_TEST_SECRET_KEY repo secret | fixed | OWNER: add STRIPE_TEST_SECRET_KEY repo secret |
| 5736 | Untracked docs/archive/naming-and-dead-code-2026-09-13.md (old gift-card name) and docs/audit/morning/ | fixed | Untracked docs/archive/naming-and-dead-code-2026-09-13.md (old gift-card name) and docs/audit/morning/ — commit or delete, keep morning/ gitig |
| 5745 | RUNNING: integrations + secrets audit (Slack, Sentry, Supabase, Vercel, GitHub, Stripe test, Resend/ | obsolete | RUNNING: integrations + secrets audit (Slack, Sentry, Supabase, Vercel, GitHub, Stripe test, Resend/PostHog/MapKit) — report only |
| 5762 | OWNER QUESTION: should jobs.offered_to_helper_id be hidden from non-posters or cleared on decline/ex | fixed | OWNER QUESTION: should jobs.offered_to_helper_id be hidden from non-posters or cleared on decline/expiry? |
| 5773 | QUEUED — OWNER DECISION 2026-09-14: hide jobs.offered_to_helper_id from non-posters | fixed | QUEUED — OWNER DECISION 2026-09-14: hide jobs.offered_to_helper_id from non-posters |
| 5774 | offer-privacy branch BUILT + gate-clean, NOT LANDED, NOT PROBED ON PROD as of this snapshot | fixed | offer-privacy branch BUILT + gate-clean, NOT LANDED, NOT PROBED ON PROD as of this snapshot |
| 5776 | REPORT: npm run deadcode (knip) red on origin/main — unused HelperAvailabilityDisplay.tsx — someone | fixed `986262fcd` | REPORT: npm run deadcode (knip) red on origin/main — unused HelperAvailabilityDisplay.tsx — someone must decide delete vs revive |
| 5777 | PRE-DECLARED: write-contract.snapshot.json carries get_job_offer_targets before prod has it; clears | obsolete | PRE-DECLARED: write-contract.snapshot.json carries get_job_offer_targets before prod has it; clears itself on deploy |
| 6417 | stalled-completion-reminder and arrival-confirm-reminder linked to unread ?job= param | fixed `0bf3fb1c5` | Fixed (0bf3fb1c5): admin escalation alert links now point to real routes, not the unread /admin?job=. |
| 6576 | Ratchet does not cover 327 colocated specs or src/test/edge/** | fixed | Fixed: colocated specs and src/test/edge/ are now both COMPLETE in the burn-down score (0 owed each). |
| 6612 | in_progress job without confirmation stamps offers a control the server refuses | fixed `a17f8e955` | Fixed (a17f8e955): the confirmation gate is now keyed on the button's target step, not on rail position. |
| 6622 | Arrival copy density: 10 lines of overlapping amber/muted prose on claimed-no-GPS card | fixed `a17f8e955` | Fixed (a17f8e955): arrival copy split so amber carries only the blocker and muted only the benefit; block size ~429->178 chars. |
| 6726 | Post-job FAB missing aria-current="page" | fixed | Fixed: MobileNav FAB now sets aria-current="page" when on /post-job. |
| 7157 | Mechanical hover-tint migration across activity/admin/profile/dashboard/etc. components (147 entries) | fixed | Fixed: controlInteractionLedger.json is now empty — all hover-tint migration entries closed out. |
| 7369 | ScreenHeaderRow should own an expandingSearch slot instead of 3 callers hand-rolling it | fixed | Fixed: ScreenHeaderRow now exposes an expandingSearch slot, adopted by ConversationList. |
| 7397 | LOST WORK claim (search lane) — later shown WRONG, work was not actually lost | obsolete | Obsolete — superseded 2026-09-20: the search lane work was not actually lost; it landed. |
| 7408 | Status tab row overflows the phone viewport (372px in 320), Cancelled/Done clip, no scroll affordance | fixed `e24ed9535` | Fixed (verified on built app 2026-09-20): all 5 Activity tabs now visible with zero overflow at 375/414/1440. |
| 7444 | 414 breakpoint contradicted its own contract (short labels shown where full words were documented) | fixed | Fixed: 414px breakpoint now shows full tab-label words, matching its documented contract. |
| 7460 | A test file's min-[Npx]: interpolated string disabled Tailwind's entire min-[Npx] variant family (40 of 42 arbitrary-width classes dead across 10 files) | fixed | Fixed and verified in built CSS 2026-09-20: all 8 arbitrary Tailwind breakpoints now emit real media queries. |
| 7483 | /legal search field was 107px at 320 | fixed `f148bb5c6` | Fixed (f148bb5c6): /legal search field now 222px at 320, verified on production build. |
| 7491 | 59/188 (originally) guards proven able to fail; target 188/188 (later rescoped to 723 total) | fixed | Fixed/current: guard burn-down score is now 715/723 proven + 8 exempt = 723, 0 still owed in every scope. |
| 7561 | Notification panel bottom edge jumps 71px in one frame when a row leaves and list is shorter than max height | fixed | Fixed 2026-09-22 (self-documented): notification panel now animates height on row exit; edge-move dropped from 100.4px to 13.1px. |
| 7583 | Admin People header badge says 'ID Not Submitted' on seed profiles that have an id_document_url (reads wrong column) | fixed `9ffdaa72b` | Fixed (9ffdaa72b, Q6): admin ID badge no longer says 'ID Not Submitted' for people mid-Stripe-check; now exhaustively matches profiles_idv_status_check. |
