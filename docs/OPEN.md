...
- [x] **Q32 DONE 2026-09-23: all 11 stranded PRs decided; 0 open besides the bot refresh PR #1713.** Landed on main: #1622 (AGENTS.md drift-detect time, 74a9faaa5), #1639 (Sentry normalises plain Supabase error objects, 2010e8f4f; 2 tests red without it), #1550+#1599+#1549 (upload-artifact v7, download-artifact v8, cache v6 everywhere, e93f20d97), #1653+#1652 (33 npm minor/patch bumps, npm ci clean, 6b37a29cd). Closed as already on main: #1621 (ForgotPassword -> errorLogger landed in ede8b383f; its lockfile/fixture commits are churn), #1623/#1624 (identical "dev" flag lockfile churn; main's lockfile already carries it, bbd2c0672), #1529 (its hunks landed in 72e98ba58/5c8fadfbb; the AASA-403-skip hunk rejected: it turns a real failure into a skip). #1607 was already closed 12:47Z. RECURRING CAUSE FOUND (#1713): refresh PRs on github.token dispatched only test.yml+vitest.yml, which produce 1 of main's 3 required checks, so no refresh PR could ever auto-merge; default now dispatches e2e-happy-path.yml + mobile-viewports.yml too (e460c6aef). GUARD: src/test/refreshWorkflowsOpenPrs.test.ts ("refresh PRs dispatch every workflow behind main's required checks", red on the old default; @mutate). #1713's own red was main's (same ESLint error and same 4 vitest failures as main at its base 1f9af64e4). Dependabot PRs strand because main is red, not from a missing auto-merge: see Q300.
...
- [ ] **Q300 Dependabot PRs strand on a red main; decide auto-merge (found by Q32, 2026-09-23).** Every dependabot PR carries main's reds (on 09-23: stale inventories, deadcodeRatchet, the userRealtimeBus ESLint error), so none stays green long enough to merge. Branch protection requires only 3 checks (Vitest unit tests + 2 Playwright jobs, API 2026-09-23); "Lint, type-check, build, test" is NOT required, so `gh pr merge --auto` on dependabot would merge a bump that breaks typecheck. OWNER DECISION: add "Lint, type-check, build, test" to main's required checks, then a workflow can enable auto-merge on dependabot minor/patch PRs. Guard when done: extend REQUIRED_CHECKS in src/test/refreshWorkflowsOpenPrs.test.ts.
