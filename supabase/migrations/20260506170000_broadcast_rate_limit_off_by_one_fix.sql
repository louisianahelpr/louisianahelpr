-- Fix off-by-one: previous version allowed 4 broadcasts in 24h despite
-- "max 3" comment because count > 3 lets the 4th through.
-- Rewriting to >= 3 enforces the documented limit.
--
-- (Note: 20260506180000 below moves rate-limit enforcement to a BEFORE
-- INSERT trigger so the row is never created over-limit. This migration
-- still fixes the legacy fan-out function for any direct callers.)
